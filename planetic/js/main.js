...
const viewTilesetInCesiumBtn = document.getElementById('view-tileset-in-cesium-btn');
    ...
    // --- Scene Panel Viewer Logic --- (Declarations moved to DOMContentLoaded)
    // const sceneTypeDropdown = document.getElementById('scene-type-dropdown'); // Redundant
    // const sceneViewer3dtile = document.getElementById('scene-viewer-3dtile'); // Redundant
    // const sceneViewerUsd = document.getElementById('scene-viewer-usd'); // Redundant for now
    // const sceneViewerI3s = document.getElementById('scene-viewer-i3s'); // Redundant for now
function setupEditorPanelLogic() {
    ...
    const editorUrlMap = {
        ...
        'wings3d': 'packages/wings3d/html/index.html',   // Adjusted path relative to planetic app root
...
document.addEventListener('keydown', function(event) {
    ...
    // The following block (approx lines 894-910 in previous state) related to
    // sceneViewerThreejs, sceneViewers array, sceneViewerMap, and an incomplete
    // sceneTypeDropdown listener is being removed as it conflicts with the more
    // complete scene type switching logic implemented earlier within DOMContentLoaded.
    // The necessary element lookups (sceneTypeDropdown, scenePanelViewerContainer, sceneViewer3dtile)
    // are already handled in that more complete section.

    // The entire conflicting 'if (sceneTypeDropdown)' block and its 'else'
    // (previously lines approx 901-932) is removed.
    // The correct logic for sceneTypeDropdown is handled earlier in DOMContentLoaded.
...
window.sendGltfToJanusIfActive = function() {
    ...
    try {
        ...
        const payload = {
            ...
            geoBounds: window.latestGeneratedGltf.geoBounds,
            geoCenter: window.latestGeneratedGltf.geoCenter,
            tileZoomLevel: window.latestGeneratedGltf.tileZoomLevel,
            tileBoundingBox: window.latestGeneratedGltf.tileBoundingBox
            // orientation: window.latestGeneratedGltf.orientation // Add if/when orientation is calculated
        };
        console.log("[GLTF_TO_JANUS] Payload for JanusWeb:", payload);
        xrIframe.contentWindow.postMessage(payload, '*');
    } catch (error) {
...
console.log(`UI List Click: Setting highlightedGlobeGroupId to: ${state.highlightedGlobeGroupId}`);
                    ...
                    if (firstFeature) {
                        ...
                        // Auto-load GLTF into Scene Panel logic removed due to pivot to "maximize thumbnail" approach.
                    } else {
...
// Removing Scene Panel specific JS:
// - Global variables: mainSceneRenderer, mainScene, mainSceneCamera, mainSceneControls, mainSceneTextureToggleCheckbox
// - State variables: window.state.mainSceneShowTextures, currentMainSceneGltfData, currentMainSceneModelName
// - Functions: initMainSceneViewer, loadGltfIntoMainScene, updateMainSceneViewerSize
// These are being removed to pivot to a "maximize thumbnail" approach instead of a separate scene panel.

function updateThumbnailViewerSize() {
    const container = document.getElementById('tileset-thumbnail-3d'); // Thumbnail container
    if (!container) {
        console.error("[THUMBNAIL_RESIZE] Thumbnail container ('tileset-thumbnail-3d') not found!");
        return;
    }
    if (thumbnailRenderer && thumbnailCamera) {
        requestAnimationFrame(() => { // Use rAF to get dimensions after potential reflow
            const currentWidth = container.clientWidth;
            const currentHeight = container.clientHeight;
            console.log(`[THUMBNAIL_RESIZE] In rAF. Container current dimensions: W=${currentWidth}, H=${currentHeight}`);

            if (currentWidth > 0 && currentHeight > 0) {
                if (thumbnailCamera.aspect !== currentWidth / currentHeight) {
                    thumbnailCamera.aspect = currentWidth / currentHeight;
                    thumbnailCamera.updateProjectionMatrix();
                }
                thumbnailRenderer.setSize(currentWidth, currentHeight);
                console.log("[THUMBNAIL_RESIZE] Thumbnail renderer size set to:", currentWidth, currentHeight);
            } else {
                console.warn("[THUMBNAIL_RESIZE] Thumbnail container has zero/small dimensions in rAF. Renderer not resized.");
            }
        });
    } else {
        console.warn("[THUMBNAIL_RESIZE] Thumbnail renderer or camera not ready for resize.");
    }
}

function initThumbnailViewer(containerId, tilesetGroupId, showTextures = true) { // Added showTextures parameter
    console.log(`[THUMBNAIL_3D] Initializing for container ${containerId}, tileset ID: ${tilesetGroupId}, Show Textures: ${showTextures}`);
    ...
    thumbnailRenderer.outputColorSpace = THREE.SRGBColorSpace; // Correct color space for textures
    container.appendChild(thumbnailRenderer.domElement);

    // Lighting
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.8); // Increased ambient intensity
    thumbnailScene.add(ambientLight);
    const directionalLight = new THREE.DirectionalLight(0xffffff, 0.9); // Increased directional intensity
    ...
    if (window.latestGeneratedGltf && window.latestGeneratedGltf.id === tilesetGroupId && window.latestGeneratedGltf.data) {
        ...
        // Reverted: Stringify the GLTF data again, as thumbnail was reported black after removing it.
        loader.parse(JSON.stringify(window.latestGeneratedGltf.data), '', (gltf) => {
            ...
            // Apply/Remove textures based on showTextures parameter
            // Process materials based on showTextures AFTER the model is fully parsed
            if (!showTextures) {
                console.log(`[THUMBNAIL_TEXTURE_TOGGLE] showTextures is FALSE. Applying simple materials. GroupID: ${tilesetGroupId}`);
                gltf.scene.traverse((child) => {
                    if (child.isMesh && child.material) {
                        const originalMaterialName = child.material.name || 'Unnamed Original';
                        const simpleMaterial = new THREE.MeshStandardMaterial({
                            color: 0x444444, // Darker grey for wireframe
                            roughness: 0.8,
                            metalness: 0.2,
                            wireframe: true, // Show wireframe when textures are off
                            name: `SimpleWireframeMaterialFor_${child.name || 'mesh'}`
                        });
                        if (Array.isArray(child.material)) {
                            console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Mesh ${child.name || 'N/A'} has array material. Original names: ${child.material.map(m=>m.name).join(', ')}. Replacing with simple.`);
                            child.material = child.material.map(() => simpleMaterial.clone());
                        } else {
                            console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Mesh ${child.name || 'N/A'} has single material. Original name: ${originalMaterialName}. Replacing with simple.`);
                            child.material = simpleMaterial;
                        }
                    }
                });
            } else {
                 console.log(`[THUMBNAIL_TEXTURE_TOGGLE] showTextures is TRUE. Using materials from GLTF loader. GroupID: ${tilesetGroupId}`);
                 gltf.scene.traverse((child) => {
                    if (child.isMesh && child.material) {
                        if (Array.isArray(child.material)) {
                             console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Mesh ${child.name || 'N/A'} (textures ON) using loader materials (Array): ${child.material.map(m=>m.name).join(', ')}`);
                        } else {
                             console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Mesh ${child.name || 'N/A'} (textures ON) using loader material: ${child.material.name || 'Unnamed'}`);
                        }
                    }
                 });
            }
            // If showTextures is true, the materials from the GLTF loader are used directly.
            // Since we re-parse the GLTF data each time initThumbnailViewer is called for a toggle,
            // we always get fresh materials from the loader when showTextures is true.
            console.log(`[THUMBNAIL_3D] Textures ${showTextures ? 'enabled' : 'disabled'} for model.`);
    ...
    function openTilesetDetailsModal(feature) {
        ...
        initThumbnailViewer('tileset-thumbnail-3d', groupId); // Initial call, will be updated by toggle

        // --- Texture Toggle Logic for 3D Thumbnail ---
        const textureToggleCheckbox = document.getElementById('details-texture-toggle');
        if (textureToggleCheckbox) {
            // Set initial state (default to true, or load from a saved preference if implemented)
            let initialShowTexture = true;
            if (window.userLayers && window.userLayers[groupId] && typeof window.userLayers[groupId].showTexture === 'boolean') {
                initialShowTexture = window.userLayers[groupId].showTexture;
            } else if (window.userLayers && window.userLayers[groupId]) {
                // If the group exists but showTexture is not defined, set it to the default
                window.userLayers[groupId].showTexture = initialShowTexture;
            }
            textureToggleCheckbox.checked = initialShowTexture;
            
            // Call initThumbnailViewer again with the initial texture state.
            // This assumes initThumbnailViewer will be modified to accept a showTexture parameter.
            if (typeof initThumbnailViewer === 'function') {
                 initThumbnailViewer('tileset-thumbnail-3d', groupId, initialShowTexture);
            }

            textureToggleCheckbox.onchange = function() {
                const show = this.checked;
                console.log(`[TEXTURE_TOGGLE] Group ${groupId} showTexture changed to: ${show}`);
                if (window.userLayers && window.userLayers[groupId]) {
                    window.userLayers[groupId].showTexture = show;
                }
                // Re-initialize or update the thumbnail viewer with the new texture state
                if (typeof initThumbnailViewer === 'function') {
                    initThumbnailViewer('tileset-thumbnail-3d', groupId, show);
                } else {
                    console.warn("initThumbnailViewer function not found for texture toggle.");
                }
            };
        } else {
            console.warn("Texture toggle checkbox ('details-texture-toggle') not found in details modal.");
        }
        ...
        // The following block for 'details-show-texture' is removed as it was redundant
        // and conflicted with the new 'details-texture-toggle' logic added earlier.
        ...
        if (window.scenePanel && window.scenePanel.style.display !== 'none' &&
            ...
            if (typeof loadGltfDataIntoSceneViewer === 'function') {
                console.log(`[AutoLoadGLTF_ModalOpen] Attempting to auto-load GLTF for ${currentEditingGroupId}. GLTF data present: ${!!window.latestGeneratedGltf.data}, Name: ${window.latestGeneratedGltf.name}`);
                loadGltfDataIntoSceneViewer(window.latestGeneratedGltf.data, window.latestGeneratedGltf.name);
            } else {
                console.warn("[AutoLoadGLTF_ModalOpen] loadGltfDataIntoSceneViewer function not found.");
            }
        } else if (window.scenePanel && window.scenePanel.style.display !== 'none') {
            console.log(`[AutoLoadGLTF_ModalOpen] Scene panel visible. Condition for load not met: latestGLTF ID: ${window.latestGeneratedGltf?.id}, currentEditingGroupId: ${currentEditingGroupId}, data present: ${!!window.latestGeneratedGltf?.data}`);
        }
    }
...
async function generate3DAssetsFromTileset(tilesetGroupId, tilesetName, savedFeatures) {
    ...
    function getTileGeographicBounds(tileZ, tileX, tileY) {
        if (typeof ol === 'undefined' || !ol.tilegrid || !ol.proj) {
            console.warn("[GEO_HELPER] OpenLayers tilegrid or projection not available for getTileGeographicBounds.");
            return null;
        }
        try {
            // Assuming default Web Mercator projection for tiles if not specified otherwise
            const tileGrid = ol.tilegrid.createXYZ({ maxZoom: tileZ + 1 }); // Ensure grid covers the zoom level
            const tileExtent = tileGrid.getTileCoordExtent([tileZ, tileX, tileY]);
            
            const minLonLat = ol.proj.toLonLat([tileExtent[0], tileExtent[1]]);
            const maxLonLat = ol.proj.toLonLat([tileExtent[2], tileExtent[3]]);
            ...
            console.error(`[GEO_HELPER] Error in getTileGeographicBounds for ZXY ${tileZ}-${tileX}-${tileY}:`, e);
            return null;
        }
    }
    ...
    if (tileMeshesGroup.children.length > 0) {
        ...
        try {
            ...
            let geoData = {};
            if (tileInfoList && tileInfoList.length > 0 && typeof tileInfoList[0].zLevel === 'number') {
                const zLevel = tileInfoList[0].zLevel;
                const boundsMinTile = getTileGeographicBounds(zLevel, minX, maxY); // minY in screen coords is max Lat, maxY is min Lat
                const boundsMaxTile = getTileGeographicBounds(zLevel, maxX, minY); // minY tile (smallest Y index) corresponds to max Latitude (North)

                if (boundsMinTile && boundsMaxTile) {
                    // boundsMinTile is from (minX, maxY) -> (minLon_west, minLat_south)
                    // boundsMaxTile is from (maxX, minY) -> (maxLon_east, maxLat_north)
                    const overallGeoBounds = {
                        minLon: boundsMinTile.minLon,
                        minLat: boundsMinTile.minLat,
                        maxLon: boundsMaxTile.maxLon,
                        maxLat: boundsMaxTile.maxLat
                    };
                    geoData.geoBounds = overallGeoBounds;
                    geoData.geoCenter = {
                        lon: (overallGeoBounds.minLon + overallGeoBounds.maxLon) / 2,
                        lat: (overallGeoBounds.minLat + overallGeoBounds.maxLat) / 2
                    };
                    geoData.tileZoomLevel = zLevel;
                    geoData.tileBoundingBox = { minX, minY, maxX, maxY, z: zLevel };
                    console.log("[3D_ASSETS] Calculated geospatial data for GLTF:", geoData);
                } else {
                    console.warn("[3D_ASSETS] Could not calculate geographic bounds for GLTF.");
                }
            }

            window.latestGeneratedGltf = {
                id: tilesetGroupId,
                name: tilesetName,
                data: gltfData,
                source: demSourcePreference,
                ...geoData // Spread the geoData object here
            };
        } catch (error) {
...
// Helper function to get a color for heatmap rendering
function getHeatmapColor(value, maxValue, minOpacity = 0.05, maxOpacity = 0.6) {
    if (value <= 0) return 'rgba(0,0,0,0)'; // Transparent if no value
    const ratio = Math.min(Math.max(value / maxValue, 0), 1);
    
    // Simple gradient: blue (low density) -> green -> yellow -> red (high density)
    let r, g, b;
    if (ratio < 0.25) { // Blue to Cyan
        r = 0;
        g = Math.floor(255 * (ratio / 0.25));
        b = 255;
    } else if (ratio < 0.5) { // Cyan to Green
        r = 0;
        g = 255;
        b = Math.floor(255 * (1 - (ratio - 0.25) / 0.25));
    } else if (ratio < 0.75) { // Green to Yellow
        r = Math.floor(255 * ((ratio - 0.5) / 0.25));
        g = 255;
        b = 0;
    } else { // Yellow to Red
        r = 255;
        g = Math.floor(255 * (1 - (ratio - 0.75) / 0.25));
        b = 0;
    }
...
// It was inserted inside the new og.Globe options object.
                    ...
                    // sun: { stopped: true } // Removed to allow default dynamic sun for daytime view
                });
                ...
                window.ogSavedTilesetsLayer = new og.layer.CanvasTiles("Saved Tilesets", {
                    ...
                    drawTile: function (material, applyTexture) {
                        ...
                        if (tileZoom >= this.minZoom && tileZoom < TILE_SELECTION_ZOOM_CONST) {
                            // Heatmap logic for overview tiles (ZL < TILE_SELECTION_ZOOM_CONST)
                            // This will now draw a general heatmap based on all saved ZL21 tiles in the active layer.
                            console.log(`[OG_HEATMAP_GENERAL] Drawing overview tile ${ogTileId} for general heatmap.`);
                            const overviewTileGeoExtent = material.segment.getExtent();
                            // Adjust MAX_DENSITY_FOR_HEATMAP based on expected ZL21 tile density in an overview tile.
                            // E.g., a ZL15 tile can contain (2^(21-15))^2 = (2^6)^2 = 64^2 = 4096 ZL21 tiles.
                            // Let's set a more practical max for visual scaling, e.g., 50-100 ZL21 tiles for full red.
                            const MAX_DENSITY_FOR_HEATMAP = 50;

                            if (!(window.selectedLayerId && window.userLayers && window.userLayers[window.selectedLayerId])) {
                                console.log("[OG_HEATMAP_GENERAL] No selected layer or userLayers. Clearing tile.");
                                applyTexture(canvas); return;
                            }
                            const olLayerSource = window.userLayers[window.selectedLayerId].layer.getSource();
                            if (!olLayerSource || olLayerSource.getFeatures().length === 0) {
                                console.log("[OG_HEATMAP_GENERAL] No OL layer source or no features in source. Clearing tile.");
                                applyTexture(canvas); return;
                            }
                            
                            // Consider ALL ZL21 features in the current OpenLayers layer for the heatmap
                            const allZl21FeaturesInLayer = olLayerSource.getFeatures().filter(f => {
                                const tileIdStr = f.get('tileId');
                                if (!tileIdStr) return false;
                                const parts = tileIdStr.split('-').map(Number);
                                return parts.length === 3 && parts[0] === TILE_SELECTION_ZOOM_CONST;
                            });

                            if (allZl21FeaturesInLayer.length === 0) {
                                // console.log("[OG_HEATMAP_GENERAL] No ZL21 features in the current layer. Clearing tile.");
                                applyTexture(canvas); return;
                            }
                            // console.log(`[OG_HEATMAP_GENERAL] Found ${allZl21FeaturesInLayer.length} total ZL21 features in layer ${window.selectedLayerId}.`);
                            let intersectingZl21Count = 0;

                            allZl21FeaturesInLayer.forEach(feature => {
                                ...
                                if (intersectingZl21Count > 0) {
                                    // Always use heatmap color for general heatmap
                                    ctx.fillStyle = getHeatmapColor(intersectingZl21Count, MAX_DENSITY_FOR_HEATMAP);
                                    // console.log(`[OG_HEATMAP_GENERAL] Overview tile ${ogTileId}, density ${intersectingZl21Count}, color: ${ctx.fillStyle}`);
                                    ctx.fillRect(0, 0, size, size);
                                    drawn = true;
                                } else {
                                     // If no ZL21 tiles intersect this overview tile, ensure it's clear
                                     // applyTexture(canvas); // This might not be necessary if canvas is fresh or cleared by default
                                }
                            // Removed the 'else' block that was tied to state.highlightedGlobeGroupId.
                            // The general heatmap should draw regardless of a specific group highlight.
                            // If drawn is still false here, it means no ZL21 tiles from the active layer intersected this overview tile.
...
console.log("%cDEBUG: POST-INSTANTIATION of ogSavedTilesetsLayer & addLayer call", "color: yellow; font-weight: bold;");
    ...
    if (layersPanel) {
        ...
        // setupEditorPanelLogic(); // Call moved to a more consolidated place later in DOMContentLoaded
    ...
    if (document.getElementById('scene-panel-viewer-container') && typeof initMainSceneViewer === 'function') {
        initMainSceneViewer('scene-panel-viewer-container');
    }
    
    // Scene Panel Type Dropdown Logic
    const sceneTypeDropdown = document.getElementById('scene-type-dropdown');
    const scenePanelViewerContainer = document.getElementById('scene-panel-viewer-container');
    const sceneViewer3dtile = document.getElementById('scene-viewer-3dtile');
    // Add other viewer containers here if they exist (e.g., for USD, I3S)
    ...
    function updateScenePanelDisplay(selectedType) {
        const sceneViewerWrapper = document.querySelector('#scene-panel .panel-content .scene-viewer-container-wrapper');

        if (scenePanelViewerContainer) scenePanelViewerContainer.style.display = 'none';
        if (sceneViewer3dtile) sceneViewer3dtile.style.display = 'none'; // This is inside the wrapper
        if (sceneViewerWrapper) sceneViewerWrapper.style.display = 'none'; // Hide the wrapper too
        // if (sceneViewerUsd) sceneViewerUsd.style.display = 'none';
        // if (sceneViewerI3s) sceneViewerI3s.style.display = 'none';

        if (selectedType === 'threejs' && scenePanelViewerContainer) {
            scenePanelViewerContainer.style.display = 'block';
            if (typeof updateMainSceneViewerSize === 'function') {
                updateMainSceneViewerSize(); // Ensure size is correct
            }
            // If a model was previously loaded, re-trigger its display or ensure it's visible
            if (window.state.currentMainSceneGltfData && typeof loadGltfIntoMainScene === 'function') {
                // loadGltfIntoMainScene(window.state.currentMainSceneGltfData, window.state.currentMainSceneModelName, window.state.mainSceneShowTextures);
                // Avoid full reload if model is already in mainScene, just ensure camera/visibility.
                // For now, a full reload is safer if switching view types.
            }
        } else if (selectedType === '3dtile' && sceneViewer3dtile) {
            sceneViewer3dtile.style.display = 'block';
            if (sceneViewerWrapper) sceneViewerWrapper.style.display = 'block'; // Show wrapper for 3D Tiles
            // TODO: Initialize/load 3D Tiles content if necessary
        }
        // TODO: Add cases for 'usd', 'i3s'
        console.log(`[SCENE_PANEL_TYPE] Switched to ${selectedType}`);
    }

    if (sceneTypeDropdown) {
        sceneTypeDropdown.addEventListener('change', function() {
            updateScenePanelDisplay(this.value);
        });
        // Set initial display based on default selected value
        updateScenePanelDisplay(sceneTypeDropdown.value);
    } else {
        // Fallback: if dropdown doesn't exist, ensure threejs viewer is shown by default if its container exists
        if (scenePanelViewerContainer) {
            scenePanelViewerContainer.style.display = 'block';
             if (typeof updateMainSceneViewerSize === 'function') {
                updateMainSceneViewerSize();
            }
        }
    }

    // Tileset Details Modal Maximize Logic
    const maximizeDetailsModalBtn = document.getElementById('maximize-details-modal-btn');
    // tilesetDetailsModal is already a global-like variable from earlier in the script
    if (tilesetDetailsModal && maximizeDetailsModalBtn) {
        maximizeDetailsModalBtn.addEventListener('click', () => {
            const thumbnailSection = document.getElementById('thumbnail-viewer-section');
            if (!thumbnailSection) {
                console.error("Cannot maximize: #thumbnail-viewer-section not found.");
                return;
            }

            // Toggle a class on the thumbnail's section for direct styling (e.g., position:fixed)
            thumbnailSection.classList.toggle('maximized-thumbnail-view');
            
            // Toggle a class on the main modal to allow hiding/adjusting other modal content
            tilesetDetailsModal.classList.toggle('thumbnail-is-maximized-mode');

            const isMaximized = thumbnailSection.classList.contains('maximized-thumbnail-view');
            maximizeDetailsModalBtn.innerHTML = isMaximized ? '&#x29C7;' : '&#x26F6;'; // Change symbol
            maximizeDetailsModalBtn.title = isMaximized ? 'Restore View' : 'Maximize Thumbnail';
            
            if (typeof updateThumbnailViewerSize === 'function') {
                setTimeout(updateThumbnailViewerSize, 50);
            }
        });
    } else {
        if (!tilesetDetailsModal) console.warn("Maximize button logic: tilesetDetailsModal not found.");
        if (!maximizeDetailsModalBtn) console.warn("Maximize button logic: maximize-details-modal-btn not found.");
    }
    ...
    // Listener for "Import to Editor" button in Tileset Details Modal
    const importToEditorBtn = document.getElementById('import-to-editor-btn');
    if (importToEditorBtn) {
        importToEditorBtn.addEventListener('click', () => {
            const editorIframe = document.getElementById('editor-iframe');
            const editorTypeDropdown = document.getElementById('editor-type-dropdown');

            if (!editorIframe || !editorTypeDropdown) {
                alert("Editor panel elements not found.");
                return;
            }

            if (editorTypeDropdown.value !== 'wings3d') {
                alert("Please select Wings3D as the active editor to import GLTF.");
                // Optionally, switch to Wings3D and open editor panel:
                // editorTypeDropdown.value = 'wings3d';
                // editorTypeDropdown.dispatchEvent(new Event('change')); // Trigger updateEditorIframe
                // document.getElementById('editor-btn').click(); // Open editor panel
                return;
            }
            
            // Ensure the GLTF data is for the currently editing tileset group
            if (!currentEditingGroupId) {
                alert("No tileset is currently being edited in the details modal.");
                return;
            }
            if (!window.latestGeneratedGltf || window.latestGeneratedGltf.id !== currentEditingGroupId || !window.latestGeneratedGltf.data) {
                alert(`GLTF data for the current tileset (${currentEditingGroupId}) is not available or not yet generated. Please ensure assets are generated.`);
                // Optionally, trigger generation here if features are accessible
                // const features = ... get features for currentEditingGroupId ...
                // generate3DAssetsFromTileset(currentEditingGroupId, tilesetName, features).then(...)
                return;
            }

            if (!editorIframe.contentWindow) {
                alert("Editor iframe content window is not available. Ensure the editor is loaded.");
                return;
            }

            try {
                const gltfDataToImport = window.latestGeneratedGltf.data;
                const tilesetNameToImport = window.latestGeneratedGltf.name || 'Imported Tileset';
                
                // Wings3D might prefer the GLTF as an object, or might need it stringified.
                // For consistency with how GLTFLoader was sensitive, stringifying is safer for postMessage.
                // The Wings3D side will need to JSON.parse() this.
                const gltfString = JSON.stringify(gltfDataToImport);

                console.log(`[IMPORT_TO_WINGS] Posting 'importGLTF' message for tileset: ${tilesetNameToImport}`);
                editorIframe.contentWindow.postMessage({
                    type: 'importGLTF',
                    gltfString: gltfString,
                    tilesetName: tilesetNameToImport
                }, '*'); // Consider a more specific target origin if Wings3D is on a known domain

                // Optionally, ensure the editor panel is visible
                const editorPanel = document.getElementById('editor-panel');
                if (editorPanel && editorPanel.style.display === 'none') {
                     const editorButton = document.getElementById('editor-btn'); // Assuming 'editor-btn' toggles the panel
                     if(editorButton) editorButton.click();
                }
                alert(`Attempting to import ${tilesetNameToImport} into Wings3D. Check Wings3D for import confirmation or messages.`);

            } catch (error) {
                console.error("[IMPORT_TO_WINGS] Error preparing or posting GLTF data:", error);
                alert("An error occurred while trying to send the GLTF to Wings3D.");
            }
        });
    } else {
        console.warn("Button #import-to-editor-btn not found.");
    }

    // Listen for messages from the editor iframe (e.g., Wings3D requesting GLTF)
    window.addEventListener('message', (event) => {
        const editorIframe = document.getElementById('editor-iframe');
        // Ensure the message is from the editor iframe and we have its contentWindow
        if (!editorIframe || event.source !== editorIframe.contentWindow) {
            // console.log("Message received, but not from editorIframe or source mismatch.", event.source, editorIframe ? editorIframe.contentWindow : 'no editorIframe');
            return;
        }

        const messageData = event.data;
        if (messageData && messageData.type === 'requestLatestGLTF') {
            console.log("[PARENT_APP] Received 'requestLatestGLTF' from editor iframe.");
            if (window.latestGeneratedGltf && window.latestGeneratedGltf.data) {
                // Ensure it's for the contextually relevant tileset (e.g., currentEditingGroupId)
                // For now, just send the latest one available.
                // A more robust solution might involve the iframe sending the desired tilesetGroupId.
                if (!currentEditingGroupId || window.latestGeneratedGltf.id === currentEditingGroupId) {
                    try {
                        const gltfDataToSend = window.latestGeneratedGltf.data;
                        const tilesetName = window.latestGeneratedGltf.name || 'Current Tileset';
                        const gltfString = JSON.stringify(gltfDataToSend); // Stringify for postMessage

                        console.log(`[PARENT_APP] Sending 'latestGLTFResponse' for tileset: ${tilesetName} to editor.`);
                        editorIframe.contentWindow.postMessage({
                            type: 'latestGLTFResponse',
                            gltfString: gltfString,
                            tilesetName: tilesetName,
                            // Include geospatial data if Wings3D can use it
                            geoBounds: window.latestGeneratedGltf.geoBounds,
                            geoCenter: window.latestGeneratedGltf.geoCenter,
                            tileZoomLevel: window.latestGeneratedGltf.tileZoomLevel
                        }, '*'); // Consider targetOrigin if known
                    } catch (error) {
                        console.error("[PARENT_APP] Error stringifying or posting GLTF data in response to request:", error);
                        editorIframe.contentWindow.postMessage({ type: 'latestGLTFResponse', error: 'Error preparing GLTF data.' }, '*');
                    }
                } else {
                    console.warn(`[PARENT_APP] Latest GLTF (id: ${window.latestGeneratedGltf.id}) does not match currentEditingGroupId (${currentEditingGroupId}). Not sending.`);
                    editorIframe.contentWindow.postMessage({ type: 'latestGLTFResponse', error: 'No relevant GLTF data available for the current context.' }, '*');
                }
            } else {
                console.warn("[PARENT_APP] No latestGeneratedGltf data available to send.");
                editorIframe.contentWindow.postMessage({ type: 'latestGLTFResponse', error: 'No GLTF data available.' }, '*');
            }
        }
    });
    ...
    if (baseLayerSelectElement) { // Use renamed variable
        baseLayerSelectElement.addEventListener('change', function() {
            ...
            if (state.activeMapLibrary === 'openlayers') {
                if (state.olMap) {
                    const currentBaseLayer = state.olMap.getLayers().getArray().find(layer => layer.get('type') === 'base'); // Corrected: find by 'type'
                    if (currentBaseLayer) {
                        state.olMap.removeLayer(currentBaseLayer);
                    }
                    if (olBaseLayers[selectedValue]) {
                        const newSource = olBaseLayers[selectedValue].source();
                        const newBaseLayerOL = new ol.layer.Tile({ source: newSource, type: 'base' }); // Corrected: Tag with 'type'
