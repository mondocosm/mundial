...
detailsSaveBtn.addEventListener('click', () => {
    if (!currentEditingFeatureId || !selectedLayerId || !userLayers[selectedLayerId]) {
        console.error("Cannot save: No feature or layer context.");
        return;
    }
    ...
    if (!feature) {
        console.error("Cannot save: Feature not found.");
        return;
    }

    // Update feature properties from modal inputs
    const newName = detailsTilesetNameInput.value.trim();
    const newColor = detailsColorPicker.value;
    const newImageUrl = detailsTilesetImageUrlInput.value.trim();
    const newLinkUrl = detailsTilesetLinkInput.value.trim();
    const newTags = detailsTilesetTagsTextarea.value.trim();

    feature.set('tilesetName', newName || 'Unnamed Tileset'); // Ensure name isn't empty
    feature.set('color', newColor);
    feature.set('imageUrl', newImageUrl);
    feature.set('linkUrl', newLinkUrl);
    feature.set('tags', newTags);

    // Update style if visible
    ...
    // Update the name in the tileset list UI immediately
    populateTilesetList(selectedLayerId); // Easiest way to refresh the specific item's name/style

    console.log(`Saved changes for tileset ${currentEditingFeatureId}`);
    tilesetDetailsModal.style.display = 'none'; // Close modal after save
    currentEditingFeatureId = null; // Clear the editing state
});
...
const clickSelectHandler = function (evt) {
    ...
    map.forEachFeatureAtPixel(evt.pixel, function (feature, layer) {
        ...
        if (layer && layer.get('title') === selectedLayerId && feature.get('tilesetName')) {
            ...
            // Hide the old inline picker if it was somehow visible
