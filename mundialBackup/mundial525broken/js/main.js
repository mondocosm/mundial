...
function updateDetailsColorPreview(event) {
    const colorPreviewSpan = document.getElementById('details-color-preview');
    if (colorPreviewSpan && event && event.target) {
        colorPreviewSpan.style.backgroundColor = event.target.value;
    }
}
...
function setupEditorPanelLogic() {
    console.log("[EditorPanel] setupEditorPanelLogic called.");
    const editorTypeDropdown = document.getElementById('editor-type-dropdown');
    const editorIframe = document.getElementById('editor-iframe');

    if (!editorTypeDropdown || !editorIframe) {
        console.error("[EditorPanel] Critical: editor-type-dropdown or editor-iframe not found. Editor panel will NOT work.");
        return;
    }
    console.log("[EditorPanel] Dropdown and Iframe elements found.");
    ...
    function updateEditorIframe() {
        ...
        console.log(`[EditorPanel] updateEditorIframe called. Selected editor in dropdown: '${selectedEditor}'`);
        const targetUrl = editorUrlMap[selectedEditor];
        
        if (targetUrl) {
            console.log(`[EditorPanel] Target URL for '${selectedEditor}': ${targetUrl}`);
            editorIframe.src = targetUrl;
            console.log(`[EditorPanel] Editor iframe src set to: ${targetUrl}`);
        } else {
            console.warn(`[EditorPanel] No URL defined in editorUrlMap for editor type: '${selectedEditor}'. Setting iframe to blank.`);
            editorIframe.src = 'about:blank#error_unknown_editor_type';
        }
    }
// --- Global Keyboard Shortcut for Interaction Mode Toggle (Ctrl Key) ---
// This event listener seems misplaced inside updateEditorIframe. It should be at a higher scope, likely DOMContentLoaded.
// Moving it out or ensuring it's only added once. For now, just noting its odd placement.
// If it's intended to be here, it will re-add itself every time updateEditorIframe is called, which is not ideal.
document.addEventListener('keydown', function(event) {
    ...
    if (event.ctrlKey && !event.altKey && !event.shiftKey && !event.metaKey) {
            ...
            // Only preventDefault if we are actually handling the key.
            // if (interactionModeBtn) { // Check if we will handle it
...
function updateInteractionModeUI(mode) { // Now only updates cursor
    ...
    function toggleTileSelection(tileCoord) {
        ...
        if (existingFeature) {
            ...
            if (state.highlightedGlobeGroupId) {
                console.log(`toggleTileSelection: An individual tile was clicked while group ${state.highlightedGlobeGroupId} was highlighted. Clearing group highlight.`);
                state.highlightedGlobeGroupId = null;
...
state.highlightedGlobeGroupId = null; // Clear globe highlight
        if (window.ogSavedTilesetsLayer) {
        ...
        try {
            ...
            const clickSelectHandler = function (evt) {
                ...
                if (clickedSavedFeature && clickedGroupId) {
                    ...
                    state.highlightedGlobeGroupId = clickedGroupId; // Set for globe highlighting
...
function setupXRPanelLogic() {
    ...
    setTimeout(() => {
        ...
        if (xrEngineDropdown && xrIframe) {
            console.log("[XRPanelLogic] xrEngineDropdown and xrIframe found.");

            function updateXRIframeBasedOnSelection(selectedEngineValue, currentXrIframe) {
                let targetUrl = '';
                const engine = selectedEngineValue;
                console.log(`[XRPanelLogic] updateXRIframeBasedOnSelection: Engine selected/defaulted: '${engine}'`);

                switch (engine) {
                    case 'janusweb':
                        targetUrl = 'janus_og_wrapper.html'; // New wrapper that loads client + minimal_test_room
                        console.log(`[XRPanelLogic] JanusWeb target URL (using OG wrapper): ${targetUrl}`);
                        break;
                    ...
                    case 'babylonjs':
                        ...
                        if (currentXrIframe) { // Renamed xrIframe to currentXrIframe for clarity
                            const sendDataToBabylonIframe = () => {
                                if (currentXrIframe.contentWindow) {
                                    let initialView = null;
                                    if (window.olMap && window.olMap.getView()) {
                                        const view = window.olMap.getView();
                                        const center = ol.proj.toLonLat(view.getCenter());
                                        initialView = { center: center, zoom: view.getZoom() };
                                    }
                                    currentXrIframe.contentWindow.postMessage({
                                        type: 'initialSetup',
                                        tilesetData: window.currentTilesetForBabylon,
                                        mapView: initialView
                                    }, '*');
                                    console.log("Sent initialSetup data to babylon_maplibre.html iframe.");
                                } else { console.warn("Babylon iframe contentWindow not available."); }
                                currentXrIframe.removeEventListener('load', sendDataToBabylonIframe);
                            };
                            currentXrIframe.addEventListener('load', sendDataToBabylonIframe);
                        }
                        break;
                    case 'aframe':
                        ...
                        break;
                    case 'thirdroom':
                        targetUrl = 'packages/thirdroom/index.html';
                        console.log("Third Room selected. Placeholder integration.");
                        break;
                    case 'webxr':
                        targetUrl = 'packages/spatial-webxr/index.html';
                        break;
                    default:
                        console.error(`[XRPanelLogic] Unknown XR engine: '${engine}'`);
                        if(currentXrIframe) currentXrIframe.src='about:blank#unknown_xr_engine';
                        return;
                }
                if (targetUrl && currentXrIframe) {
                    console.log(`[XRPanelLogic] Setting XR iframe src to: ${targetUrl}`);
                    currentXrIframe.src = targetUrl;
                    currentXrIframe.dataset.currentEngine = engine;
                }
            }

            xrEngineDropdown.addEventListener('change', (event) => {
                updateXRIframeBasedOnSelection(event.target.value, xrIframe);
            });

            // Initial load based on current dropdown value
            if (xrEngineDropdown.value) {
                console.log(`[XRPanelLogic] Initializing XR iframe with dropdown default: '${xrEngineDropdown.value}'`);
                updateXRIframeBasedOnSelection(xrEngineDropdown.value, xrIframe);
            } else {
                console.warn("[XRPanelLogic] xrEngineDropdown has no initial value. XR Iframe not set on initial load.");
            }
...
window.sendGltfToJanusIfActive = function() {
    console.log("[GLTF_TO_JANUS] Attempting to send GLTF to JanusWeb if active.");
    const xrIframe = document.getElementById('xr-iframe');
    if (!xrIframe) {
        console.warn("[GLTF_TO_JANUS] xr-iframe not found.");
        return;
    }

    const currentEngine = xrIframe.dataset.currentEngine;
    const isJanusEngine = ['janusweb', 'janusweb-preload', 'janusweb-fixed'].includes(currentEngine);

    if (!isJanusEngine) {
        console.log(`[GLTF_TO_JANUS] JanusWeb is not the current XR engine (current: ${currentEngine}). GLTF not sent.`);
        return;
    }

    if (!window.latestGeneratedGltf || !window.latestGeneratedGltf.data) {
        console.warn("[GLTF_TO_JANUS] window.latestGeneratedGltf.data is not available. Cannot send to JanusWeb.");
        return;
    }

    if (!xrIframe.contentWindow) {
        console.error("[GLTF_TO_JANUS] xr-iframe.contentWindow is not available. Cannot post message to JanusWeb.");
        return;
    }

    try {
        console.log("[GLTF_TO_JANUS] GLTF data to be sent:", window.latestGeneratedGltf.data);
        const gltfString = JSON.stringify(window.latestGeneratedGltf.data);
        // console.log(`[GLTF_TO_JANUS] Stringified GLTF (first 200 chars): ${gltfString.substring(0,200)}`);
        
        console.log(`[GLTF_TO_JANUS] Posting 'loadGLTFAsRoom' to JanusWeb for tileset: ${window.latestGeneratedGltf.name}`);
        xrIframe.contentWindow.postMessage({
            type: 'loadGLTFAsRoom',
            gltfString: gltfString,
            tilesetName: window.latestGeneratedGltf.name || 'Unnamed Tileset'
            // TODO: Add geospatial data here: center, extent, orientation
        }, '*');
    } catch (error) {
        console.error("[GLTF_TO_JANUS] Error stringifying GLTF data or posting message:", error);
        console.error("[GLTF_TO_JANUS] GLTF data that caused error:", window.latestGeneratedGltf.data);
    }
}
...
console.log(`populateTilesetList: Generated ${Object.keys(groupedTilesets).length} groups. Group IDs:`, Object.keys(groupedTilesets));
        ...
        if (state.highlightedGlobeGroupId) {
            console.log(`populateTilesetList: Attempting to re-apply highlight for ${state.highlightedGlobeGroupId}`);
            highlightListItem(state.highlightedGlobeGroupId);
        } else {
        ...
        updateTilesetGroupMarkersOG(); // Update the globe markers
    ...
    function highlightListItem(groupId) {
        console.log(`[HIGHLIGHT_LIST_ITEM] Called with groupId: ${groupId}`);
        state.highlightedGlobeGroupId = groupId; // Set the state variable

        if (!tilesetListDiv) {
            console.log("[HIGHLIGHT_LIST_ITEM] tilesetListDiv not found, returning.");
            return;
        }
        const currentlyHighlighted = tilesetListDiv.querySelector('.highlighted');
        if (currentlyHighlighted) {
            console.log("[HIGHLIGHT_LIST_ITEM] Removing 'highlighted' class from previously selected item:", currentlyHighlighted);
            currentlyHighlighted.classList.remove('highlighted');
        }
        if (groupId) {
            const listItem = tilesetListDiv.querySelector(`.tileset-item[data-tileset-group-id="${groupId}"]`);
            if (listItem) {
                console.log("[HIGHLIGHT_LIST_ITEM] Found listItem, adding 'highlighted' class:", listItem);
                listItem.classList.add('highlighted');
            } else {
                console.log(`[HIGHLIGHT_LIST_ITEM] listItem NOT found for groupId: ${groupId}`);
            }
        } else {
            console.log("[HIGHLIGHT_LIST_ITEM] groupId is null/undefined, so no new item will be highlighted. state.highlightedGlobeGroupId also cleared.");
        }

        // Redraw the OpenGlobus saved tilesets layer to update heatmap
        if (window.ogSavedTilesetsLayer && typeof window.ogSavedTilesetsLayer.redraw === 'function') {
            console.log(`[HIGHLIGHT_LIST_ITEM] Redrawing ogSavedTilesetsLayer due to highlight change for groupId: ${groupId}`);
            window.ogSavedTilesetsLayer.redraw();
        } else {
            console.warn("[HIGHLIGHT_LIST_ITEM] window.ogSavedTilesetsLayer or its redraw function is not available.");
        }
    }
        ...
        tilesetListDiv.addEventListener('click', (event) => {
            ...
            if (target.tagName === 'SPAN' || target.innerHTML === '✏️' || target === itemDiv || itemDiv.contains(target) && !target.closest('button') && target.type !== 'checkbox') {
                ...
                if (layer) {
                    ...
                    clearMapSelectionAndDetails();
state.highlightedGlobeGroupId = groupId; // Set for globe highlighting
console.log(`UI List Click: Setting highlightedGlobeGroupId to: ${state.highlightedGlobeGroupId}`);
...
function initThumbnailViewer(containerId, tilesetGroupId) {
    ...
    const hemisphereLight = new THREE.HemisphereLight(0xffffff, 0x444444, 1.0); // Sky, Ground, Intensity
    hemisphereLight.position.set(0, 20, 0);
    thumbnailScene.add(hemisphereLight);

    const directionalLight = new THREE.DirectionalLight(0xffffff, 1.0); // Increased intensity
    directionalLight.position.set(5, 10, 7.5);
    thumbnailScene.add(directionalLight);

    const directionalLight2 = new THREE.DirectionalLight(0xffffff, 0.6); // Added another directional light from a different angle
    ...
    if (window.latestGeneratedGltf && window.latestGeneratedGltf.id === tilesetGroupId && window.latestGeneratedGltf.data) {
        ...
        loader.parse(JSON.stringify(window.latestGeneratedGltf.data), '', (gltf) => {
            ...
            // Attempt to send this GLTF to JanusWeb if it's the active XR engine
            if (typeof window.sendGltfToJanusIfActive === 'function') {
                window.sendGltfToJanusIfActive();
            } else {
                console.error("[THUMBNAIL_3D] sendGltfToJanusIfActive function not found on window object.");
            }
...
let mainSceneRenderer, mainScene, mainCamera, mainControls; // Keep references for main scene viewer

function initMainSceneViewer(containerId, gltfData, modelName = 'Loaded Model') {
    console.log(`[MAIN_SCENE_3D] Initializing for container ${containerId}, model: ${modelName}`);
    const container = document.getElementById(containerId);
    if (!container) {
        console.error(`[MAIN_SCENE_3D] Container element #${containerId} not found.`);
        return;
    }

    if (typeof THREE === 'undefined') {
        console.error("[MAIN_SCENE_3D] THREE.js is not loaded.");
        container.innerHTML = '<p style="color:red;padding:5px;">THREE.js missing</p>';
        return;
    }
    if (typeof THREE.GLTFLoader === 'undefined') {
        console.error("[MAIN_SCENE_3D] THREE.GLTFLoader is not loaded.");
        container.innerHTML = '<p style="color:red;padding:5px;">GLTFLoader missing</p>';
        return;
    }
    if (typeof THREE.OrbitControls === 'undefined') {
        console.warn("[MAIN_SCENE_3D] THREE.OrbitControls is not loaded. Navigation will be limited.");
    }
    ...
    if (mainSceneRenderer) {
        mainSceneRenderer.dispose();
    }

    // Scene
    mainScene = new THREE.Scene();
    mainScene.background = new THREE.Color(0x333333); // Darker background for main scene
    ...
    // Camera
    mainCamera = new THREE.PerspectiveCamera(60, container.clientWidth / container.clientHeight, 0.1, 2000);
    mainCamera.position.set(5, 5, 5); 
    mainCamera.lookAt(0, 0, 0);

    // Renderer
    mainSceneRenderer = new THREE.WebGLRenderer({ antialias: true });
    mainSceneRenderer.setPixelRatio(window.devicePixelRatio);
    // Deferring setSize to ensure container has dimensions
    requestAnimationFrame(() => {
        if (container && mainSceneRenderer) {
            const width = container.clientWidth;
            const height = container.clientHeight;
            if (width > 0 && height > 0) {
                mainSceneRenderer.setSize(width, height);
                mainCamera.aspect = width / height;
                mainCamera.updateProjectionMatrix();
                console.log(`[MAIN_SCENE_3D] Renderer size set to: ${width}x${height}`);
            } else {
                console.warn("[MAIN_SCENE_3D] Container for renderer still has zero dimensions even after rAF.", {width, height});
            }
        }
    });
    container.appendChild(mainSceneRenderer.domElement);

    // Lighting (similar to thumbnail, can be adjusted)
    ...
    // Controls
    if (typeof THREE.OrbitControls !== 'undefined') {
        mainControls = new THREE.OrbitControls(mainCamera, mainSceneRenderer.domElement);
    ...
    // Load GLTF model if data is provided
    if (gltfData) {
        console.log(`[MAIN_SCENE_3D] GLTF data provided for ${modelName}. Loading...`);
        const loader = new THREE.GLTFLoader();
        
        loader.parse(JSON.stringify(gltfData), '', (gltf) => {
            console.log(`[MAIN_SCENE_3D] GLTF loaded successfully into main scene: ${modelName}`, gltf.scene);
            ...
            let cameraZ = Math.abs(maxDim / 2 / Math.tan(fov / 2));
            cameraZ *= 1.8; // Zoom out a bit more for main scene
            
            mainCamera.position.copy(center);
            mainCamera.position.x += size.x * 0.6; 
            mainCamera.position.y += size.y * 0.6;
            mainCamera.position.z += cameraZ;
            mainCamera.lookAt(center);
            
            if(mainControls) mainControls.target.copy(center);
            ...
            console.error(`[MAIN_SCENE_3D] Error parsing GLTF for main scene (${modelName}):`, error);
            container.innerHTML = '<p style="color:red;padding:5px;">Error loading 3D model.</p>';
        });
    } else {
        console.log(`[MAIN_SCENE_3D] No GLTF data provided for ${modelName}. Scene initialized empty.`);
        container.innerHTML = '<p style="color:grey;padding:5px;text-align:center;">Select a tileset to view 3D model.</p>';
    }

    // Animation loop
    function animateMainScene() {
        if (!mainSceneRenderer) return; 
        requestAnimationFrame(animateMainScene);
        if(mainControls) mainControls.update();
        mainSceneRenderer.render(mainScene, mainCamera);
    }
    animateMainScene();

    // Handle resize
    // TODO: Consider a more robust resize observer if panel resizes dynamically often
    window.addEventListener('resize', () => {
        if (container && mainSceneRenderer && mainCamera) {
            const width = container.clientWidth;
            const height = container.clientHeight;
            if (width > 0 && height > 0) {
                 mainCamera.aspect = width / height;
    ...
    console.log(`[MAIN_SCENE_3D] Initialization complete for ${containerId}.`);
}

// This function will be called to load/update the main scene viewer
function loadGltfDataIntoSceneViewer(gltfDataObject, tilesetName) {
    console.log(`[SCENE_LOADER] Attempting to load GLTF for tileset: '${tilesetName}'. Has gltfDataObject: ${!!gltfDataObject}`);

    if (!gltfDataObject) {
        console.warn(`[SCENE_LOADER] No GLTF data provided for '${tilesetName}'. Cannot load into scene viewer.`);
        // Optionally display a message in the scene panel
        const sceneContainer = document.getElementById('main-scene-viewer-container');
        if (sceneContainer) {
            sceneContainer.innerHTML = `<p style="color:orange;padding:10px;text-align:center;">No 3D model data available for ${tilesetName}.</p>`;
        }
        return;
    }

    const scenePanelContainerId = 'main-scene-viewer-container';
    const scenePanel = document.getElementById('scene-panel');
    const sceneContainer = document.getElementById(scenePanelContainerId);

    if (!scenePanel) {
        console.error("[SCENE_LOADER] Crucial HTML Error: #scene-panel not found. Cannot display scene.");
        return;
    }
    if (!sceneContainer) {
        console.error(`[SCENE_LOADER] Crucial HTML Error: #${scenePanelContainerId} not found inside #scene-panel. Cannot display scene.`);
        return;
    }
    
    console.log(`[SCENE_LOADER] Scene panel found (display: ${scenePanel.style.display}, classes: ${scenePanel.className}). Container found.`);

    // Ensure panel is visible
    let panelWasHidden = false;
    if (scenePanel.style.display === 'none' || scenePanel.classList.contains('minimized')) {
        console.log("[SCENE_LOADER] Scene panel was hidden or minimized. Making it visible.");
        scenePanel.style.display = 'block';
        scenePanel.classList.remove('minimized');
        panelWasHidden = true;
    } else {
        console.log("[SCENE_LOADER] Scene panel already visible.");
    }
    
    // Call initMainSceneViewer. If panel was just made visible, use a small timeout for rendering.
    if (panelWasHidden) {
        setTimeout(() => {
            console.log(`[SCENE_LOADER] Calling initMainSceneViewer for '${tilesetName}' (after panel display update).`);
            initMainSceneViewer(scenePanelContainerId, gltfDataObject, tilesetName);
        }, 50); // Delay to allow DOM to update display property
    } else {
        console.log(`[SCENE_LOADER] Calling initMainSceneViewer for '${tilesetName}' (panel was already visible).`);
        initMainSceneViewer(scenePanelContainerId, gltfDataObject, tilesetName);
    }
}
    function openTilesetDetailsModal(feature) {
        ...
        if (colorPreviewSpan) {
            ...
            console.warn("[DETAILS_MODAL] Color preview span ('details-color-preview') not found.");
        }
        // Add event listener to update preview swatch when color picker changes
        detailsColorPicker.removeEventListener('input', updateDetailsColorPreview); // Remove previous if any
        detailsColorPicker.addEventListener('input', updateDetailsColorPreview);
        ...
        console.log(`[TILE_COUNT_DEBUG] Opening details for groupId: ${groupId}`);
        const layer = window.userLayers[window.selectedLayerId]?.layer;
        let tileCount = 0; let coordsStr = 'N/A';

        if (layer && layer.getSource()) {
            console.log(`[TILE_COUNT_DEBUG] Found layer and source for layerId: ${window.selectedLayerId}`);
            const allFeatures = layer.getSource().getFeatures();
            const groupFeatures = allFeatures.filter(f => f.get('tilesetGroupId') === groupId);
            tileCount = groupFeatures.length;
            console.log(`[TILE_COUNT_DEBUG] Found ${allFeatures.length} total features in layer, ${groupFeatures.length} features for groupId ${groupId}. Tile count set to: ${tileCount}`);
            ...
            if (tileCount > 0 && selectionTileGrid) {
                 ...
                 if (firstTileId) {
                     ...
                     // TODO: Implement reverse geocoding here to update detailsLocationInfoSpan with actual place name.
                     // For now, it remains "Loading..." or can be set to coordsStr if no reverse geocoding is planned.
                     if (detailsLocationInfoSpan) {
                        detailsLocationInfoSpan.textContent = 'Approx. ' + coordsStr; // Show coordinates for now
                        // detailsLocationInfoSpan.textContent = 'Loading address...'; // Or keep loading if reverse geocoding will be added
                     }
                 }
            }
        } else {
            console.warn(`[TILE_COUNT_DEBUG] Layer or layer source not found for layerId: ${window.selectedLayerId}. Tile count remains ${tileCount}.`);
        }

        detailsTilesetCoordsSpan.textContent = coordsStr;
        const detailsTileCountSpan = document.getElementById('details-tileset-tile-count'); // Corrected ID
        if (detailsTileCountSpan) {
            console.log(`[TILE_COUNT_DEBUG] Found 'details-tileset-tile-count'. Setting textContent to: ${tileCount}`);
            detailsTileCountSpan.textContent = tileCount;
        } else {
            console.warn("[TILE_COUNT_DEBUG] Element with ID 'details-tileset-tile-count' not found for updating tile count.");
        }
        // if (detailsLocationInfoSpan) detailsLocationInfoSpan.textContent = 'Loading...'; // This was overwriting the coordsStr
        ...
        const viewInSceneBtn = document.getElementById('view-tileset-in-cesium-btn'); // Changed ID
        if (viewInSceneBtn) {
            // Remove old listener to prevent duplicates
            const newViewInSceneBtn = viewInSceneBtn.cloneNode(true);
            viewInSceneBtn.parentNode.replaceChild(newViewInSceneBtn, viewInSceneBtn);

            newViewInSceneBtn.addEventListener('click', () => {
                console.log("[DETAILS_MODAL] 'View in Scene' (view-tileset-in-cesium-btn) button clicked.");
                if (typeof loadGltfDataIntoSceneViewer === 'function') {
                    if (window.latestGeneratedGltf && window.latestGeneratedGltf.id === currentEditingGroupId && window.latestGeneratedGltf.data) {
                        // Ensure scene type dropdown is set to threejs before loading
                        const sceneTypeDropdown = document.getElementById('scene-type-dropdown');
                        if (sceneTypeDropdown) {
                            sceneTypeDropdown.value = 'threejs';
                             // Dispatch a change event if needed, though loadGltfDataIntoSceneViewer directly uses Three.js
                            console.log("[DETAILS_MODAL] Set scene-type-dropdown to 'threejs' before loading.");
                        }
                        loadGltfDataIntoSceneViewer(window.latestGeneratedGltf.data, window.latestGeneratedGltf.name || name);
                    } else {
                        console.warn(`[DETAILS_MODAL] 'View in Scene': GLTF data for group ${currentEditingGroupId} not readily available in window.latestGeneratedGltf or ID mismatch.`);
                        alert("GLTF data for this tileset is not currently loaded for the main scene viewer. It might load if thumbnail generation was successful for this specific tileset.");
                    }
                } else {
                    console.error("[DETAILS_MODAL] loadGltfDataIntoSceneViewer function not found.");
                }
            });
        } else {
            console.warn("[DETAILS_MODAL] 'View in Scene' button (view-tileset-in-cesium-btn) not found.");
        }

        tilesetDetailsModal.style.setProperty('display', 'block', 'important');

        // Attempt to auto-load GLTF into Scene panel if it's visible
        console.log(`[AutoLoadGLTF_ModalOpen_DEBUG] Checking conditions for auto-load. ScenePanel visible: ${window.scenePanel && window.scenePanel.style.display !== 'none'}, latestGLTF ID: ${window.latestGeneratedGltf?.id}, currentEditingGroupId: ${currentEditingGroupId}, latestGLTF data present: ${!!window.latestGeneratedGltf?.data}`);
        if (window.scenePanel && window.scenePanel.style.display !== 'none' &&
            window.latestGeneratedGltf && window.latestGeneratedGltf.data &&
            window.latestGeneratedGltf.id === currentEditingGroupId) {
            if (typeof loadGltfDataIntoSceneViewer === 'function') {
                console.log(`[AutoLoadGLTF_ModalOpen] Conditions MET. Attempting to auto-load GLTF for ${currentEditingGroupId}. GLTF data present: ${!!window.latestGeneratedGltf.data}, Name: ${window.latestGeneratedGltf.name}`);
                loadGltfDataIntoSceneViewer(window.latestGeneratedGltf.data, window.latestGeneratedGltf.name);
            } else {
                console.warn("[AutoLoadGLTF_ModalOpen] loadGltfDataIntoSceneViewer function not found, cannot auto-load.");
            }
        } else {
            // This 'else if' was potentially too broad, let's refine the logging for why it didn't meet conditions.
            if (!(window.scenePanel && window.scenePanel.style.display !== 'none')) {
                console.log(`[AutoLoadGLTF_ModalOpen] Auto-load condition NOT MET: Scene panel not visible or not found.`);
            } else if (!window.latestGeneratedGltf) {
                console.log(`[AutoLoadGLTF_ModalOpen] Auto-load condition NOT MET: window.latestGeneratedGltf is not defined.`);
            } else if (!window.latestGeneratedGltf.data) {
                console.log(`[AutoLoadGLTF_ModalOpen] Auto-load condition NOT MET: window.latestGeneratedGltf.data is not present for ID ${window.latestGeneratedGltf.id}.`);
            } else if (window.latestGeneratedGltf.id !== currentEditingGroupId) {
                console.log(`[AutoLoadGLTF_ModalOpen] Auto-load condition NOT MET: ID mismatch. latestGLTF.id ('${window.latestGeneratedGltf.id}') !== currentEditingGroupId ('${currentEditingGroupId}').`);
            } else {
                 console.log(`[AutoLoadGLTF_ModalOpen] Auto-load conditions NOT MET for other reasons. ScenePanel visible: ${window.scenePanel && window.scenePanel.style.display !== 'none'}`);
            }
        }
    }

function handleShowTextureToggle(groupId, showTexture) {
    console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Toggling texture for group ${groupId} to ${showTexture}`);
    
    // Update OpenLayers feature property and style
    if (applyGroupPropertyChange('showTexture', showTexture, false)) { // false to trigger style update
        console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Applied 'showTexture: ${showTexture}' to group ${groupId} features and updated OL styles.`);
    }

    // Update 3D thumbnail viewer
    if (thumbnailScene && thumbnailScene.children) {
        thumbnailScene.traverse((object) => {
            if (object.isMesh && object.material) {
                const materials = Array.isArray(object.material) ? object.material : [object.material];
                materials.forEach(material => {
                    if (showTexture) {
                        // Restore texture if it was stored
                        if (material.userData.originalMap) {
                            material.map = material.userData.originalMap;
                            console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Restored texture for material: ${material.name || material.uuid}`);
                        } else if (material.map === null) {
                             // If originalMap is not set but map is null, it implies texture was off.
                             // This case might need re-fetching or a default texture if one was intended.
                             // For now, we assume if originalMap is missing, it was never textured or texture is already on.
                             console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Material ${material.name || material.uuid} has no originalMap stored, or texture already on.`);
                         }
                         // Ensure material is not transparent if texture is shown
                         material.transparent = material.userData.originalTransparent !== undefined ? material.userData.originalTransparent : false;
                         material.opacity = material.userData.originalOpacity !== undefined ? material.userData.originalOpacity : 1.0;
                         material.wireframe = false; // Turn off wireframe when texture is shown
                        if (material.userData.originalColor) { // Restore original color if it was changed for wireframe
                            material.color.copy(material.userData.originalColor);
                            delete material.userData.originalColor; // Clean up
                            console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Restored original color for material: ${material.name || material.uuid}`);
                        }
                         console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Set wireframe to false for material: ${material.name || material.uuid}`);
 
                     } else {
                         // Store original texture and remove it
                         if (material.map) {
                             material.userData.originalMap = material.map;
                             material.map = null;
                             console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Removed texture and stored original for material: ${material.name || material.uuid}`);
                         }
                        
                        // Store original color if not already stored, then set to dark for wireframe
                        if (!material.userData.originalColor) {
                            material.userData.originalColor = material.color.clone();
                        }
                        material.color.set(0x222222); // Set wireframe color to dark grey

                        material.wireframe = true; // Turn on wireframe when texture is hidden
                        console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Set wireframe to true and color to dark grey for material: ${material.name || material.uuid}`);
                         
                         // Store original transparency settings
                        ...
                        // Make material opaque if needed, or handle as desired
        ...
        console.log(`[THUMBNAIL_TEXTURE_TOGGLE] Traversed thumbnail scene to update textures.`);
    } else {
        console.warn("[THUMBNAIL_TEXTURE_TOGGLE] thumbnailScene not found or has no children. Cannot toggle 3D thumbnail texture.");
    }
    // Note: The thumbnailRenderer.render call is in its own animateThumbnail loop, so changes should pick up.
                    ...
                    for (let i_vert = 0; i_vert < VERTS_PER_TILE_EDGE; i_vert++) { // X-vertex index (columns)
                        ...
                        if (!isFinite(height)) {
                            ...
                            height = 0;
                             ...
                             height = height > MAX_EXPECTED_HEIGHT ? MAX_EXPECTED_HEIGHT : MIN_EXPECTED_HEIGHT; // Clamp to bounds
                             console.warn(`[3D_ASSETS_QUAD_SPIKE] Tile ${tileInfo.originalIndex} (X:${tileInfo.x},Y:${tileInfo.y}) vert(${i_vert},${j_vert}): Extreme height ${originalHeight.toFixed(2)} from RGB(${rVal},${gVal},${bVal}) at composite(${cImgX},${cImgY}). Clamped to ${height.toFixed(2)}.`);
                        }
                    ...
                    generate3DAssetsFromTileset(tilesetGroupId, tilesetName, featuresToAdd)
                        .then(assetResults => { // assetResults will be { pointCloud: ..., gltf: ... }
                            if (assetResults && assetResults.gltf && assetResults.gltf.data) {
                                ...
                                // Auto-load into Scene panel if visible
                                if (window.scenePanel && window.scenePanel.style.display !== 'none') {
                                    if (typeof loadGltfDataIntoSceneViewer === 'function') {
                                        console.log(`[AutoLoadGLTF_Save] Attempting to auto-load newly generated GLTF for ${tilesetGroupId} (Name: ${tilesetName}). Data present: ${!!assetResults.gltf.data}`);
                                        loadGltfDataIntoSceneViewer(assetResults.gltf.data, tilesetName);
                                    } else {
                                        console.warn("[AutoLoadGLTF_Save] loadGltfDataIntoSceneViewer function not found after save.");
                                    }
                                } else {
                                    console.log(`[AutoLoadGLTF_Save] Scene panel not visible or no GLTF data after save. Scene visible: ${window.scenePanel?.style.display !== 'none'}, GLTF data: ${!!assetResults.gltf?.data}`);
                                }
                                // TODO: Further action like updating UI list thumbnail if needed.
    
                                // Update the details modal if it's open for this newly saved/updated tileset
                                // This ensures if the user was editing it, the modal reflects any new info (though not much changes on save itself)
                                if (tilesetDetailsModal.style.display === 'block' && currentEditingGroupId === tilesetGroupId) {
                                    // We need the main feature representing the group to refresh the modal
                                    const mainFeatureForModal = featuresToAdd.find(f => f.get('tilesetGroupId') === tilesetGroupId); // Use featuresToAdd as targetSource might not be updated yet
                                    if (mainFeatureForModal && typeof openTilesetDetailsModal === 'function') {
                                        console.log(`[SAVE_HANDLER] Refreshing details modal for ${tilesetGroupId}.`);
                                        openTilesetDetailsModal(mainFeatureForModal);
                                    }
                                }
...
console.log("%cSAVE HANDLER: populateTilesetList has been called from save handler.", "color: green; font-weight: bold;");
    ...
    // let tileCubeLayer = null; // Layer for the ZL21 tile *indicator* cube on OpenGlobus - OBSOLETE
    // let selectedTileCubeEntity = null; // The currently displayed indicator cube entity on OpenGlobus - OBSOLETE
    console.log("DEBUG: OpenGlobus related variables declared.");

// Helper function to get heatmap color
// value: current density, maxValue: max expected density for full intensity
function getHeatmapColor(value, maxValue) {
    const ratio = Math.min(Math.max(value / maxValue, 0), 1); // Normalize value to 0-1 range
    ...
    const alpha = 0.6; // Fixed opacity for heatmap tiles
    return `rgba(${r},${g},${b},${alpha})`;
}

let ogTilesetGroupMarkersLayer = null; // Declare globally for the new marker layer

function updateTilesetGroupMarkersOG() {
    if (!window.globus || !ogTilesetGroupMarkersLayer || !og.Entity || !og.LonLat || !og.utils?.dotCanvas) {
        console.warn("[MARKERS_OG] OpenGlobus, marker layer, or required OG components not ready for updating markers.");
        return;
    }
    if (!window.selectedLayerId || !window.userLayers || !window.userLayers[window.selectedLayerId] || !window.userLayers[window.selectedLayerId].layer) {
        console.log("[MARKERS_OG] No selected layer or layer data. Clearing markers.");
        ogTilesetGroupMarkersLayer.clear();
        return;
    }
    ...
    if (!olLayerSource || olLayerSource.getFeatures().length === 0) {
        console.log("[MARKERS_OG] No features in selected layer source. Clearing markers.");
        ogTilesetGroupMarkersLayer.clear();
        return;
    }

    const allFeatures = olLayerSource.getFeatures();
    const groups = {};
    ...
    allFeatures.forEach(feature => {
        ...
        if (groupId && tileId) {
            if (!groups[groupId]) {
                groups[groupId] = {
                    features: [],
                    name: feature.get('tilesetName') || groupId,
                    tileIds: new Set()
                };
            }
            if (!groups[groupId].tileIds.has(tileId)) { // Ensure unique tiles per group for centroid calculation
                 groups[groupId].features.push(feature);
                 groups[groupId].tileIds.add(tileId);
            }
        }
    });

    ogTilesetGroupMarkersLayer.clear();
    console.log(`[MARKERS_OG] Updating markers for ${Object.keys(groups).length} groups.`);

    for (const groupId in groups) {
        const group = groups[groupId];
        if (group.features.length > 0 && selectionTileGrid && ol && ol.extent && ol.proj) {
            const groupExtentEPSG3857 = ol.extent.createEmpty();
            ...
            if (!ol.extent.isEmpty(groupExtentEPSG3857)) {
                ...
                const markerEntity = new og.Entity({
                    lonlat: new og.LonLat(centerLon, centerLat),
                    billboard: {
                        src: og.utils.dotCanvas(new og.Vec4(255, 255, 0, 0.9)), // Bright yellow dot
                        size: [12, 12], // Increased size for better visibility. Non-scaling.
                        offset: [0, 6], // Adjusted offset for larger size
                        rotation: 0,
                        alignedAxis: [0,0,1], // Keep upright
                        color: new og.Vec4(1,1,1,1) // Ensure full brightness if src has alpha
                    },
                    properties: {
                        name: group.name,
                        tilesetGroupId: groupId,
                        type: 'tileset_group_marker'
                    }
                });
                ogTilesetGroupMarkersLayer.add(markerEntity);
            }
        }
    }
    console.log(`[MARKERS_OG] Added ${ogTilesetGroupMarkersLayer.getEntities().length} marker entities.`);
}
...
// Removing current definition of updateCesiumZL21Grid to redefine it earlier.
                ...
                console.log(`DEBUG_OG_CANVAS_SIZE: Before new og.Globe() - globusContainerElement clientWidth: ${globusContainerElement.clientWidth}, clientHeight: ${globusContainerElement.clientHeight}`);
...
// It was inserted inside the new og.Globe options object.
                    ...
                    atmosphereEnabled: true, // Good for daytime
                    lon: -74.0445,
                    lat: 40.6892,
                    alt: 3000,
                    ...
                    // sun: { stopped: true } // Removed to allow default dynamic sun for daytime
                });
...
if (window.globus.planet && og.control && og.control.TimelineControl) {
            ...
            if (window.globus) {
                ...
                // Attempt an immediate resize if renderer is available
                if (window.globus.planet && window.globus.planet.renderer && typeof window.globus.planet.renderer.resize === 'function') {
                    console.log("DEBUG_OG_INIT: Attempting immediate renderer resize.");
                    window.globus.planet.renderer.resize();
                } else {
                    console.warn("DEBUG_OG_INIT: Renderer or resize function not immediately available after Globe creation for immediate resize.");
                }

                // Ensure rendering loop is started
                if (window.globus.planet && window.globus.planet.renderer && typeof window.globus.planet.renderer.start === 'function') {
                    console.log("DEBUG_OG_INIT: Starting OpenGlobus renderer.");
                    window.globus.planet.renderer.start();
                } else {
                    console.warn("DEBUG_OG_INIT: Renderer or start function not available to start rendering loop.");
                }
            ...
            if (window.globus.planet) {
                ...
                // Check initial terrain (should be GlobusRgbTerrain from constructor)
                if (window.globus.planet.terrain && window.globus.planet.terrain.name === 'GlobusEarthRgb') {
                    console.log("%cTERRAIN CHECKPOINT 1 (Constructor): Planet has 'GlobusRgbTerrain'.", "color: #FF8C00; font-size: 1.1em; font-weight: bold;", window.globus.planet.terrain);
                    console.log("  URL:", window.globus.planet.terrain.url);
                    window.globus.planet.terrain.enabled = true; // Ensure enabled
                    console.log("  Enabled state:", window.globus.planet.terrain.enabled);
                    ...
                    console.warn("WARNING: Globe constructor did NOT set GlobusRgbTerrain as expected. Current terrain:", window.globus.planet.terrain);
                }
...
console.log("%cDEBUG: PRE-INSTANTIATION of ogSavedTilesetsLayer", "color: yellow; font-weight: bold;");
                window.ogSavedTilesetsLayer = new og.layer.CanvasTiles("Saved Tilesets", {
                    ...
                    minZoom: 5, // Show from zoom level 5
                    ...
                    drawTile: function (material, applyTexture) {
                        ...
                        if (tileZoom >= this.minZoom && tileZoom < TILE_SELECTION_ZOOM_CONST) {
                            // Heatmap logic for overview tiles (ZL15-ZL20)
                            // Only draw heatmap if a specific tileset group is highlighted on the globe.
                            if (state.highlightedGlobeGroupId) {
                                console.log(`[OG_HEATMAP_DEBUG] Drawing overview tile ${ogTileId}. Highlighted group: ${state.highlightedGlobeGroupId}`);
                                const overviewTileGeoExtent = material.segment.getExtent();
                                const MAX_DENSITY_FOR_HEATMAP = 20; // Max ZL21 tiles for full red color

                                if (!(window.selectedLayerId && window.userLayers && window.userLayers[window.selectedLayerId])) {
                                    console.log("[OG_HEATMAP_DEBUG] No selected layer or userLayers. Clearing tile.");
                                    applyTexture(canvas); return;
                                }
                                const olLayerSource = window.userLayers[window.selectedLayerId].layer.getSource();
                                if (!olLayerSource || olLayerSource.getFeatures().length === 0) {
                                    console.log("[OG_HEATMAP_DEBUG] No OL layer source or no features in source. Clearing tile.");
                                    applyTexture(canvas); return;
                                }
                                
                                // Filter features to only those belonging to the highlighted group
                                const zl21FeaturesInGroup = olLayerSource.getFeatures().filter(f => f.get('tilesetGroupId') === state.highlightedGlobeGroupId);
                                console.log(`[OG_HEATMAP_DEBUG] Found ${zl21FeaturesInGroup.length} features in highlighted group ${state.highlightedGlobeGroupId}.`);
                                let intersectingZl21Count = 0;

                                zl21FeaturesInGroup.forEach(feature => {
                                    ...
                                    if (og && og.mercator && typeof og.mercator.getTileExtent === 'function') {
                                        ...
                                        if (overviewTileGeoExtent && typeof overviewTileGeoExtent.intersects === 'function' && zl21FeatureGeoExtent && overviewTileGeoExtent.intersects(zl21FeatureGeoExtent)) {
                                            intersectingZl21Count++;
                                        }
                                    } else {
                                        console.warn("og.mercator.getTileExtent is not available for heatmap calculation.");
                                    }
                                });

                                if (intersectingZl21Count > 0) {
                                    // If count is low (e.g., 1-2), use a distinct color from the group.
                                    // Otherwise, use the heatmap gradient.
                                    const DENSITY_THRESHOLD_FOR_GROUP_COLOR = 3; // If count < this, use group color.
                                    let groupDataForColor = null;
                                    if (zl21FeaturesInGroup.length > 0) { // Should always be true if intersectingZl21Count > 0
                                        groupDataForColor = { // Reconstruct enough group data for color
                                            color: zl21FeaturesInGroup[0].get('color') || '#008080', // Default if no color
                                            fillOpacity: zl21FeaturesInGroup[0].get('fillOpacity') === undefined ? 0.4 : zl21FeaturesInGroup[0].get('fillOpacity') // Slightly less opaque for overview
                                        };
                                    }

                                    if (intersectingZl21Count < DENSITY_THRESHOLD_FOR_GROUP_COLOR && groupDataForColor) {
                                        let tileColorToDraw = groupDataForColor.color;
                                        let featureFillOpacity = groupDataForColor.fillOpacity;
                                        if (tileColorToDraw.startsWith('#')) {
                                            ...
                                            ctx.fillStyle = `rgba(${r},${g},${b},${featureFillOpacity})`;
                                        } else if (tileColorToDraw.startsWith('rgba')) {
                                            ctx.fillStyle = tileColorToDraw.replace(/[\d\.]+\)$/, `${featureFillOpacity})`);
                                        } else { // Fallback if color format is unexpected
                                            ctx.fillStyle = getHeatmapColor(intersectingZl21Count, MAX_DENSITY_FOR_HEATMAP);
                                        }
                                        console.log(`[OG_HEATMAP_DEBUG] Highlighted group ${state.highlightedGlobeGroupId}, low density (${intersectingZl21Count}), using group color: ${ctx.fillStyle}`);
                                    } else {
                                        ctx.fillStyle = getHeatmapColor(intersectingZl21Count, MAX_DENSITY_FOR_HEATMAP);
                                        console.log(`[OG_HEATMAP_DEBUG] Highlighted group ${state.highlightedGlobeGroupId}, high density (${intersectingZl21Count}), using heatmap color: ${ctx.fillStyle}`);
                                    }
                                    ctx.fillRect(0, 0, size, size);
                                    drawn = true;
                                }
                            } else {
                                // No specific group highlighted, so don't draw any area-based heatmap on overview tiles.
                                // The "point glow" markers are handled by ogTilesetGroupMarkersLayer.
                                // console.log(`[OG_HEATMAP_DEBUG] Overview tile ${ogTileId}. No specific group highlighted. Clearing tile / making transparent.`);
                                applyTexture(canvas); // Ensures the tile is cleared or transparent
                                // 'drawn' remains false if nothing else is on this tile from this layer's logic
                            }
                            ...
                            // 2. Else, check for saved tilesets in the active layer (this will now also catch tiles that were tempSelected but are now saved)
                                ...
                                // console.log(`OG DrawTile: Tile ${ogTileId}, GroupID: ${featureGroupId}, HighlightedGroupID: ${state.highlightedGlobeGroupId}`); // Verbose
                                if (featureGroupId && featureGroupId === state.highlightedGlobeGroupId) {
                                    ...
                                    // console.log(`OG DrawTile: Tile ${ogTileId} (group ${featureGroupId}) NOT highlighted. Current highlight: ${state.highlightedGlobeGroupId}`);
                                }
                            }
                        }
...
console.log("%cDEBUG: POST-INSTANTIATION of ogSavedTilesetsLayer & addLayer call", "color: yellow; font-weight: bold;");
                ...
                // Initialize and add the new vector layer for group markers
                ogTilesetGroupMarkersLayer = new og.layer.Vector("Tileset Group Markers", {
                    pickingEnabled: false, // No need to pick these markers for now
                    minZoom: 0, // Visible at all zoom levels
                    maxZoom: 22,
                    zIndex: 100 // Ensure it's drawn on top of heatmaps/other tiles
                });
                window.globus.planet.addLayer(ogTilesetGroupMarkersLayer);
                console.log("DEBUG: ogTilesetGroupMarkersLayer added to planet.");
                    ...
                    drawTile: function (material, applyTexture) {
                        ...
                        if (currentTileZoom >= this.minZoom && currentTileZoom <= this.maxZoom) {
                            ...
                            if (zoomDiff >= 0) { // Only draw grid if current zoom is <= targetGridZoom
                                ...
                                // Only draw detailed subgrid lines if zoomDiff is not too large (e.g., <= 3 or 4)
                                // This prevents overly dense lines at very low map zooms.
                                // The ZL21 selected tile fill (yellow) is handled by a separate block earlier.
                                if (subdivisions > 1 && zoomDiff <= 3) { // Threshold for drawing sub-lines
                                ...
                                // Always draw border for the main tile canvas of this grid layer if grid is active
                                // This ensures the tile itself is outlined even if subgrids are too dense to draw.
                ...
                // Removed the setTimeout block that attempted to re-set GlobusRgbTerrain.
                // The constructor should handle this.
                // A single resize call after a brief moment might still be beneficial if the container size isn't immediately correct.
                setTimeout(() => {
                    if (window.globus && window.globus.planet && window.globus.planet.renderer && typeof window.globus.planet.renderer.resize === 'function') {
                        console.log("DEBUG: Performing resize of OpenGlobus renderer (after initial setup).");
                        window.globus.planet.renderer.resize();
                    }
                }, 250);
        ...
        updateTilesetGroupMarkersOG(); // Update markers after initialization
        ...
        try {
            ...
            if (targetLayer) {
                ...
                if (featuresAtTile.length > 0) {
                    ...
                    if (state.highlightedGlobeGroupId === groupId) {
                        ...
                        state.highlightedGlobeGroupId = groupId; // For globe layer drawing
    ...
    */
    ...
    if (baseLayerSelectElement) { // Use renamed variable
        baseLayerSelectElement.addEventListener('change', function() {
            ...
            console.log(`[BASEMAP_SWITCH] 'base-layer-select' changed. Selected value: ${selectedValue}, Active Map Library: ${state.activeMapLibrary}`);
            ...
            if (selectedValue === 'add-custom') {
                ...
                console.log("[BASEMAP_SWITCH] 'add-custom' selected. Showing custom inputs.");
                return;
            }

            if (state.activeMapLibrary === 'openlayers') {
                console.log("[BASEMAP_SWITCH] Active library is OpenLayers.");
                if (state.olMap) {
                    console.log("[BASEMAP_SWITCH_OL] olMap instance exists.");
                    const layers = state.olMap.getLayers().getArray();
                    console.log(`[BASEMAP_SWITCH_OL] Current OL layers (count: ${layers.length}):`, layers.map(l => ({id: l.get('id'), type: l.get('type'), isBaseLayer: l.get('isBaseLayer'), sourceClass: l.getSource() ? l.getSource().constructor.name : 'N/A'})));
                    
                    const currentBaseLayer = layers.find(layer => layer.get('isBaseLayer') === true);
                    
                    if (currentBaseLayer) {
                        console.log("[BASEMAP_SWITCH_OL] Found current base layer:", currentBaseLayer);
                        try {
                            state.olMap.removeLayer(currentBaseLayer);
                            console.log("[BASEMAP_SWITCH_OL] Removed current base layer.");
                        } catch (e) {
                            console.error("[BASEMAP_SWITCH_OL] Error removing current base layer:", e);
                        }
                    } else {
                        console.warn("[BASEMAP_SWITCH_OL] No layer found with get('isBaseLayer') === true. Attempting to remove layer at index 0 if layers exist.");
                        if (layers.length > 0) {
                            try {
                                state.olMap.removeLayer(layers[0]);
                                console.log("[BASEMAP_SWITCH_OL] Removed layer at index 0 as a fallback.");
                            } catch (e) {
                                console.error("[BASEMAP_SWITCH_OL] Error removing layer at index 0:", e);
                            }
                        } else {
                             console.warn("[BASEMAP_SWITCH_OL] No layers to remove as fallback.");
                        }
                    }

                    if (olBaseLayers[selectedValue]) {
                        console.log(`[BASEMAP_SWITCH_OL] Definition found for selected value: ${selectedValue}`);
                        try {
                            const newSource = olBaseLayers[selectedValue].source();
                            console.log(`[BASEMAP_SWITCH_OL] Created new source for ${selectedValue}:`, newSource);
                            const newBaseLayerOL = new ol.layer.Tile({ source: newSource, isBaseLayer: true }); // Tag it
                            console.log(`[BASEMAP_SWITCH_OL] Created new OL base layer:`, newBaseLayerOL);
                            state.olMap.getLayers().insertAt(0, newBaseLayerOL);
                            console.log(`[BASEMAP_SWITCH_OL] OpenLayers basemap switched to: ${selectedValue}. New layers array:`, state.olMap.getLayers().getArray().map(l => ({id: l.get('id'), type: l.get('type'), isBaseLayer: l.get('isBaseLayer')})));
                        } catch (e) {
                            console.error(`[BASEMAP_SWITCH_OL] Error creating or inserting new base layer for ${selectedValue}:`, e);
                        }
                    } else {
                        console.warn(`[BASEMAP_SWITCH_OL] OpenLayers definition for ${selectedValue} not found.`);
                    }
                } else {
                    console.warn("[BASEMAP_SWITCH_OL] OpenLayers map (state.olMap) not initialized when trying to change basemap.");
                }
            } else if (state.activeMapLibrary === 'maplibre') {
                console.log("[BASEMAP_SWITCH] Active library is MapLibre.");
                if (state.mapLibreMap && state.mapLibreMap.isStyleLoaded()) {
                    updateMapLibreBasemap(selectedValue);
                } else if (state.mapLibreMap) {
                     console.log("[BASEMAP_SWITCH] MapLibre style not loaded, deferring basemap update to map 'load' event or next interaction.");
                } else {
                    console.warn("[BASEMAP_SWITCH] MapLibre map not initialized when trying to change basemap.");
                }
            ...
            console.log("[BASEMAP_SWITCH] Checking OpenGlobus sync requirement.");
            if (window.globus && window.globus.planet && ogBaseLayerMappings[selectedValue]) {
                 ...
                 if (window.globus.planet.layers && window.globus.planet.layers[ogLayerName]) { // Added check for layers object
                    window.globus.planet.setBaseLayer(window.globus.planet.layers[ogLayerName]);
                    console.log(`[BASEMAP_SWITCH] OpenGlobus basemap synced to: ${ogLayerName}`);
                 } else {
                    console.warn(`[BASEMAP_SWITCH] OpenGlobus layer named '${ogLayerName}' (for basemap '${selectedValue}') not found or layers object missing. Layers:`, window.globus.planet.layers);
                 }
            } else {
                console.log(`[BASEMAP_SWITCH] OpenGlobus sync not applicable. Globus: ${!!window.globus}, Planet: ${!!window.globus?.planet}, Mapping: ${ogBaseLayerMappings[selectedValue]}`);
            }
        });
    }
    ...
    if (masterMapModeBtn) {
        masterMapModeBtn.addEventListener('click', () => {
            ...
            if (state.activeMapLibrary === 'openlayers') {
                ...
                // OLCesium specific enable/disable based on masterMapModeIs3D
                if (olcsMapPanel) {
                    const cesiumIsEnabled = olcsMapPanel.getEnabled();
                    if (state.masterMapModeIs3D && !cesiumIsEnabled) {
                        ...
                        if (cesiumMapContainer) cesiumMapContainer.style.display = 'block';
                        if (mapElementForOL) mapElementForOL.style.display = 'none';
                        console.log("Master Toggle: OLCesium enabled for OpenLayers.");
                        ...
                        if (mapElementForOL) mapElementForOL.style.display = 'block';
                        if (state.olMap) state.olMap.updateSize();
                        console.log("Master Toggle: OLCesium disabled for OpenLayers.");
                    }
                }
            } else if (state.activeMapLibrary === 'maplibre') {
                console.log("Master Toggle: Handling MapLibre view.");
                if (mapLibreToggleBtn && state.mapLibreMap) {
                    const currentPitch = state.mapLibreMap.getPitch();
                    if (state.masterMapModeIs3D && currentPitch === 0) {
                        mapLibreToggleBtn.click();
                    } else if (!state.masterMapModeIs3D && currentPitch > 0) {
                        mapLibreToggleBtn.click();
                    }
                }
            }
            // OpenGlobus camera adjustment for 2D/3D master mode
            if (state.globus && state.globus.planet && state.globus.planet.camera) {
                const cam = state.globus.planet.camera;
                if (!state.masterMapModeIs3D) {
                    cam.setPitch(-Math.PI / 2);
                    cam.setBearing(0);
                    console.log("Master Toggle: OpenGlobus camera set to 2D (nadir).");
                } else {
                    if (cam.getPitch() === -Math.PI / 2) {
                         cam.setPitch(-Math.PI / 4);
                    }
                    console.log("Master Toggle: OpenGlobus camera set to 3D.");
                }
            }
            // Potentially call a function here to update visibility of all map/globe panels based on state.masterMapModeIs3D
            // and state.activeMapLibrary / state.activeGlobeLibrary
            console.log("Master Toggle: TODO - Implement unified panel visibility update based on masterMapModeIs3D and active libraries after mode switch.");
        });
    } else {
        console.warn("Master map mode toggle button ('master-map-mode-btn') not found.");
    }
    // --- End Master 2D/3D Button Logic ---

    // Set default for Scene Panel engine/type dropdown
    // Assuming sceneTypeDropdown is declared earlier in this scope or globally.
    // const sceneTypeDropdown = document.getElementById('scene-type-dropdown'); // Removed duplicate declaration
    if (document.getElementById('scene-type-dropdown')) { // Check directly
        document.getElementById('scene-type-dropdown').value = 'threejs';
        console.log("[INIT] Set 'scene-type-dropdown' default value to 'threejs'.");
        // If changing the value should trigger an action (like loading a default empty scene or specific UI changes):
        // const event = new Event('change', { bubbles: true });
        // document.getElementById('scene-type-dropdown').dispatchEvent(event);
    } else {
        console.warn("[INIT] Scene type dropdown ('scene-type-dropdown') not found. Cannot set default.");
    }

    // Initial setup of panel visibility based on default active libraries
    // This is a simplified version; a more robust solution would check current state.
    // For now, assume OpenLayers and OpenGlobus are default and visible.
    const olMapContainer = document.getElementById('map');
    const mapLibreContainer = document.getElementById('maplibre-map-container');
    // const leafletContainer = document.getElementById('leaflet-map-container'); // Leaflet commented out
    const globusContainer = document.getElementById('globusContainer');
    const itownsContainer = document.getElementById('itowns-container');
    const cesiumContainer = document.getElementById('cesium-map-container'); // OLCesium container

    if (olMapContainer) olMapContainer.style.display = (state.activeMapLibrary === 'openlayers' && !state.masterMapModeIs3D) ? 'block' : 'none';
    if (mapLibreContainer) mapLibreContainer.style.display = (state.activeMapLibrary === 'maplibre' && !state.masterMapModeIs3D) ? 'block' : 'none';
    // if (leafletContainer) leafletContainer.style.display = (state.activeMapLibrary === 'leaflet' && !state.masterMapModeIs3D) ? 'block' : 'none'; // Leaflet commented out
    
    if (globusContainer) globusContainer.style.display = (state.activeGlobeLibrary === 'openglobus' && state.masterMapModeIs3D) ? 'block' : 'none';
    if (itownsContainer) itownsContainer.style.display = (state.activeGlobeLibrary === 'itowns' && state.masterMapModeIs3D) ? 'block' : 'none';
    if (cesiumContainer) cesiumContainer.style.display = (state.activeMapLibrary === 'openlayers' && state.masterMapModeIs3D && olcsMapPanel && olcsMapPanel.getEnabled()) ? 'block' : 'none';


    console.log("DEBUG: DOMContentLoaded complete. Initial setup functions called.");
    // Roo Test: DOMContentLoaded finished
    // Note: The comments from lines 8630-8632 about orphaned braces were removed in the previous step.
    // The line 8633 "// --- End Master 2D/3D Button Logic ---" is also removed as it's now inaccurate.
    // The structure should flow directly from the panel visibility logic to the MutationObserver setup.
...
// Duplicated "Toggle for Maps Menu" block removed.
    ...
    // --- MutationObserver for Scene Panel Visibility to Auto-Load GLTF ---
    const scenePanelForObserver = document.getElementById('scene-panel');
    if (scenePanelForObserver && typeof loadGltfDataIntoSceneViewer === 'function') {
        let scenePanelWasVisible = scenePanelForObserver.style.display === 'block'; // Check initial state

        const scenePanelObserver = new MutationObserver((mutationsList, observer) => {
            for (const mutation of mutationsList) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                    const isNowVisible = scenePanelForObserver.style.display === 'block';
                    if (isNowVisible && !scenePanelWasVisible) { // Panel just became visible
                        console.log("[ScenePanelObserver] Scene panel became visible.");
                        if (window.latestGeneratedGltf && window.latestGeneratedGltf.data) {
                            console.log(`[ScenePanelObserver] Attempting to auto-load GLTF. ID: ${window.latestGeneratedGltf.id || 'latest generated'}, Name: ${window.latestGeneratedGltf.name}, Data present: ${!!window.latestGeneratedGltf.data}`);
                            loadGltfDataIntoSceneViewer(window.latestGeneratedGltf.data, window.latestGeneratedGltf.name);
                        } else {
                            console.log(`[ScenePanelObserver] Scene panel became visible, but no GLTF data in window.latestGeneratedGltf. latestGeneratedGltf:`, window.latestGeneratedGltf);
                        }
                    } else {
                        // console.log("[ScenePanelObserver] Scene panel became hidden or visibility check failed.");
                    }
                    scenePanelWasVisible = isNowVisible;
                    break;
                }
            }
        });
        scenePanelObserver.observe(scenePanelForObserver, { attributes: true, attributeFilter: ['style'] });
        console.log("MutationObserver set up for Scene Panel visibility changes to auto-load GLTF.");
    } else {
        if (!scenePanelForObserver) console.warn("Scene panel element ('scene-panel') not found for MutationObserver setup.");
        if (typeof loadGltfDataIntoSceneViewer !== 'function') console.warn("loadGltfDataIntoSceneViewer function not found for MutationObserver setup.");
    }
    // --- End MutationObserver for Scene Panel ---

    // --- MutationObserver for XR Panel Visibility to Auto-Load Default Engine ---
    const xrPanelForObserver = document.getElementById('xr-panel');
    // setupXRPanelLogic contains updateXRIframe, so we check for setupXRPanelLogic.
    if (xrPanelForObserver && typeof setupXRPanelLogic === 'function') {
        let xrPanelWasVisible = xrPanelForObserver.style.display === 'block';

        const xrPanelObserver = new MutationObserver((mutationsList, observer) => {
            for (const mutation of mutationsList) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                    const isNowVisible = xrPanelForObserver.style.display === 'block';
                    if (isNowVisible && !xrPanelWasVisible) {
                        console.log("[XRPanelObserver] XR panel became visible. Calling setupXRPanelLogic().");
                        setupXRPanelLogic(); // This initializes/loads the default engine

                        // After setup, if JanusWeb is the engine and a GLTF is ready, send it.
                        // Need a slight delay for xrIframe to potentially load and for its dataset.currentEngine to be set.
                        setTimeout(() => {
                            const xrIframe = document.getElementById('xr-iframe');
                            // The 'janusweb' engine now loads 'janus_room_with_og_surface.html' directly,
                            // which defines its own content. The postMessage for 'loadGLTFAsRoom'
                            // was intended for xr_janus_wrapper.html.
                            // If 'janus_room_with_og_surface.html' is also meant to dynamically load GLTFs
                            // via postMessage, it would need its own listener.
                            // For now, removing this postMessage for the 'janusweb' case to simplify initial loading.
                            // The sendGltfToJanusIfActive() function also exists and is called from initThumbnailViewer,
                            // which might be another point of sending. This observer ensures it's sent when XR panel becomes visible.
                            if (xrIframe && xrIframe.dataset.currentEngine === 'janusweb' && xrIframe.contentWindow) {
                                if (window.latestGeneratedGltf && window.latestGeneratedGltf.data) {
                                    console.log(`[XRPanelObserver] XR panel visible with 'janusweb'. Posting 'loadGLTFAsRoom' for GLTF: ${window.latestGeneratedGltf.name} to janus_room_with_og_surface.html`);
                                    
                                    // TODO: Include geospatial data for precise placement.
                                    // For now, sending basic info.
                                    // const centerCoords = window.latestGeneratedGltf.center; // Example, if available
                                    // const extent = window.latestGeneratedGltf.extent; // Example

                                    xrIframe.contentWindow.postMessage({
                                        type: 'loadGLTFAsRoom',
                                        gltfString: JSON.stringify(window.latestGeneratedGltf.data), // Ensure it's stringified
                                        tilesetName: window.latestGeneratedGltf.name,
                                        // placeholder_center: centerCoords, // Send geospatial data when available
                                        // placeholder_extent: extent       // Send geospatial data when available
                                    }, '*');
                                } else {
                                    console.log("[XRPanelObserver] XR panel visible with 'janusweb', but no latestGeneratedGltf data to send.");
                                }
                            } else if (xrIframe && xrIframe.contentWindow) { // For other potential Janus wrappers or future use
                                 console.log(`[XRPanelObserver] XR panel visible with ${xrIframe.dataset.currentEngine}. Not 'janusweb', or contentWindow not ready. Current logic might not post.`);
                                // If other engines need similar behavior, add specific conditions here.
                            } else {
                                console.log("[XRPanelObserver] XR Iframe not ready for GLTF postMessage or engine not identified for this specific logic.");
                            }
                        }, 1000); // Delay to allow iframe to initialize
                    }
                    xrPanelWasVisible = isNowVisible;
                    break;
                }
            }
        });
        xrPanelObserver.observe(xrPanelForObserver, { attributes: true, attributeFilter: ['style'] });
        console.log("MutationObserver set up for XR Panel visibility changes to auto-load default XR engine.");
    } else {
        if (!xrPanelForObserver) console.warn("XR panel element ('xr-panel') not found for MutationObserver setup.");
        if (typeof setupXRPanelLogic !== 'function') console.warn("setupXRPanelLogic function not found for XR Panel MutationObserver setup.");
    }
    // --- End MutationObserver for XR Panel ---
