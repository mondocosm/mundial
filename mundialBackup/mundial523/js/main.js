...
function initThumbnailViewer(containerId, tilesetGroupId) {
    ...
    function openTilesetDetailsModal(feature) {
        ...
        if (!feature || !tilesetDetailsModal || !detailsTilesetNameInput || !detailsColorPicker ||
            ...
            !detailsTilesetImage || !detailsTilesetCoordsSpan || !detailsLocationInfoSpan ||
...
async function generate3DAssetsFromTileset(tilesetGroupId, tilesetName, savedFeatures) {
    ...
    const use3DTilesMethod = false; // FORCE RASTER DEM PATH for debugging raster issues
    console.log(`[3D_ASSETS] Thumbnail generation method: FORCED Raster DEM (Scanned)`);
    ...
    tileMeshesGroup = new THREE.Group();
    allPoints = [];
    const material = new THREE.MeshStandardMaterial({ color: 0x808080, wireframe: true, side: THREE.DoubleSide }); // Enabled wireframe
    ...
    if (compositeImageData && tileInfoList.some(ti => !ti.fetchError && ti.heightDecodeFn)) {
        console.log("[3D_ASSETS_COMPOSITE_TEST] Generating single mesh from composite DEM.");
        ...
        if (segmentsWidth <= 0 || segmentsHeight <= 0) { // Check for non-positive segments
            ...
            const geometry = new THREE.PlaneGeometry(planeWidth, planeHeight, segmentsWidth, segmentsHeight);
            const vertices = geometry.attributes.position;
            const compData = compositeImageData.data;
            const compWidth = compositeImageData.width;

            const firstValidTileInfo = tileInfoList.find(ti => ti.heightDecodeFn && !ti.fetchError);
            const decodeFnToUse = firstValidTileInfo ? firstValidTileInfo.heightDecodeFn : (r,g,b) => 0;

            for (let j = 0; j <= segmentsHeight; j++) {
                for (let k = 0; k <= segmentsWidth; k++) {
                    const u_norm_plane = k / segmentsWidth;
                    const v_norm_plane = j / segmentsHeight;

                    const imgPx = Math.min(Math.floor(u_norm_plane * compWidth), compWidth - 1);
                    const imgPy = Math.min(Math.floor(v_norm_plane * compositeImageData.height), compositeImageData.height - 1);
                    
                    const rIndex = (imgPy * compWidth + imgPx) * 4;
                    const rVal = compData[rIndex], gVal = compData[rIndex + 1], bVal = compData[rIndex + 2];
                    let height = decodeFnToUse(rVal, gVal, bVal);
                    
                    // Spike detection and clamping
                    const MAX_EXPECTED_HEIGHT = 9000; // Slightly above Everest
                    const MIN_EXPECTED_HEIGHT = -11000; // Slightly below Mariana Trench
                    if (height > MAX_EXPECTED_HEIGHT || height < MIN_EXPECTED_HEIGHT) {
                        console.warn(`[3D_ASSETS_COMPOSITE_SPIKE] Extreme decoded height: ${height} from R=${rVal},G=${gVal},B=${bVal} at imgPx=${imgPx},imgPy=${imgPy}. Clamping to 0.`);
                        height = 0; // Clamp extreme values to 0 for now
                    }

                    const vertexIndex = j * (segmentsWidth + 1) + k;
                    vertices.setZ(vertexIndex, height * MESH_HEIGHT_SCALE);
                }
            }
            geometry.attributes.position.needsUpdate = true;
            geometry.computeVertexNormals();
            const compositeMesh = new THREE.Mesh(geometry, material);
            compositeMesh.rotation.x = -Math.PI / 2;
            compositeMesh.position.set( (planeWidth / 2) - 0.5 , 0, -((planeHeight / 2) - 0.5) );
            tileMeshesGroup.add(compositeMesh);
        }
    } else {
        console.log("[3D_ASSETS_COMPOSITE] Fallback: Generating single large flat plane (no compositeImageData or no valid decoder).");
        const planeW = numTilesX > 0 ? numTilesX : 1;
        const planeH = numTilesY > 0 ? numTilesY : 1;
        const fallbackSegmentsX = Math.max(1, planeW > 0 ? planeW * (SAMPLING_RESOLUTION -1) : 1);
        const fallbackSegmentsY = Math.max(1, planeH > 0 ? planeH * (SAMPLING_RESOLUTION -1) : 1);

        const fallbackPlaneGeom = new THREE.PlaneGeometry(planeW, planeH, fallbackSegmentsX, fallbackSegmentsY);
        const fallbackMesh = new THREE.Mesh(fallbackPlaneGeom, material);
        fallbackMesh.rotation.x = -Math.PI / 2;
        fallbackMesh.position.set((planeW / 2) - 0.5, 0, -((planeH / 2) - 0.5));
        tileMeshesGroup.add(fallbackMesh);
    }
    allPoints = [];
