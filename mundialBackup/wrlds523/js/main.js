...
async function generate3DAssetsFromTileset(tilesetGroupId, tilesetName, savedFeatures) {
    ...
    if (use3DTilesMethod) {
        ...
        // const userMapTilerApiKey = localStorage.getItem('mapTilerApiKey') || 'YOUR_MAPTILER_API_KEY_PLACEHOLDER'; // Moved to be defined just before skipDemFetching calculation
    ...
    const userMapTilerApiKey = localStorage.getItem('mapTilerApiKey') || 'YOUR_MAPTILER_API_KEY_PLACEHOLDER'; // Ensure this is defined before use
    ...
    let demPromises = [];
    // Fetch DEMs and Basemaps
    // if (!skipDemFetching) { // This condition is for DEMs, basemaps might still be fetched
    console.log(`[3D_ASSETS_COMPOSITE] Path: Preparing to fetch DEMs (if not skipped) and Basemaps.`);
    tileInfoList.forEach(tileInfo => {
        if (tileInfo.x === null || tileInfo.y === null) {
            tileInfo.fetchError = true;
            tileInfo.basemapFetchError = true; // Also mark basemap error
            return;
        }

        const { x: xOrig, y: yOrig, zLevel: zOrig } = tileInfo; // zOrig is the target ZL21

        // DEM Fetching Logic (only if not skipping)
        if (!skipDemFetching) {
            let demUrl = '', heightDecodeFn = null, zDem = zOrig, MAX_DEM_SERVICE_ZOOM = 15; // Renamed MAX_SERVICE_ZOOM for clarity
            // ... (existing DEM source logic for terrarium, maptiler, arcgis) ...
            if (actualDemSourceToUse === 'terrarium') {
                MAX_DEM_SERVICE_ZOOM = 15;
                zDem = zOrig > MAX_DEM_SERVICE_ZOOM ? MAX_DEM_SERVICE_ZOOM : zOrig;
                let xDemTile = zOrig > MAX_DEM_SERVICE_ZOOM ? Math.floor(xOrig / (2 ** (zOrig - MAX_DEM_SERVICE_ZOOM))) : xOrig;
                let yDemTile = zOrig > MAX_DEM_SERVICE_ZOOM ? Math.floor(yOrig / (2 ** (zOrig - MAX_DEM_SERVICE_ZOOM))) : yOrig;
                ...
                MAX_DEM_SERVICE_ZOOM = 15; // MapTiler Terrain RGB max zoom is typically 15
                zDem = zOrig > MAX_DEM_SERVICE_ZOOM ? MAX_DEM_SERVICE_ZOOM : zOrig;
                let xDemTile = zOrig > MAX_DEM_SERVICE_ZOOM ? Math.floor(xOrig / (2 ** (zOrig - MAX_DEM_SERVICE_ZOOM))) : xOrig;
                let yDemTile = zOrig > MAX_DEM_SERVICE_ZOOM ? Math.floor(yOrig / (2 ** (zOrig - MAX_DEM_SERVICE_ZOOM))) : yOrig;
                ...
                MAX_DEM_SERVICE_ZOOM = 17; // ArcGIS Terrain3D LERC max zoom
                zDem = zOrig > MAX_DEM_SERVICE_ZOOM ? MAX_DEM_SERVICE_ZOOM : zOrig;
                let xDemTile = zOrig > MAX_DEM_SERVICE_ZOOM ? Math.floor(xOrig / (2 ** (zOrig - MAX_DEM_SERVICE_ZOOM))) : xOrig;
                let yDemTile = zOrig > MAX_DEM_SERVICE_ZOOM ? Math.floor(yOrig / (2 ** (zOrig - MAX_DEM_SERVICE_ZOOM))) : yOrig;
                demUrl = `https://elevation3d.arcgis.com/arcgis/rest/services/WorldElevation3D/Terrain3D/ImageServer/tile/${zDem}/${yDemTile}/${xDemTile}`; // Note: y,x order for ArcGIS
                heightDecodeFn = (r, g, b) => (r * 256 + g + b / 256) - 32768; // LERC for ArcGIS
            } else {
                ...
                tileInfo.fetchError = true;
                // Do not return here, still attempt basemap fetch
            }
            
            if (!tileInfo.fetchError) { // Only set DEM info and add promise if not already errored from source type
                tileInfo.demUrl = demUrl;
                tileInfo.heightDecodeFn = heightDecodeFn;
                tileInfo.zDem = zDem;
                ...
                demPromises.push(
                    fetch(demUrl)
                        .then(response => {
                            if (!response.ok) {
                                console.warn(`[3D_ASSETS_COMPOSITE] Failed to fetch DEM for tile ${tileInfo.originalIndex} from ${demUrl}: ${response.status} ${response.statusText}`);
                        ...
                        .catch(error => {
                            console.error(`[3D_ASSETS_COMPOSITE] Error fetching/processing DEM for tile ${tileInfo.originalIndex} from ${demUrl}:`, error);
                            tileInfo.fetchError = true;
                        })
                );
            }
        } else {
            tileInfo.fetchError = true; // If skipping DEM fetching, mark as error so it generates flat
        }
        
        // Prepare Basemap Fetch (always attempt this, independent of DEM skipping)
        ...
        tileInfo.zBasemapActual = zBasemapFetch; // Store the actual zoom level fetched for the basemap

        console.log(`[3D_ASSETS_COMPOSITE] Basemap Fetch Prep: Tile ${tileInfo.originalIndex} (Target Z${zOrig}/${xOrig}/${yOrig}), Fetching Basemap Z${zBasemapFetch} from ${basemapUrl}`);

        // Basemap Promise
demPromises.push( 
            fetch(tileInfo.basemapUrl) 
                .then(response => {
                    if (!response.ok) {
                        console.warn(`[3D_ASSETS_COMPOSITE] Failed to fetch Basemap for tile ${tileInfo.originalIndex} from ${tileInfo.basemapUrl}: ${response.status} ${response.statusText}`);
                ...
                .catch(error => {
                    console.error(`[3D_ASSETS_COMPOSITE] Error fetching/processing Basemap for tile ${tileInfo.originalIndex} from ${tileInfo.basemapUrl}:`, error);
                    tileInfo.basemapFetchError = true;
                })
        ); // End of Basemap Promise push
    }); // End tileInfoList.forEach
// Await all promises (DEM and Basemap) after the loop
    if (demPromises.length > 0) {
        console.log(`[3D_ASSETS_COMPOSITE] Waiting for ${demPromises.length} DEM/Basemap fetch promises.`);
        await Promise.allSettled(demPromises);
        console.log(`[3D_ASSETS_COMPOSITE] All DEM/Basemap promises settled.`);
    } else {
         console.log(`[3D_ASSETS_COMPOSITE] No DEM/Basemap promises created (e.g. all tiles invalid or fetch skipped).`);
    }
// This entire block (lines 4353-4402) is a remnant of old logic and is being removed.
// The correct promise handling and await logic will be inserted separately
// after the main tileInfoList.forEach loop which ends at line 4352.
    ...
    if (compositeImageData && tileInfoList.some(ti => !ti.fetchError && ti.heightDecodeFn)) {
        ...
        if (segmentsWidth <= 0 || segmentsHeight <= 0) { // Check for non-positive segments
            ...
            tileInfoList.forEach((tileInfo, idx) => {
                ...
                // 1. Main mesh, now potentially textured
                let meshMaterial = material; // Default to the wireframe material
                if (tileInfo.basemapImageBitmap) {
                    try {
                        const satTexture = new THREE.Texture(tileInfo.basemapImageBitmap);
                        satTexture.needsUpdate = true;
                        // Ensure image is power-of-two if using WebGL1 and certain wrap/filter modes,
                        // but for simple map:texture it's often fine.
                        // Consider satTexture.colorSpace = THREE.SRGBColorSpace; if using MeshStandardMaterial and colors look off.
                        
                        meshMaterial = new THREE.MeshStandardMaterial({ // Using StandardMaterial for better lighting if desired
                            map: satTexture,
                            side: THREE.DoubleSide,
                            // wireframe: false, // Optionally turn off wireframe when texture is applied
                        });
                        console.log(`[3D_ASSETS_TEXTURE] Applied satellite texture to tile ${tileInfo.originalIndex}`);
                    } catch (texError) {
                        console.error(`[3D_ASSETS_TEXTURE] Error creating texture for tile ${tileInfo.originalIndex}:`, texError);
                        // Fallback to default material if texture creation fails
