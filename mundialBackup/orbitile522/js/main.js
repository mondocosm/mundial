...
import { load } from '../packages/loaders.gl/modules/core/src/index.js'; // Adjust path if needed by user
import { Tiles3DLoader } from '../packages/loaders.gl/modules/3d-tiles/src/index.js'; // Adjust path
import { TerrainLoader } from '../packages/loaders.gl/modules/terrain/src/index.js'; // Adjust path
import { QuantizedMeshLoader } from '../packages/loaders.gl/modules/terrain/src/quantized-mesh-loader.js'; // Adjust path
import { load } from '../packages/loaders.gl/modules/core/src/index.js'; // Adjust path if needed by user
import { Tiles3DLoader } from '../packages/loaders.gl/modules/3d-tiles/src/index.js'; // Adjust path
import { TerrainLoader } from '../packages/loaders.gl/modules/terrain/src/index.js'; // Adjust path
import { QuantizedMeshLoader } from '../packages/loaders.gl/modules/terrain/src/quantized-mesh-loader.js'; // Adjust path
        ...
        try {
            ...
            const clickSelectHandler = function (evt) {
                ...
                if (clickedSavedFeature && clickedGroupId) {
                    ...
                    if (selectionSource) {
                        const featuresBeforeClear = selectionSource.getFeatures().map(f => ({ id: f.getId(), isGroup: f.get('isGroupSelection') }));
                        console.log("[ClickSelect] Features in selectionSource BEFORE potential clear:", featuresBeforeClear);
                        const isGroupCurrentlySelected = featuresBeforeClear.some(f => f.isGroup);
                        console.log("[ClickSelect] isGroupCurrentlySelected:", isGroupCurrentlySelected);
                        if (isGroupCurrentlySelected) {
                            console.log("[ClickSelect] Group was selected, calling clearMapSelectionAndDetails().");
                            clearMapSelectionAndDetails();
                        } else {
                            console.log("[ClickSelect] No group was selected, or selectionSource was empty. Not calling clearMapSelectionAndDetails() based on group presence.");
                        }
                    } else {
                        console.log("[ClickSelect] selectionSource is null.");
                    }
...
function initThumbnailViewer(containerId, tilesetGroupId) {
    ...
    alert("DEBUG: Script is trying to set up modal button listeners NOW."); // Simple alert for execution check
    console.log("!!! SCRIPT EXECUTION REACHED DETAILS MODAL LISTENERS SETUP !!!"); // VERY BASIC DEBUG (Pre-Details Modal Listeners)
...
async function generate3DAssetsFromTileset(tilesetGroupId, tilesetName, savedFeatures) {
    console.log(`[3D_ASSETS_NEW] Starting 3D asset generation for ${tilesetName} (ID: ${tilesetGroupId}) with ${savedFeatures.length} features.`);

    const use3DTilesMethod = true; // Control flag for new method, can be tied to settings later
    ...
    if (typeof THREE === 'undefined') {
        console.error("[3D_ASSETS_NEW] THREE.js is not loaded.");
        return { pointCloud: { error: "THREE.js missing" }, gltf: { error: "THREE.js missing" } };
    }
    ...
    if (use3DTilesMethod) {
        ...
        try {
            const tileset = await load(TILESET_URL_CESIUM_WORLD_TERRAIN, Tiles3DLoader, {
                cesiumion: { accessToken: CESIUM_ION_TOKEN }
            });
            console.log('[3D_ASSETS_NEW] Root tileset.json loaded:', tileset);

            // TODO: Implement traversal logic based on 'savedFeatures' geographic extent
            // For each relevant tile from traversal:
            //   Determine content type (b3dm, i3dm, terrain, glb)
            //   Load content using appropriate loader (Tiles3DLoader for b3dm/i3dm, QuantizedMeshLoader for .terrain, GLTFLoader for .glb)
            //   Parse to THREE.js mesh, apply transform, add to tileMeshesGroup
            //   Collect points for allPoints array from the final transformed meshes
        ...
        console.log(`[3D_ASSETS] Using DEM source preference (fallback): ${demSourcePreference}`);
        console.error("[3D_ASSETS] THREE.js is not loaded.");
        window.latestGeneratedPointCloud = { id: tilesetGroupId, name: tilesetName, data: [], error: "THREE.js missing", source: demSourcePreference };
        window.latestGeneratedGltf = { id: tilesetGroupId, name: tilesetName, data: null, error: "THREE.js missing", source: demSourcePreference };
        return { pointCloud: window.latestGeneratedPointCloud, gltf: window.latestGeneratedGltf };
    }
    ...
    tileMeshesGroup = new THREE.Group();
    allPoints = [];
    const material = new THREE.MeshStandardMaterial({ color: 0x808080, wireframe: false, side: THREE.DoubleSide });
    const TILE_SIZE = 256; // Standard tile size for DEM images
    const MESH_HEIGHT_SCALE = 0.01; // Reduced scale for less bumpy terrain
    const SAMPLING_RESOLUTION = 16; // Default sampling resolution for mesh generation
    ...
    // Calculate minX, minY, and parse tile coordinates for all saved features ONCE.
    ...
    if (savedFeatures && savedFeatures.length > 0) {
        ...
        if (minX !== Infinity && minY !== Infinity) {
            ...
            minX = (minX === Infinity) ? 0 : minX; // Default if no valid tiles
            minY = (minY === Infinity) ? 0 : minY;
        }
    } else { // No features, handle early exit for asset generation
        console.warn("[3D_ASSETS] No saved features to process for 3D asset generation.");
        window.latestGeneratedPointCloud = { id: tilesetGroupId, name: tilesetName, data: [], source: demSourcePreference, error: "No features" };
        if (typeof THREE !== 'undefined' && THREE.GLTFExporter) { // Check if THREE and exporter exist
            const exporter = new THREE.GLTFExporter();
            exporter.parse(new THREE.Scene(), (gltf) => {
                window.latestGeneratedGltf = { id: tilesetGroupId, name: tilesetName, data: gltf, source: demSourcePreference, error: "No features" };
            }, (error) => {
                 window.latestGeneratedGltf = { id: tilesetGroupId, name: tilesetName, data: null, error: "GLTF Export Error on empty scene", source: demSourcePreference };
            }, { onlyVisible: false });
        } else {
            window.latestGeneratedGltf = { id: tilesetGroupId, name: tilesetName, data: null, error: "THREE.GLTFExporter not available, no features", source: demSourcePreference };
        }
        // Return early as there's nothing to process further in this function
        return { pointCloud: window.latestGeneratedPointCloud, gltf: window.latestGeneratedGltf };
    }

    if (demSourcePreference === 'flat' ||
       (demSourcePreference === 'maptiler' && (userMapTilerApiKey === 'YOUR_MAPTILER_API_KEY_PLACEHOLDER' || !userMapTilerApiKey))
       // Add other conditions for flat fallback if new sources are added and not configured
    ) {
        let reasonForFlat = demSourcePreference === 'flat' ? "Preference is 'flat'" : "MapTiler API key not configured";
        // Example if Terrarium was temporarily disabled by a flag (demonstrating extensibility)
        // if (demSourcePreference === 'terrarium' && SOME_FLAG_TO_DISABLE_TERRARIUM) {
        //     reasonForFlat = "Terrarium processing temporarily disabled";
        // }
        console.log(`[3D_ASSETS] Generating flat placeholder planes. Reason: ${reasonForFlat}.`);
        for (let i = 0; i < savedFeatures.length; i++) {
            // const feature = savedFeatures[i]; // feature is in coord object
            ...
            if (coord && coord.x !== null && coord.y !== null) {
                ...
                const flatMesh = new THREE.Mesh(flatPlaneGeom, material); // material is defined above
                ...
                console.warn(`[3D_ASSETS] Flat plane: Could not use pre-calculated coords for feature index ${i}. Using ad-hoc fallback.`);
                // Ad-hoc fallback if coords were bad from pre-scan
                const flatPlaneGeomAdhoc = new THREE.PlaneGeometry(1, 1, 1, 1);
                const flatMeshAdhoc = new THREE.Mesh(flatPlaneGeomAdhoc, material);
                flatMeshAdhoc.position.x = (i % 10) * 1.05; // Simple indexed positioning
                flatMeshAdhoc.position.y = 0;
                flatMeshAdhoc.position.z = Math.floor(i / 10) * 1.05;
                flatMeshAdhoc.rotation.x = -Math.PI / 2;
                tileMeshesGroup.add(flatMeshAdhoc);
            }
        }
    } else { // DEM Source selected
        console.log(`[3D_ASSETS] Processing DEM source: ${demSourcePreference}`); // Added log from previous attempt
        let demPromises = []; // Initialize demPromises here
        for (let i = 0; i < savedFeatures.length; i++) {
            const feature = savedFeatures[i];
            const tileId = feature.get('tileId');
            if (!tileId) {
                console.warn(`[3D_ASSETS] DEM: Feature index ${i} missing tileId. Using flat fallback.`);
                const flatPlaneGeomNoID = new THREE.PlaneGeometry(1, 1, 1, 1);
                const flatMeshNoID = new THREE.Mesh(flatPlaneGeomNoID, material);
                // Use simple indexed positioning as no coord info is available
                flatMeshNoID.position.x = (i % 10) * 1.05; flatMeshNoID.position.y = 0; flatMeshNoID.position.z = Math.floor(i / 10) * 1.05;
                flatMeshNoID.rotation.x = -Math.PI / 2;
                tileMeshesGroup.add(flatMeshNoID);
                demPromises.push(Promise.resolve()); // Add a resolved promise
                continue;
            }
            const parts = tileId.split('-');
            if (parts.length !== 3) {
                console.warn(`[3D_ASSETS] DEM: Invalid tileId format '${tileId}' for feature index ${i}. Using flat fallback.`);
                const coord = tileCoords.find(tc => tc.originalIndex === i); // Attempt to use pre-calculated coord if available
                const flatPlaneGeomFormat = new THREE.PlaneGeometry(1, 1, 1, 1);
                const flatMeshFormat = new THREE.Mesh(flatPlaneGeomFormat, material);
                if (coord && coord.x !== null && coord.y !== null) {
                    flatMeshFormat.position.set((coord.x - minX) * 1.0, 0, -(coord.y - minY) * 1.0);
                } else { // Absolute fallback if no coords
                    flatMeshFormat.position.x = (i % 10) * 1.05; flatMeshFormat.position.y = 0; flatMeshFormat.position.z = Math.floor(i / 10) * 1.05;
                }
                flatMeshFormat.rotation.x = -Math.PI / 2;
                tileMeshesGroup.add(flatMeshFormat);
                demPromises.push(Promise.resolve()); // Add a resolved promise
                continue;
            }
            const [zOrig, xOrig, yOrig] = parts.map(Number);

            let demUrl = '';
            let heightDecodeFn = null;
            let currentDemSourceForTile = demSourcePreference;
            let MAX_SERVICE_ZOOM = 15;
            let zDem; // Declare zDem here to be accessible in sampling logic
            ...
            if (demSourcePreference === 'terrarium') {
                ...
                console.log(`[3D_ASSETS] Terrarium: Fetching Z${zDem} tile (${zDem}-${xDem}-${yDem}) for original Z${zOrig} tile ${tileId}`);
            } else if (demSourcePreference === 'maptiler') {
                MAX_SERVICE_ZOOM = 15;
                zDem = zOrig > MAX_SERVICE_ZOOM ? MAX_SERVICE_ZOOM : zOrig; // Assign to common zDem
                let xDem = zOrig > MAX_SERVICE_ZOOM ? Math.floor(xOrig / (2 ** (zOrig - MAX_SERVICE_ZOOM))) : xOrig;
                let yDem = zOrig > MAX_SERVICE_ZOOM ? Math.floor(yOrig / (2 ** (zOrig - MAX_SERVICE_ZOOM))) : yOrig;
                ...
                console.log(`[3D_ASSETS] MapTiler: Fetching Z${zDem} tile (${zDem}-${xDem}-${yDem}) for original Z${zOrig} tile ${tileId}`);
            } else if (demSourcePreference === 'arcgis') {
                MAX_SERVICE_ZOOM = 17;
                zDem = zOrig > MAX_SERVICE_ZOOM ? MAX_SERVICE_ZOOM : zOrig; // Assign to common zDem
                let xDem = zOrig > MAX_SERVICE_ZOOM ? Math.floor(xOrig / (2 ** (zOrig - MAX_SERVICE_ZOOM))) : xOrig;
                let yDem = zOrig > MAX_SERVICE_ZOOM ? Math.floor(yOrig / (2 ** (zOrig - MAX_SERVICE_ZOOM))) : yOrig;
                demUrl = `https://elevation3d.arcgis.com/arcgis/rest/services/WorldElevation3D/Terrain3D/ImageServer/tile/${zDem}/${yDem}/${xDem}`;
                heightDecodeFn = (r, g, b) => (r * 256 + g + b / 256) - 32768; // Terrarium style, experimental for ArcGIS
                console.log(`[3D_ASSETS] ArcGIS: Fetching Z${zDem} tile (${zDem}-${yDem}-${xDem}) for original Z${zOrig} tile ${tileId}. (Experimental)`);
            } else { // Final else for unknown/unconfigured DEM source
                console.warn(`[3D_ASSETS] Unknown or unconfigured DEM source: ${demSourcePreference} for tile index ${i}, ID ${tileId}. Using flat fallback.`);
                const coord = tileCoords.find(tc => tc.originalIndex === i);
                if (coord && coord.x !== null && coord.y !== null) {
                    const flatPlaneGeom = new THREE.PlaneGeometry(1, 1, 1, 1);
                    ...
                    flatMesh.rotation.x = -Math.PI / 2;
                    tileMeshesGroup.add(flatMesh);
                } else { // Absolute fallback if no coords from pre-scan
                    const flatPlaneGeom = new THREE.PlaneGeometry(1, 1, 1, 1);
                    const flatMesh = new THREE.Mesh(flatPlaneGeom, material);
                ...
                demPromises.push(Promise.resolve()); // Add a resolved promise as this path doesn't fetch
            ...
            try {
                ...
                if (!response.ok) {
                    console.warn(`[3D_ASSETS] Failed to fetch DEM for ${tileId} (index ${i}) from ${currentDemSourceForTile}: ${response.statusText} (URL: ${demUrl.replace(userMapTilerApiKey, '***KEY***')})`);
                    const coordForFetchFail = tileCoords.find(tc => tc.originalIndex === i);
                    const flatPlaneGeomFetchFail = new THREE.PlaneGeometry(1, 1, 1, 1);
                    const flatMeshFetchFail = new THREE.Mesh(flatPlaneGeomFetchFail, material);
                    if (coordForFetchFail && coordForFetchFail.x !== null && coordForFetchFail.y !== null) {
                        flatMeshFetchFail.position.set((coordForFetchFail.x - minX) * 1.0, 0, -(coordForFetchFail.y - minY) * 1.0);
                    } else { // Absolute fallback if no coords
                        flatMeshFetchFail.position.x = (i % 10) * 1.05; flatMeshFetchFail.position.y = 0; flatMeshFetchFail.position.z = Math.floor(i / 10) * 1.05;
                    }
                    flatMeshFetchFail.rotation.x = -Math.PI / 2;
                    tileMeshesGroup.add(flatMeshFetchFail);
                    // Note: This path is inside a .then() for a promise in demPromises.
                    // The promise itself will reject due to the error, so no need to push another resolved promise.
                    continue; // Continue to next iteration in the main for loop if possible (though this is inside a promise chain)
                }
                ...
                const tilePoints = [];
                // Use SAMPLING_RESOLUTION for the plane geometry segments
                ...
                for (let py = 0; py < SAMPLING_RESOLUTION; py++) {
                    for (let px = 0; px < SAMPLING_RESOLUTION; px++) {
                        // Determine which pixel to sample from the TILE_SIZE x TILE_SIZE DEM image
                        // This needs to map the Z_orig tile's sub-region within the Z_dem tile if they are different.
                        let finalSampleX, finalSampleY;

                        if (zOrig > zDem) {
                            const scaleFactor = 2 ** (zOrig - zDem);
                            // xOrig, yOrig are for the Z21 tile
                            const tileX_in_parent_at_zDem = Math.floor(xOrig / scaleFactor); // This should match xDem
                            const tileY_in_parent_at_zDem = Math.floor(yOrig / scaleFactor); // This should match yDem

                            // Calculate the Z21 tile's offset within its Z_dem parent tile (0 to scaleFactor-1)
                            const xOffset_within_zDem_tile_in_zOrig_units = xOrig % scaleFactor;
                            const yOffset_within_zDem_tile_in_zOrig_units = yOrig % scaleFactor;

                            // Size of one zOrig tile in terms of pixels of the zDem image
                            const pixels_per_zOrig_tile_in_DEM_image = TILE_SIZE / scaleFactor;
                            
                            // Top-left corner of the target zOrig sub-region within the zDem image (in zDem image pixels)
                            const subRegionX_start_pixels = xOffset_within_zDem_tile_in_zOrig_units * pixels_per_zOrig_tile_in_DEM_image;
                            const subRegionY_start_pixels = yOffset_within_zDem_tile_in_zOrig_units * pixels_per_zOrig_tile_in_DEM_image;

                            // Sample within this sub-region
                            const sampleX_local_to_subregion = Math.floor(px * (pixels_per_zOrig_tile_in_DEM_image / (SAMPLING_RESOLUTION - 1)));
                            const sampleY_local_to_subregion = Math.floor(py * (pixels_per_zOrig_tile_in_DEM_image / (SAMPLING_RESOLUTION - 1)));
                            
                            finalSampleX = Math.min(Math.floor(subRegionX_start_pixels + sampleX_local_to_subregion), TILE_SIZE - 1);
                            finalSampleY = Math.min(Math.floor(subRegionY_start_pixels + sampleY_local_to_subregion), TILE_SIZE - 1);
                        } else { // zOrig <= zDem (usually zOrig == zDem), sample the whole tile normally
                            finalSampleX = Math.min(Math.floor(px * (TILE_SIZE / (SAMPLING_RESOLUTION - 1))), TILE_SIZE - 1);
                            finalSampleY = Math.min(Math.floor(py * (TILE_SIZE / (SAMPLING_RESOLUTION - 1))), TILE_SIZE - 1);
                        }

                        const rIndex = (finalSampleY * TILE_SIZE + finalSampleX) * 4;
                        
                        const r = data[rIndex], g = data[rIndex + 1], b = data[rIndex + 2];
                        const height = heightDecodeFn(r, g, b);
                        
                        // The tilePixelToLocal3D function expects px,py in the SAMPLING_RESOLUTION grid
                        const point3D = tilePixelToLocal3D(px, py, height, i % 5, Math.floor(i / 5), SAMPLING_RESOLUTION);
                        tilePoints.push(point3D);
                        const vertexIndex = py * SAMPLING_RESOLUTION + px;
                        if (vertices.getZ(vertexIndex) !== undefined) {
                             vertices.setZ(vertexIndex, point3D.z);
                        }
                    }
                }
                ...
                // Position the DEM tile mesh correctly using pre-calculated coords
                const coordForDEM = tileCoords.find(tc => tc.originalIndex === i);
                if (coordForDEM && coordForDEM.x !== null && coordForDEM.y !== null) {
                    tileMesh.position.set(
                        (coordForDEM.x - minX) * 1.0, // Use 1.0 spacing, adjust if overlap desired
                        0,                            // Base Y for the plane itself, height is in vertices
                        -(coordForDEM.y - minY) * 1.0  // Map map Y to Three.js -Z
                    );
                    // The PlaneGeometry vertices define height along their local Z.
                    // Rotate the plane so its local Z (height) aligns with world Y (up).
                    tileMesh.rotation.x = -Math.PI / 2;
                } else {
                    console.warn(`[3D_ASSETS] DEM: Could not find pre-calculated coords for feature index ${i} (TileID: ${tileId}). Using simple indexed grid for this tile.`);
                    // Fallback to old indexed positioning if coords are missing for this specific tile
                    tileMesh.position.x = (i % 5) * 1.05;
                    tileMesh.position.y = 0;
                    tileMesh.position.z = Math.floor(i / 5) * 1.05;
                    tileMesh.rotation.x = -Math.PI / 2; // Make it horizontal
                }
                tileMeshesGroup.add(tileMesh);

                // Add corners of the mesh's world AABB to allPoints
                const tileAABB = new THREE.Box3().setFromObject(tileMesh);
                allPoints.push(
                    { x: tileAABB.min.x, y: tileAABB.min.y, z: tileAABB.min.z },
                    { x: tileAABB.max.x, y: tileAABB.min.y, z: tileAABB.min.z },
                    { x: tileAABB.min.x, y: tileAABB.max.y, z: tileAABB.min.z },
                    { x: tileAABB.max.x, y: tileAABB.max.y, z: tileAABB.min.z },
                    { x: tileAABB.min.x, y: tileAABB.min.y, z: tileAABB.max.z },
                    { x: tileAABB.max.x, y: tileAABB.min.y, z: tileAABB.max.z },
                    { x: tileAABB.min.x, y: tileAABB.max.y, z: tileAABB.max.z },
                    { x: tileAABB.max.x, y: tileAABB.max.y, z: tileAABB.max.z }
                );
                ...
                // Fallback for this specific tile if DEM processing fails
                const coordForCatchFallback = tileCoords.find(tc => tc.originalIndex === i);
                const flatPlaneGeomCatch = new THREE.PlaneGeometry(1, 1, 1, 1);
                const flatMeshCatch = new THREE.Mesh(flatPlaneGeomCatch, material);
                if (coordForCatchFallback && coordForCatchFallback.x !== null) {
                    flatMeshCatch.position.set((coordForCatchFallback.x - minX) * 1.0, 0, -(coordForCatchFallback.y - minY) * 1.0);
                } else { // Absolute fallback if no coords
                    flatMeshCatch.position.x = (i % 10) * 1.05; flatMeshCatch.position.y = 0; flatMeshCatch.position.z = Math.floor(i / 10) * 1.05;
                }
                flatMeshCatch.rotation.x = -Math.PI / 2;
                tileMeshesGroup.add(flatMeshCatch);
            }
        }
        // Wait for all DEM processing to settle before proceeding to GLTF export
        if (demPromises.length > 0) {
            console.log(`[3D_ASSETS] Waiting for ${demPromises.length} DEM processing promises to settle.`);
            await Promise.allSettled(demPromises);
        } else {
            console.log(`[3D_ASSETS] No DEM promises to wait for (e.g., if 'flat' preference or all DEM tiles had errors before fetch).`);
        }
    }
