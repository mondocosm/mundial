...
// clickSelectInteraction (ol.interaction.Select) removed in favor of map.on('singleclick', ...)
...
console.log("MODE_BTN_INIT_DEBUG: interactionModeBtn element after getElementById:", interactionModeBtn);
    ...
    let currentInteractionMode = 'pan'; // Initial mode is 'pan' (to match Tessellar)
...
// Removed duplicate/older version of updateInteractionModeUI from here.
// The active and more comprehensive version is defined around line 2012.
    function getTileId(tileCoord) { return `${tileCoord[0]}-${tileCoord[1]}-${tileCoord[2]}`; }

    function toggleTileSelection(tileCoord) {
        console.log(`[toggleTileSelection] Called with tileCoord: ${JSON.stringify(tileCoord)}`);
        ...
        console.log(`[toggleTileSelection] Generated tileId: ${tileId}`);
        const existingFeature = selectionSource.getFeatureById(tileId);
        console.log(`[toggleTileSelection] existingFeature in selectionSource for tileId ${tileId}:`, existingFeature ? existingFeature.getId() : null);
...
function loadTestTilesetToLayer0() {
    ...
    function initializeOpenLayersMap() {
        ...
        if (typeof ol === 'undefined') {
            ...
            return;
// This is the clickSelectHandler logic directly from Tessellar
// It will be attached to map.on('singleclick', ...) and should internally check currentInteractionMode if needed.
const clickSelectHandler = function (evt) {
    // Tessellar's clickSelectHandler doesn't have an explicit mode check at the top,
    // it seems to rely on other interactions (pan/dragbox) consuming events when they are active.
    // For Orbitile, we will add the mode check here for clarity, or ensure the listener is only active in select mode.
    // For now, let's keep the mode check as it was in orbitileMapSingleClickHandler.
    if (currentInteractionMode !== 'select') {
        // console.log('[clickSelectHandler] Ignored: Not in select mode.');
        return;
    }
    console.log(`[clickSelectHandler] Triggered. Mode: ${currentInteractionMode}. Coord:`, evt.coordinate);

    if (!state.olMap) { // Orbitile uses state.olMap
        console.warn("[clickSelectHandler] state.olMap not ready.");
        return;
    }
    const currentZoom = state.olMap.getView().getZoom();
    if (currentZoom < GRID_VISIBILITY_MIN_ZOOM) { // Orbitile uses GRID_VISIBILITY_MIN_ZOOM
        console.log("[clickSelectHandler] Zoom too low, clearing selection.");
        clearMapSelectionAndDetails(); // Orbitile's function
        return;
    }
    ...
    const pixel = state.olMap.getEventPixel(evt.originalEvent);
    // Orbitile uses window.selectedLayerId and window.userLayers
    const targetLayer = window.selectedLayerId && window.userLayers ? window.userLayers[window.selectedLayerId]?.layer : null;

    if (targetLayer) {
        state.olMap.forEachFeatureAtPixel(pixel, (feature, layer) => {
            if (layer === targetLayer) {
                const groupId = feature.get('tilesetGroupId');
                if (groupId) {
                    clickedGroupId = groupId;
                    clickedSavedFeature = feature;
    ...
    if (clickedSavedFeature && clickedGroupId) {
        console.log(`[clickSelectHandler] Clicked saved group ${clickedGroupId}.`);
        // Orbitile uses currentEditingGroupId and tilesetDetailsModal
        if (currentEditingGroupId === clickedGroupId && tilesetDetailsModal.style.display === 'block') {
            console.log(`[clickSelectHandler] Deselecting active group ${clickedGroupId}`);
            clearMapSelectionAndDetails(); // Orbitile's function
            updateSelectedTileCountDisplay(); // Orbitile's function
            return;
        }

        clearMapSelectionAndDetails(); // Orbitile's function
        window.highlightedGlobeGroupId = clickedGroupId; // Orbitile uses this
        ...
        const featuresToAdd = groupFeatures.map(f => {
            const clone = f.clone();
            clone.setId(`selection-${f.getId() || f.ol_uid}`); // Orbitile's ID format
            clone.set('originalTileId', f.get('tileId'));
            clone.set('isGroupSelection', true);
            return clone;
        });

        if (featuresToAdd.length > 0 && selectionSource) { // Orbitile uses selectionSource
            selectionSource.addFeatures(featuresToAdd);
        }
        
        // Orbitile's functions
        if (typeof openTilesetDetailsModal === 'function' && clickedSavedFeature) {
           openTilesetDetailsModal(clickedSavedFeature);
        } else {
            console.warn("[clickSelectHandler] openTilesetDetailsModal function not found or feature missing.");
        }
        if (typeof zoomToTilesetGroup === 'function') {
            zoomToTilesetGroup(clickedGroupId);
        } else {
            console.warn("[clickSelectHandler] zoomToTilesetGroup function not found.");
        }
        if (typeof highlightListItem === 'function') {
            highlightListItem(clickedGroupId);
        } else {
            console.warn("[clickSelectHandler] highlightListItem function not found.");
        }
        if (window.ogSavedTilesetsLayer) window.ogSavedTilesetsLayer.clear();
    } else {
        // Individual tile selection logic
        console.log("[clickSelectHandler] Path for individual tile selection.");
        if (currentEditingGroupId) {
            console.log(`[clickSelectHandler] Deactivating previously detailed group '${currentEditingGroupId}'.`);
            currentEditingGroupId = null;
            if (tilesetDetailsModal) tilesetDetailsModal.style.display = 'none';
            if (typeof unhighlightAllListItems === 'function') unhighlightAllListItems();
            if (selectionSource) {
                ...
                if (groupFeaturesInSelection.length > 0) {
                    console.log("[clickSelectHandler] Clearing group features from selectionSource.");
                    groupFeaturesInSelection.forEach(f => selectionSource.removeFeature(f));
                }
            }
        }
        
        // Orbitile uses selectionTileGrid, TILE_SELECTION_ZOOM, toggleTileSelection
        const tileCoord = selectionTileGrid.getTileCoordForCoordAndZ(coordinate, TILE_SELECTION_ZOOM);
        if (tileCoord && typeof toggleTileSelection === 'function') {
            toggleTileSelection(tileCoord);
        } else {
            console.error("[clickSelectHandler] Could not get tileCoord or toggleTileSelection is undefined.");
        }
    }
    updateSelectedTileCountDisplay(); // Orbitile's function
};
...
function initializeITowns() {
    ...
    if (itownsContainer && !itownsContainer.dataset.initialized) {
        ...
        try {
            ...
            // Tessellar-style interaction setup:
            // Find and store dragPanInteraction
            ...
            // Create and configure dragBoxInteraction (Tessellar logic)
            // isShiftKeyPressed is declared globally in Orbitile (around line 1996)
            ...
            // Set initial active states for 'pan' default (Tessellar style)
            if (dragPanInteraction) {
            ...
            dragBoxInteraction.on('boxstart', function(event) {
                console.log("[Tessellar Logic] DragBox started.");
                if (!window.olMap || !selectionSource) return;
                
                isShiftKeyPressed = ol.events.condition.shiftKeyOnly(event.mapBrowserEvent);
                console.log("[Tessellar Logic] Shift key pressed:", isShiftKeyPressed);
                
                if (!isShiftKeyPressed) {
                    // const isGroupCurrentlySelected = selectionSource.getFeatures().some(f => f.get('isGroupSelection'));
                    // if (isGroupCurrentlySelected || selectionSource.getFeatures().length > 0) { // Simplified condition from Tessellar
                    if (selectionSource.getFeatures().length > 0) { // If anything is selected, clear it if not shift-dragging
                        console.log("[Tessellar Logic] Clearing previous selection (Shift not pressed).");
                        clearMapSelectionAndDetails(); // Orbitile's function
                    }
                }
            });
            ...
            dragBoxInteraction.on('boxend', function() {
                console.log("[Tessellar Logic] DragBox ended.");
                if (!dragBoxInteraction.getGeometry()) {
                    console.warn("[Tessellar Logic] DragBox ended with no geometry.");
                    return;
                }
                const boxExtent = dragBoxInteraction.getGeometry().getExtent();
                if (!state.olMap || !selectionSource || !selectionTileGrid || !window.userLayers || !window.selectedLayerId) {
                    console.warn("[Tessellar Logic] DRAGBOX_BOXEND: Missing critical components, aborting.");
                    return;
                }
                
                const targetLayerId = window.selectedLayerId;
                if (!window.userLayers[targetLayerId] || !window.userLayers[targetLayerId].layer) {
                    console.error(`[Tessellar Logic] DRAGBOX_BOXEND: userLayers[${targetLayerId}] or its layer is undefined.`);
                    return;
                }
                const targetSource = window.userLayers[targetLayerId].layer.getSource();
                if (!targetSource) {
                    console.error(`[Tessellar Logic] DRAGBOX_BOXEND: targetSource for layer ${targetLayerId} is undefined.`);
                    return;
                }
                const existingFeaturesInLayer = targetSource.getFeatures();
                ...
                console.log(`[Tessellar Logic] Added ${tilesAdded} tiles to selection.`);
                updateSelectedTileCountDisplay();
            });
            
            // Attach the clickSelectHandler (defined at line 1600, contains Tessellar's logic)
            state.olMap.on('singleclick', clickSelectHandler);
    ...
    function updateInteractionModeUI(mode) {
        ...
        if (mode === 'select') { // Unified 'select' mode
            if (interactionModeBtn) {
                interactionModeBtn.textContent = 'Mode: Select'; // Button text when Select mode is active
                interactionModeBtn.title = 'Selection active (click or drag). Click to switch to Pan mode. (Shortcut: B)';
                interactionModeBtn.classList.add('active'); // Visually indicate selection mode is active
            }
            if (mapElement) mapElement.style.cursor = 'crosshair'; // Cursor for selection
            console.log("UI updated to: Select Mode");
        } else { // mode === 'pan'
            if (interactionModeBtn) {
                interactionModeBtn.textContent = 'Mode: Pan'; // Button text when Pan mode is active
                interactionModeBtn.title = 'Pan active. Click to switch to Select mode. (Shortcut: B)';
                interactionModeBtn.classList.remove('active'); // Default state for pan
            }
            if (mapElement) mapElement.style.cursor = 'grab'; // Cursor for panning
            console.log("UI updated to: Pan Mode");
        }
    }
...
function setupXRPanelLogic() {
    ...
    if (interactionModeBtn) {
        ...
        currentInteractionMode = 'pan'; // Default to pan mode (to match Tessellar)
        ...
        interactionModeBtn.addEventListener('click', function() {
            ...
            console.log(`[DEBUG_CTRL_BTN] Status: olMap: ${window.olMap ? 'Exists' : 'NULL'}, dragPan: ${dragPanInteraction ? 'Exists' : 'NULL'}, dragBox: ${dragBoxInteraction ? 'Exists' : 'NULL'}`); // clickSelect removed
            if (!window.olMap) {
            ...
            if (currentInteractionMode === 'pan') {
                ...
                // clickSelectInteraction removed, single click handled by map.on('singleclick',...)
                if (dragBoxInteraction) dragBoxInteraction.setActive(true);
                console.log("Switched to Select mode (dragPan: off, dragBox: on). Single click selection active via map event.");
                if (dragPanInteraction && dragBoxInteraction) { // clickSelect removed
                    console.log(`[DEBUG_CTRL_BTN] Post-setActive (Select): dragPan=${dragPanInteraction.getActive()}, dragBox=${dragBoxInteraction.getActive()}`);
                }
            } else { // currentInteractionMode was 'select'
                // Switch to Pan mode
                currentInteractionMode = 'pan';
                // clickSelectInteraction removed
                if (dragBoxInteraction) dragBoxInteraction.setActive(false);
                if (dragPanInteraction) dragPanInteraction.setActive(true);
                console.log("Switched to Pan mode (dragPan: on, dragBox: off). Single click selection inactive.");
                if (dragPanInteraction && dragBoxInteraction ) { // clickSelect removed
                    console.log(`[DEBUG_CTRL_BTN] Post-setActive (Pan): dragPan=${dragPanInteraction.getActive()}, dragBox=${dragBoxInteraction.getActive()}`);
                }
            }
...
async function generate3DAssetsFromTileset(tilesetGroupId, tilesetName, savedFeatures) {
    ...
    const VERTS_PER_TILE_EDGE = SAMPLING_RESOLUTION;
    const TILE_QUAD_SEGMENTS = SAMPLING_RESOLUTION - 1;
    ...
    // --- Phase 2: Composite Canvas and DEM Drawing (REMOVED - Will be done per-tile) ---
    // The large compositeImageData generation is removed to improve scalability.
    // DEM data will be processed for each ZL21 tile individually.
    let compositeImageData = null; // This variable will be repurposed or data handled differently.
    const numTilesX = maxX - minX + 1; // Still useful for overall positioning.
    const numTilesY = maxY - minY + 1; // Still useful for overall positioning.
    console.log("[3D_ASSETS_REFACTOR] Large composite canvas generation skipped for per-tile processing.");

    // --- Phase 3: Single Mesh Generation (using compositeImageData) ---
    tileMeshesGroup.clear();

    // --- Phase 3: Per-Tile Mesh Generation (Refactored for Scalability) ---
    // tileMeshesGroup is already cleared (line 4382)
    console.log("[3D_ASSETS_REFACTOR] Starting per-tile DEM processing and mesh generation.");

    tileInfoList.forEach((tileInfo, idx) => {
        let tileSpecificImageData = null; // Will hold the 256x256 DEM image data for the current ZL21 tile

        // Attempt to prepare tile-specific DEM data if available and not skipping DEM fetching
        if (!skipDemFetching && tileInfo.imageBitmap && !tileInfo.fetchError && tileInfo.heightDecodeFn) {
            const demBitmap = tileInfo.imageBitmap; // This is the (e.g., ZL15) DEM tile
            const zDemActual = tileInfo.zDem;       // Actual zoom of demBitmap
            const zL21 = tileInfo.zLevel;         // Target zoom for the mesh (e.g., 21)

            const tempCanvas = document.createElement('canvas');
            tempCanvas.width = TILE_SIZE; // TILE_SIZE is 256, defined at line 4183
            tempCanvas.height = TILE_SIZE;
            const tempCtx = tempCanvas.getContext('2d');

            if (tempCtx) {
                let sx = 0, sy = 0, sWidth = demBitmap.width, sHeight = demBitmap.height;
                // If the fetched DEM (zDemActual) is lower zoom than the target ZL21 tile (zL21), calculate sub-rectangle
                if (zL21 > zDemActual) {
                    const scaleFactor = 2 ** (zL21 - zDemActual);
                    // tileInfo.x and tileInfo.y are ZL21 coordinates
                    const xOffsetInParent = tileInfo.x % scaleFactor; // x-offset of ZL21 tile within its ZL-demActual parent
                    const yOffsetInParent = tileInfo.y % scaleFactor; // y-offset of ZL21 tile within its ZL-demActual parent
                    
                    sWidth = demBitmap.width / scaleFactor;   // width of the ZL21-equivalent portion in the ZL-demActual image
                    sHeight = demBitmap.height / scaleFactor; // height of the ZL21-equivalent portion in the ZL-demActual image
                    
                    sx = xOffsetInParent * sWidth;  // sourceX in the demBitmap
                    sy = yOffsetInParent * sHeight; // sourceY in the demBitmap
                }
                // Draw (and scale if necessary) the relevant part of demBitmap to the 256x256 tempCanvas
                tempCtx.drawImage(demBitmap, sx, sy, sWidth, sHeight, 0, 0, TILE_SIZE, TILE_SIZE);
                
                try {
                    tileSpecificImageData = tempCtx.getImageData(0, 0, TILE_SIZE, TILE_SIZE);
                } catch (e) {
                    console.error(`[3D_ASSETS_REFACTOR] Error getting ImageData from temp DEM canvas for tile ${idx} (X:${tileInfo.x}, Y:${tileInfo.y}):`, e);
                    tileSpecificImageData = null; // Ensure it's null on error
                }
            } else {
                console.error(`[3D_ASSETS_REFACTOR] Could not get 2D context for temp DEM canvas for tile ${idx} (X:${tileInfo.x}, Y:${tileInfo.y}).`);
            }
        }

        // Log processing details for the current tile
        console.log(`[3D_ASSETS_QUAD_MESH_DETAIL] Processing tileInfo[${idx}]: X=${tileInfo.x}, Y=${tileInfo.y}, ZL=${tileInfo.zLevel}, relX=${tileInfo.relX}, relY=${tileInfo.relY}, DEM available=${!!tileSpecificImageData}`);

        if (tileInfo.x === null || tileInfo.y === null) {
            console.warn(`[3D_ASSETS_QUAD_MESH_DETAIL] Skipping mesh for tile index ${tileInfo.originalIndex} due to null X/Y coordinates.`);
        ...
        const tileUvs = [];
        const tileIndices = [];
        // Use the tile's specific heightDecodeFn, or fallback to a function that returns 0 if none (e.g., for flat tiles or errors)
        const decodeFnToUse = tileInfo.heightDecodeFn || ((r,g,b) => 0);
        ...
        for (let j_vert = 0; j_vert < VERTS_PER_TILE_EDGE; j_vert++) {
            for (let i_vert = 0; i_vert < VERTS_PER_TILE_EDGE; i_vert++) {
                ...
                if (tileSpecificImageData) { // Only attempt to decode height if we have image data for this tile
                    // Sample from the 256x256 tileSpecificImageData
                    const cImgX = Math.min(Math.floor(u_local * (TILE_SIZE - 1)), TILE_SIZE - 1);
                    const cImgY = Math.min(Math.floor(v_local * (TILE_SIZE - 1)), TILE_SIZE - 1);
                    
                    const rIndex = (cImgY * tileSpecificImageData.width + cImgX) * 4; // tileSpecificImageData.width is TILE_SIZE
                    const rVal = tileSpecificImageData.data[rIndex];
                    const gVal = tileSpecificImageData.data[rIndex + 1];
                    const bVal = tileSpecificImageData.data[rIndex + 2];
                    height = decodeFnToUse(rVal, gVal, bVal);

                    const MAX_EXPECTED_HEIGHT = 9000;
                    const MIN_EXPECTED_HEIGHT = -11000;
                    if (height > MAX_EXPECTED_HEIGHT) {
                         console.warn(`[3D_ASSETS_QUAD_SPIKE] Tile ${tileInfo.originalIndex} (X:${tileInfo.x},Y:${tileInfo.y}) vert(${i_vert},${j_vert}): Height ${height} > MAX_EXPECTED_HEIGHT (${MAX_EXPECTED_HEIGHT}). Clamped to MAX.`);
                         height = MAX_EXPECTED_HEIGHT;
                    } else if (height < MIN_EXPECTED_HEIGHT) {
                         console.warn(`[3D_ASSETS_QUAD_SPIKE] Tile ${tileInfo.originalIndex} (X:${tileInfo.x},Y:${tileInfo.y}) vert(${i_vert},${j_vert}): Height ${height} < MIN_EXPECTED_HEIGHT (${MIN_EXPECTED_HEIGHT}). Clamped to MIN.`);
                         height = MIN_EXPECTED_HEIGHT;
                    }
                }
                // If tileSpecificImageData was null, height remains 0, resulting in a flat mesh for this tile.
                tileVertices.push(u_local - 0.5, height * MESH_HEIGHT_SCALE, v_local - 0.5); // v_local for Z
                tileUvs.push(u_local, 1.0 - v_local); // Invert v_local for UVs
            }
        }
        ...
        tileMesh.position.set(tileInfo.relX, 0, tileInfo.relY); // Use relX, relY for positioning in the group
        tileMeshesGroup.add(tileMesh);

        const edges = new THREE.EdgesGeometry(individualTileGeometry);
        const perimeterLineMaterial = new THREE.LineBasicMaterial({ color: material.color });
        const perimeterLines = new THREE.LineSegments(edges, perimeterLineMaterial);
        perimeterLines.position.set(tileInfo.relX, 0, tileInfo.relY);
        tileMeshesGroup.add(perimeterLines);
    }); // End of tileInfoList.forEach
    allPoints = [];
