...
async function exportTilesetToGLTF(
    ...
    console.log(`[EXPORTER_ENTRY] Exporting tileset "${tilesetName}". Mode: ${dataSourceType}. Target: ${target}. Options:`, JSON.stringify(options));
    console.log("[EXPORTER_ENTRY] SelectionCriteria:", JSON.stringify(selectionCriteria).substring(0, 500) + (JSON.stringify(selectionCriteria).length > 500 ? "..." : ""));


    if (!window.MyStandaloneTHREE) { // Check for our sandboxed THREE
        console.error("MyStandaloneTHREE.js is not loaded.");
        if (onComplete) onComplete({ error: "MyStandaloneTHREE.js missing." });
        return;
    }
    // Use MyStandaloneTHREE throughout this exporter
    const THREE = window.MyStandaloneTHREE;

    if (!window.THREE) { // This is a fallback or an old check, MyStandaloneTHREE is preferred
    ...
    if (dataSourceType === '3dtiles') {
        ...
        console.log(`[EXPORTER_3DTILES] Attempting to extract from ${selectedTilesData.length} selected 3D tile contents. First tile contentUrl: ${selectedTilesData[0]?.contentUrl}`);
        try {
            const loader = new THREE.GLTFLoader(); // Assumes GLTFLoader is on MyStandaloneTHREE
            for (const tileInfo of selectedTilesData) {
                ...
                if (glbBuffer) {
                    try {
                        ...
                        console.log(`[EXPORTER_3DTILES] Successfully parsed and added content from ${tileInfo.contentUrl}`);
                    } catch (parseErr) {
            ...
            console.log(`[EXPORTER_3DTILES_SUCCESS] Successfully combined ${combinedModel.children.length} tile scenes.`);
        ...
        console.log(`[EXPORTER_HEIGHTMAP] Generating model for "${tilesetName}" using heightmap method. Number of tiles: ${selectionCriteria?.length}`);
        ...
        try {
            for (let i = 0; i < tilesWithBoundsAndCoords.length; i++) {
                ...
                console.log(`[EXPORTER_HEIGHTMAP_TEXTURE] Tile Z${z}X${x}Y${y}: Texture URL: ${currentTextureUrl}, Loaded: ${!!textureImage}`);


                let heightmap = null;
                if (cesiumScene && geographicBounds) { // cesiumScene from options
                    console.log(`[EXPORTER_HEIGHTMAP_CESIUM] Attempting Cesium height sampling for Z${z}X${x}Y${y}. Bounds:`, JSON.stringify(geographicBounds));
                    heightmap = await sampleHeightsFromCesiumScene(geographicBounds, cesiumScene, TERRAIN_MESH_SEGMENTS + 1, TERRAIN_MESH_SEGMENTS + 1);
                     console.log(`[EXPORTER_HEIGHTMAP_CESIUM_RESULT] Cesium sampling for Z${z}X${x}Y${y}: ${heightmap ? 'Success (' + heightmap.length + 'x' + (heightmap[0] ? heightmap[0].length : 0) + ')' : 'Failed/Null'}`);
                }
                if (!heightmap && terrainUrlTemplate) {
                    ...
                    console.log(`[EXPORTER_HEIGHTMAP_FETCH] Attempting fallback terrain fetch for Z${z}X${x}Y${y} from URL: ${currentTerrainUrl}`);
                    heightmap = await fetchTerrainTileWithFallback([z,x,y], terrainUrlTemplate);
                    console.log(`[EXPORTER_HEIGHTMAP_FETCH_RESULT] Fallback terrain fetch for Z${z}X${x}Y${y}: ${heightmap ? 'Success (' + heightmap.length + 'x' + (heightmap[0] ? heightmap[0].length : 0) + ')' : 'Failed/Null'}`);
                }
                
                console.log(`[EXPORTER_HEIGHTMAP_MESHGEN] Generating mesh for Z${z}X${x}Y${y}. Heightmap available: ${!!heightmap}, Texture available: ${!!textureImage}`);
            ...
            gltfExporter.parse(
                ...
                function (gltf) {
                    ...
                    console.log(`[EXPORTER_HEIGHTMAP_SUCCESS] Successfully generated GLTF for "${tilesetName}". Output length: ${output?.length}`);
