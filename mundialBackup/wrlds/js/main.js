import * as og from '/packages/openglobus/lib/@openglobus/og.esm.js';
// import { TilesRenderer } from '/mundial/libs/3DTilesRendererJS/3DTilesRendererJS/dist/three-loader.module.js'; // Temporarily commented out for debugging site load
console.log("%%%%% MAIN.JS VERSION: STATE_OLCS_V4 - TIMESTAMP: 2025-05-20 04:40 AM %%%%%");
...
let globePanelOlcesiumInstance = null; // For the main OLCesium globe view
let isGlobePanelOLCesiumInitialized = false;
...
const state = {
    ...
    activeGlobeLibrary: 'openglobus', // 'openglobus', 'itowns', or 'olcesium'
    ...
    threeJsViewer: {
        ...
        container: null,
        currentModel: null, // To keep track of the currently loaded model (e.g. from GLTFExporter path)
        tilesRenderer: null, // For NASA AMMOS 3DTilesRendererJS
        currentTilesetUrl: null // To track the currently loaded 3D Tileset URL
    }
};
...
const updateCesiumZL21Grid = function(scene) {
    if (!scene || typeof Cesium === 'undefined') return;

    if (!cesiumGridDataSource) {
        if (scene && scene.dataSources && typeof scene.dataSources.add === 'function') {
            cesiumGridDataSource = new Cesium.CustomDataSource('zl21Grid');
            scene.dataSources.add(cesiumGridDataSource);
            console.log("UPDATE_CESIUM_GRID: cesiumGridDataSource created and added to scene.");
        } else {
            console.error("UPDATE_CESIUM_GRID: scene.dataSources or scene.dataSources.add is not available. Cannot create/add cesiumGridDataSource.");
            return; // Cannot proceed without the data source
        }
    }
    // Ensure cesiumGridDataSource was successfully created and added before trying to use its entities
    if (!cesiumGridDataSource || !cesiumGridDataSource.entities) {
        console.error("UPDATE_CESIUM_GRID: cesiumGridDataSource or its entities collection is not available. Aborting grid update.");
        return;
    }
    cesiumGridDataSource.entities.removeAll();

    const viewRect = scene.camera.computeViewRectangle();
    if (!viewRect) return; // Not looking at the globe

    const olTileGrid = state.selectionTileGrid || ol.tilegrid.createXYZ({ maxZoom: TILE_SELECTION_ZOOM, tileSize: 256 });
    if (!olTileGrid) {
        console.warn("updateCesiumZL21Grid: selectionTileGrid not available.");
        return;
    }

    const ZL = TILE_SELECTION_ZOOM; // Target zoom level for the grid

    // Determine the range of tiles to check based on the view rectangle
    // This is a simplified approach; a more robust method would use olTileGrid.forEachTileCoordParentMayCoverExtent
    ...
    // Get tile ranges for the ZL
    // Note: OpenLayers tile coordinates [z, x, y] where y increases downwards.
    // Cesium rectangle uses geographic coordinates.
    
    // This is a very broad check, can be optimized significantly.
    // For a proper implementation, we'd iterate through tiles at ZL that intersect the viewRect.
    // olTileGrid.getTileRangeForExtentAndZ might be useful if we convert Cesium extent to OL projection.

    // Simplified: Iterate over a large area around the center if performance allows,
    // or ideally, get the tile range from the OpenLayers tileGrid for the current Cesium extent.
    // For now, this will be a placeholder for a more efficient tile iteration.
    // Let's assume we have a function that gives us relevant [x,y] for ZL in view (placeholder)
    
    // Placeholder: For demonstration, let's just draw a few grid cells if the camera is close enough.
    // A real implementation needs to calculate visible ZL21 tiles.
    const cameraHeight = scene.camera.positionCartographic.height;
    if (cameraHeight > 200000) { // Only draw grid if reasonably zoomed in (e.g., < 200km)
         // console.log("Cesium camera too high for ZL21 grid display: ", cameraHeight);
        return;
    }

    // Example: Get center tile coord and draw a small grid around it
    ...
    try {
        const centerTileCoord = olTileGrid.getTileCoordForCoordAndZ(ol.proj.fromLonLat([centerLon, centerLat]), ZL);
        const [tileZ, tileX, tileY] = centerTileCoord;

        const range = 2; // Draw a 5x5 grid around the center tile
        for (let dx = -range; dx <= range; dx++) {
            for (let dy = -range; dy <= range; dy++) {
                const currentTileX = tileX + dx;
                const currentTileY = tileY + dy;
                const tileExtentOL = olTileGrid.getTileCoordExtent([ZL, currentTileX, currentTileY]);
                const tileExtentDegrees = ol.proj.transformExtent(tileExtentOL, 'EPSG:3857', 'EPSG:4326');
                
                cesiumGridDataSource.entities.add({
                    rectangle: {
                        coordinates: Cesium.Rectangle.fromDegrees(tileExtentDegrees[0], tileExtentDegrees[1], tileExtentDegrees[2], tileExtentDegrees[3]),
                        material: Cesium.Color.WHITE.withAlpha(0.1),
                        outline: true,
                        outlineColor: Cesium.Color.WHITE.withAlpha(0.3),
                        outlineWidth: 1,
                        height: scene.globe.getHeight(Cesium.Cartographic.fromDegrees(tileExtentDegrees[0], tileExtentDegrees[1])) || 0 // Drape on terrain
                    }
                });
            }
        }
    } catch (e) {
        // console.warn("Error calculating or drawing center tile grid for Cesium:", e);
        // This can happen if ol.proj or other OL functions are not available when Cesium is active without OL map.
    }
    // console.log("updateCesiumZL21Grid executed, entities:", cesiumGridDataSource.entities.values.length);
};
// --- THREE.js Viewer Modal Functions ---

function initThreeJsViewer() {
    if (state.threeJsViewer.isInitialized) return;

    const container = document.getElementById('threejs-viewer-container');
    if (!container) {
        console.error("THREE.js viewer container 'threejs-viewer-container' not found.");
        state.threeJsViewer.isInitialized = false; // Explicitly mark as not initialized
        return;
    }
    ...
    try {
        ...
        state.threeJsViewer.scene.background = new THREE.Color(0x2c3e50);

        state.threeJsViewer.camera = new THREE.PerspectiveCamera(75, container.clientWidth / container.clientHeight, 0.1, 10000);
        state.threeJsViewer.camera.position.set(100, 100, 100);
        state.threeJsViewer.camera.lookAt(0, 0, 0);

        state.threeJsViewer.renderer = new THREE.WebGLRenderer({ antialias: true });
        state.threeJsViewer.renderer.setSize(container.clientWidth, container.clientHeight);
        ...
        // Add a simple test cube at the origin for debugging
        const testCubeGeometry = new THREE.BoxGeometry(10, 10, 10);
        const testCubeMaterial = new THREE.MeshStandardMaterial({ color: 0x00ff00 }); // Bright green
        const testCube = new THREE.Mesh(testCubeGeometry, testCubeMaterial);
        testCube.position.set(0,0,0);
        state.threeJsViewer.scene.add(testCube);
        console.log("THREE.js viewer: Added AxesHelper and a green test cube at origin.");
        ...
        if (typeof THREE.OrbitControls === 'function') {
            ...
            state.threeJsViewer.controls.dampingFactor = 0.05;
            state.threeJsViewer.controls.screenSpacePanning = true; // Allow more flexible panning
            state.threeJsViewer.controls.minDistance = 1; // Allow closer zoom
            state.threeJsViewer.controls.maxDistance = 8000; // Allow further zoom
        } else {
            console.warn("THREE.OrbitControls not found. Viewer will not have orbit controls.");
        }

        function animateViewer() {
            state.threeJsViewer.animationFrameId = requestAnimationFrame(animateViewer);
            if (state.threeJsViewer.controls) {
                state.threeJsViewer.controls.update();
            }
            // Update 3D Tiles Renderer - Temporarily Commented Out
            // if (state.threeJsViewer.tilesRenderer && state.threeJsViewer.tilesRenderer.update) {
            //     state.threeJsViewer.tilesRenderer.update();
            // }
        ...
        // The original call to animateViewer() was here, it should be after TilesRenderer init.
        // It's now called after the TilesRenderer init block.

        // Initialize 3D Tiles Renderer - Temporarily Commented Out to debug site load
        // console.log("[DEBUG_3DTILES] Top of TilesRenderer init block. typeof TilesRenderer:", typeof TilesRenderer);
        // if (typeof TilesRenderer !== 'undefined' && TilesRenderer) {
        //     try {
        //         state.threeJsViewer.tilesRenderer = new TilesRenderer();
        //         state.threeJsViewer.tilesRenderer.setCamera(state.threeJsViewer.camera);
        //         state.threeJsViewer.tilesRenderer.setResolutionFromRenderer(state.threeJsViewer.camera, state.threeJsViewer.renderer);
        //         state.threeJsViewer.scene.add(state.threeJsViewer.tilesRenderer.group);
        //         console.log("3DTilesRendererJS: TilesRenderer initialized and group added to scene.");
        //     } catch (e) {
        //         console.error("3DTilesRendererJS: Error initializing TilesRenderer:", e);
        //         state.threeJsViewer.tilesRenderer = null;
        //     }
        // } else {
        //     console.warn("3DTilesRendererJS: TilesRenderer class not found or import failed. 3D Tiles loading will not be available.");
        //     state.threeJsViewer.tilesRenderer = null;
        // }
animateViewer(); // Start the animation loop after all initializations in initThreeJsViewer


        window.addEventListener('resize', () => {
            if (state.threeJsViewer.isInitialized && state.threeJsViewer.container && state.threeJsViewer.container.offsetParent !== null) {
                const newWidth = state.threeJsViewer.container.clientWidth;
                const newHeight = state.threeJsViewer.container.clientHeight;
                if (newWidth > 0 && newHeight > 0) {
                    state.threeJsViewer.camera.aspect = newWidth / newHeight;
                    state.threeJsViewer.camera.updateProjectionMatrix();
                    state.threeJsViewer.renderer.setSize(newWidth, newHeight);
                }
            }
        }, false);
        ...
        console.error("Error during THREE.js viewer initialization:", error);
        state.threeJsViewer.isInitialized = false;
        if (container && state.threeJsViewer.renderer && state.threeJsViewer.renderer.domElement) {
            // Attempt to clean up if renderer was partially added
            try {
                container.removeChild(state.threeJsViewer.renderer.domElement);
            } catch (cleanupError) {
                // ignore cleanup error
            }
        }
        // Reset state
        state.threeJsViewer.renderer = null;
        state.threeJsViewer.scene = null;
        state.threeJsViewer.camera = null;
        state.threeJsViewer.controls = null;
        if (state.threeJsViewer.animationFrameId) {
            cancelAnimationFrame(state.threeJsViewer.animationFrameId);
            state.threeJsViewer.animationFrameId = null;
        }
    }
}

function showThreeJsViewerModal(title = "3D View") {
    const modal = document.getElementById('threejs-viewer-modal');
    const titleElement = document.getElementById('threejs-viewer-title');
    const scenePanel = document.getElementById('scene-panel');

    if (!modal || !titleElement) {
        console.error("THREE.js viewer modal elements not found.");
        return;
    }

    // Ensure the parent scene panel is visible and styled
    const mainToolbarSceneBtn = document.getElementById('scene-btn'); // Get the main toolbar button

    if (scenePanel) {
        if (scenePanel.style.display === 'none' || !scenePanel.style.display) {
            console.log("[SHOW_3D_MODAL] Scene panel is hidden, making it visible.");
            scenePanel.style.display = 'block'; // Let CSS handle centering and size
            if (mainToolbarSceneBtn && !mainToolbarSceneBtn.classList.contains('active')) {
                 mainToolbarSceneBtn.classList.add('active');
                 console.log("[SHOW_3D_MODAL] Added 'active' class to main #scene-btn.");
            }
        } else {
            console.log("[SHOW_3D_MODAL] Scene panel already visible.");
        }
        // Centering and sizing are now handled by CSS rules for #scene-panel
    } else {
        console.error("[SHOW_3D_MODAL] CRITICAL: #scene-panel not found. Cannot show 3D viewer.");
        alert("Scene panel container is missing. Cannot display 3D view.");
        return; // Stop if the main container for the viewer is missing
    }

    if (!state.threeJsViewer.isInitialized) {
        console.log("showThreeJsViewerModal: Initializing viewer first.");
        initThreeJsViewer();
        if (!state.threeJsViewer.isInitialized) {
            alert("Failed to initialize 3D viewer. Check console for errors.");
            return; 
        }
    }
    ...
    setTimeout(() => {
        if (state.threeJsViewer.isInitialized && state.threeJsViewer.container) {
            const newWidth = state.threeJsViewer.container.clientWidth;
            const newHeight = state.threeJsViewer.container.clientHeight;
            if (newWidth > 0 && newHeight > 0 && state.threeJsViewer.camera && state.threeJsViewer.renderer) {
                 state.threeJsViewer.camera.aspect = newWidth / newHeight;
                 state.threeJsViewer.camera.updateProjectionMatrix();
                 state.threeJsViewer.renderer.setSize(newWidth, newHeight);
            } else if (newWidth === 0 || newHeight === 0) {
                console.warn("THREE.js viewer container has zero width or height after modal display.");
            }
        }
    }, 50);
}

function hideThreeJsViewerModal() {
    const modal = document.getElementById('threejs-viewer-modal');
    if (modal) {
        modal.style.display = 'none';
    }
    // Optionally stop animation loop if it's only for when modal is visible
    // if (state.threeJsViewer.animationFrameId) {
    //     cancelAnimationFrame(state.threeJsViewer.animationFrameId);
    //     state.threeJsViewer.animationFrameId = null;
    // }
...
function loadSceneObjectInViewer(sceneObject) {
    if (!state.threeJsViewer.isInitialized || !state.threeJsViewer.scene || !state.threeJsViewer.camera || !state.threeJsViewer.renderer) {
        console.error("THREE.js viewer not initialized properly. Cannot load model.");
        alert("3D Viewer is not ready. Please try again or check console.");
        return;
    }
    ...
    if (state.threeJsViewer.currentModel) {
        ...
        // Proper disposal of old model's geometries and materials would go here
        // For example, if currentModel is a THREE.Group with meshes:
        // state.threeJsViewer.currentModel.traverse(child => {
        //     if (child.isMesh) {
        ...
        //         }
        //     }
// --- NASA AMMOS 3DTilesRendererJS Integration ---
async function loadTilesetWith3DTilesRenderer(tilesetUrl) {
    if (!state.threeJsViewer.isInitialized) {
        console.log("[3DTILES_LOADER] Viewer not initialized, attempting to initialize first.");
        showThreeJsViewerModal("3D View - Initializing for 3D Tiles..."); // This will call initThreeJsViewer
        if (!state.threeJsViewer.isInitialized) {
            console.error("[3DTILES_LOADER] Failed to initialize 3D viewer. Cannot load 3D Tileset.");
            alert("3D Viewer could not be initialized. Cannot load 3D Tileset.");
            return;
        }
    }
    
    // Ensure TilesRenderer itself is initialized (it's done in initThreeJsViewer)
    if (!state.threeJsViewer.tilesRenderer) {
        console.error("[3DTILES_LOADER] 3DTilesRendererJS (tilesRenderer) not available. Cannot load 3D Tileset. Was the library imported correctly?");
        showThreeJsViewerModal("3D View - Error"); 
        alert("3D Tiles rendering subsystem not available. Check console.");
        return;
    }

    // Ensure scene panel is visible
    showThreeJsViewerModal(`3D View - Loading: ${tilesetUrl.split('/').pop()}`);

    console.log(`[3DTILES_LOADER] Attempting to load 3D Tileset: ${tilesetUrl}`);
    document.getElementById('threejs-viewer-title').textContent = `Loading: ${tilesetUrl.substring(tilesetUrl.lastIndexOf('/') + 1)}`;

    try {
        // Clear any previously loaded non-3DTiles model from our custom logic
        if (state.threeJsViewer.currentModel && state.threeJsViewer.currentModel !== state.threeJsViewer.tilesRenderer.group) {
            console.log("[3DTILES_LOADER] Removing previous non-3DTiles model from scene.");
            state.threeJsViewer.scene.remove(state.threeJsViewer.currentModel);
            // Add proper disposal here if state.threeJsViewer.currentModel has disposable resources
        }
        // The TilesRenderer group is now considered the current model container
        state.threeJsViewer.currentModel = state.threeJsViewer.tilesRenderer.group; 

        // Check if we are loading a new tileset or the same one
        if (state.threeJsViewer.currentTilesetUrl === tilesetUrl && state.threeJsViewer.tilesRenderer.roots.length > 0) {
            console.log(`[3DTILES_LOADER] Tileset ${tilesetUrl} is already loaded or loading. Focusing camera.`);
            // Potentially just re-focus camera if already loaded, or do nothing if it's still loading.
            // For now, we'll let it proceed; setTilesetUrl might handle this.
        }

        // Dispose of previous tileset if any; the library's setTilesetUrl should handle this.
        // If not, one might need: if (state.threeJsViewer.tilesRenderer.roots.length) { state.threeJsViewer.tilesRenderer.dispose(); }
        // Forcing a dispose and re-creation of the renderer if changing URL, to be safe, though setTilesetUrl should be enough.
        // This is aggressive, might not be needed if setTilesetUrl handles cleanup.
        // if (state.threeJsViewer.currentTilesetUrl && state.threeJsViewer.currentTilesetUrl !== tilesetUrl) {
        //     console.log("[3DTILES_LOADER] Disposing previous tileset before loading new one.");
        //     state.threeJsViewer.tilesRenderer.dispose(); // Dispose old tileset
        // }


        await state.threeJsViewer.tilesRenderer.setTilesetUrl(tilesetUrl);
        state.threeJsViewer.currentTilesetUrl = tilesetUrl; // Track current URL
        console.log(`[3DTILES_LOADER] TilesRenderer started loading: ${tilesetUrl}`);

        // Wait for the tileset to load to get its bounding box for camera adjustment
        // This requires listening to an event from the library.
        // Example: tilesRenderer.onLoadTileset = (tileset) => { ... }
        // For now, we'll rely on manual navigation or a generic camera position.
        // A simple camera adjustment:
        if (state.threeJsViewer.tilesRenderer.getBoundingBox(new THREE.Box3())) {
            const box = new THREE.Box3();
            state.threeJsViewer.tilesRenderer.getBoundingBox(box);
            const center = box.getCenter(new THREE.Vector3());
            const size = box.getSize(new THREE.Vector3());
            const maxDim = Math.max(size.x, size.y, size.z);
            const fov = state.threeJsViewer.camera.fov * (Math.PI / 180);
            let cameraDistance = maxDim / (2 * Math.tan(fov / 2));
            cameraDistance = Math.max(cameraDistance, maxDim); // Ensure some distance

            state.threeJsViewer.camera.position.copy(center);
            // A common strategy is to position camera along one axis or diagonally
            state.threeJsViewer.camera.position.x += cameraDistance;
            state.threeJsViewer.camera.position.y += cameraDistance;
            state.threeJsViewer.camera.position.z += cameraDistance;
            state.threeJsViewer.camera.lookAt(center);
            if (state.threeJsViewer.controls) {
                state.threeJsViewer.controls.target.copy(center);
                state.threeJsViewer.controls.update();
            }
            console.log(`[3DTILES_LOADER] Camera adjusted to tileset bounds. Center: ${center.x.toFixed(2)},${center.y.toFixed(2)},${center.z.toFixed(2)}. Distance factor: ${cameraDistance.toFixed(2)}`);
        } else {
            console.warn("[3DTILES_LOADER] Could not get bounding box after load. Manual camera adjustment needed.");
            state.threeJsViewer.camera.position.set(100,100,100); // Default fallback
            state.threeJsViewer.camera.lookAt(0,0,0);
            if (state.threeJsViewer.controls) state.threeJsViewer.controls.target.set(0,0,0);
        }
        document.getElementById('threejs-viewer-title').textContent = `3D View: ${tilesetUrl.substring(tilesetUrl.lastIndexOf('/') + 1)}`;


    } catch (error) {
        console.error(`[3DTILES_LOADER] Error in loadTilesetWith3DTilesRenderer for ${tilesetUrl}:`, error);
        alert(`Failed to load 3D Tileset: ${error.message || error}`);
        document.getElementById('threejs-viewer-title').textContent = `Error loading tileset`;
    }
}
// --- End NASA AMMOS 3DTilesRendererJS Integration ---
        // });
    }
...
// --- Function to process a tileset.json and convert its contents to a combined GLTF ---
async function initiateTilesetConversionToGltf(tilesetJsonUrl) {
    ...
    const updateStatus = (message, show = true) => {
        ...
        console.log(`[TILESET_CONVERT_STATUS] ${message}`);
    };

    updateStatus(`Starting conversion for: ${tilesetJsonUrl.split('/').pop()}`);
    showThreeJsViewerModal(`Converting: ${tilesetJsonUrl.split('/').pop()}`); // Open panel

    if (!window.exportTilesetToGLTF || typeof THREE === 'undefined' || !THREE.GLTFLoader || !THREE.GLTFExporter) {
        updateStatus("Error: Conversion components missing.", true);
        console.error("[TILESET_CONVERT] Missing critical THREE.js components (GLTFLoader, GLTFExporter) or exportTilesetToGLTF function.");
        alert("Error: Required components for tileset conversion are missing.");
        return;
    }

    try {
        updateStatus(`Fetching tileset.json...`);
        ...
        function processTile(tile, parentTransformMatrix) {
            ...
            if (tile.content && tile.content.uri) {
                ...
                // Resolve relative URI. This is a simplified resolver.
                // A proper resolver would handle various URI types.
                if (!contentUrl.startsWith('http://') && !contentUrl.startsWith('https://') && !contentUrl.startsWith('data:')) {
                    contentUrl = baseUrl + contentUrl;
                }
                
                // Check if it's a GLB or B3DM (needs B3DM parser for proper handling, GLB is simpler)
                if (contentUrl.toLowerCase().endsWith('.glb')) {
                    selectionCriteria.push({
                        contentUrl: contentUrl,
                        transformMatrix: currentTransformMatrix.clone() // Important to clone
                    });
                    console.log(`[TILESET_CONVERT] Added GLB to criteria: ${contentUrl}`);
                } else {
                    console.warn(`[TILESET_CONVERT] Skipping non-GLB content: ${contentUrl}`);
                }
            } else if (tile.content && tile.content.url) { // Some tilesets use 'url'
                 let contentUrl = tile.content.url;
                if (!contentUrl.startsWith('http://') && !contentUrl.startsWith('https://') && !contentUrl.startsWith('data:')) {
                    contentUrl = baseUrl + contentUrl;
                }
                if (contentUrl.toLowerCase().endsWith('.glb')) {
                    selectionCriteria.push({
                        contentUrl: contentUrl,
                        transformMatrix: currentTransformMatrix.clone()
                    });
                     console.log(`[TILESET_CONVERT] Added GLB (from .url) to criteria: ${contentUrl}`);
                } else {
                    console.warn(`[TILESET_CONVERT] Skipping non-GLB content (from .url): ${contentUrl}`);
                }
            }


            if (tile.children) {
                tile.children.forEach(child => processTile(child, currentTransformMatrix));
            }
        }

        if (tilesetData.root) {
            const rootTransform = new THREE.Matrix4(); // Identity for the root unless tileset has a global transform
            // Note: A tileset can have a global tileset.asset.gltfUpAxis and a root.transform.
            // For simplicity, this example assumes standard Y-up and applies root.transform.
            processTile(tilesetData.root, rootTransform);
        } else {
            console.warn("[TILESET_CONVERT] Tileset has no root tile.");
        }

        if (selectionCriteria.length === 0) {
            alert("No convertible GLB content found in the provided tileset.json.");
            console.warn("[TILESET_CONVERT] No GLB files found in tileset to process.");
            document.getElementById('threejs-viewer-title').textContent = `No GLBs in Tileset`;
            return;
        }

        console.log("[TILESET_CONVERT] Generated selectionCriteria for exportTilesetToGLTF:", selectionCriteria);

        const exportOptions = {
            tilesetName: tilesetJsonUrl.split('/').pop().replace('.tileset.json', '') || 'ConvertedTileset',
            dataSourceType: '3dtiles', // Use the direct GLB processing path
            onComplete: ({ sceneObject, error }) => {
                if (error) {
                    updateStatus(`Error during GLB merging: ${error.message}`, true);
                    console.error("[TILESET_CONVERT] Error during exportTilesetToGLTF from tileset.json:", error);
                    // alert(`Error converting tileset: ${error.message}`); // Status bar is used
                    document.getElementById('threejs-viewer-title').textContent = `Conversion Error`;
                } else if (sceneObject) {
                    updateStatus(`GLB components merged. Loading into viewer...`, true);
                    console.log("[TILESET_CONVERT] Successfully processed tileset.json into combined sceneObject.");
                    loadSceneObjectInViewer(sceneObject); // Load the combined model
                    document.getElementById('threejs-viewer-title').textContent = `View: ${exportOptions.tilesetName}`;
                    window.currentSceneObjectForDownload = sceneObject;
                    window.currentGltfJsonForDownload = null;
                    window.currentGltfFilename = exportOptions.tilesetName + "_merged.gltf";
                    updateStatus(`Model "${exportOptions.tilesetName}" ready.`, true);
                    // Optionally hide status bar after a delay: setTimeout(() => updateStatus('', false), 3000);
                } else {
                    updateStatus(`Conversion completed but no model produced.`, true);
                    console.warn("[TILESET_CONVERT] exportTilesetToGLTF completed but no sceneObject returned.");
                    // alert("Tileset conversion completed but no viewable model was produced.");
                    document.getElementById('threejs-viewer-title').textContent = `Empty Result`;
                }
            }
        };
        updateStatus(`Merging ${selectionCriteria.length} GLB component(s)...`);
        window.exportTilesetToGLTF(selectionCriteria, exportOptions);

    } catch (error) {
        updateStatus(`Error processing tileset.json: ${error.message}`, true);
        console.error(`[TILESET_CONVERT] Error processing tileset.json from ${tilesetJsonUrl}:`, error);
        // alert(`Failed to process tileset.json: ${error.message}`);
        document.getElementById('threejs-viewer-title').textContent = `Error`;
    }
}
    try {
        ...
        if (size.x === 0 && size.y === 0 && size.z === 0) { // Handle empty or point-like objects
             console.warn("Loaded scene object has zero size. Camera may not focus well.");
             state.threeJsViewer.camera.position.set(0, 0, 10); // Default fallback position
             if (state.threeJsViewer.controls) state.threeJsViewer.controls.target.set(0,0,0); else state.threeJsViewer.camera.lookAt(0,0,0);
        } else {
            const maxDim = Math.max(size.x, size.y, size.z);
            const fov = state.threeJsViewer.camera.fov * (Math.PI / 180);
            ...
            state.threeJsViewer.camera.position.copy(center);
            state.threeJsViewer.camera.position.z += cameraDistance * 1.5; // Move back along Z axis relative to center
            state.threeJsViewer.camera.lookAt(center);

            if (state.threeJsViewer.controls) {
                state.threeJsViewer.controls.target.copy(center);
                state.threeJsViewer.controls.maxDistance = cameraDistance * 5; // Adjust max distance based on object size
            }
        }
        if (state.threeJsViewer.controls) state.threeJsViewer.controls.update();

    } catch (e) {
        console.error("Error adjusting camera to new object:", e);
        // Fallback camera position
        state.threeJsViewer.camera.position.set(50, 50, 50);
        state.threeJsViewer.camera.lookAt(0,0,0);
        if (state.threeJsViewer.controls) state.threeJsViewer.controls.target.set(0,0,0);
    }
    console.log("Loaded new scene object into THREE.js viewer.");
}

function downloadCurrentModel() {
    if (!state.threeJsViewer.isInitialized || !state.threeJsViewer.currentModel) {
        alert("No model loaded in the viewer to download.");
        console.warn("Download attempt with no model or viewer not ready.");
        return;
    }

    const downloadFormatSelect = document.getElementById('model-download-format');
    const selectedFormat = downloadFormatSelect ? downloadFormatSelect.value : 'gltf';
    const sceneToExport = state.threeJsViewer.currentModel; // Or state.threeJsViewer.scene if you want the whole scene

    console.log(`Attempting to download model in ${selectedFormat} format.`);

    if (selectedFormat === 'gltf') {
        if (typeof THREE.GLTFExporter === 'undefined') {
            alert('GLTFExporter not available. Cannot download as GLTF.');
        ...
        exporter.parse(
            ...
            function (gltf) {
                ...
                link.download = `${state.threeJsViewer.title || 'model'}.gltf`; // Use title if available
                link.click();
                URL.revokeObjectURL(link.href);
                console.log("GLTF model exported for download.");
            },
            function (error) {
                console.error('Error exporting GLTF:', error);
                alert('Failed to export model as GLTF. Check console for details.');
            },
            { binary: false } // Set to true for GLB if preferred
        );
    } else if (selectedFormat === 'obj') {
        // Placeholder for OBJ export
        // Requires OBJExporter.js: https://threejs.org/examples/jsm/exporters/OBJExporter.js
        // You would need to include this script in your HTML.
        if (typeof THREE.OBJExporter === 'function') {
            ...
            try {
                ...
                const link = document.createElement('a');
                link.href = URL.createObjectURL(blob);
                link.download = `${state.threeJsViewer.title || 'model'}.obj`;
                link.click();
                URL.revokeObjectURL(link.href);
                console.log("OBJ model exported for download.");
            } catch (error) {
                console.error('Error exporting OBJ:', error);
                alert('Failed to export model as OBJ. Check console for details. Ensure OBJExporter is loaded.');
            }
        } else {
            alert('OBJExporter not available. Cannot download as OBJ. Please include OBJExporter.js.');
            console.error('THREE.OBJExporter not found.');
        }
    } else if (selectedFormat === 'fbx') {
        // Placeholder for FBX export - FBXExporter is more complex and might not be readily available as a simple script.
        // Typically requires a more involved setup or server-side conversion.
        alert('FBX export is experimental and not fully implemented in this example.');
        console.warn('FBX export selected, but not implemented.');
    } else {
        alert(`Unsupported download format: ${selectedFormat}`);
    }
}
// --- End THREE.js Viewer Modal Utility Functions ---

// --- Main function to link Tileset Exporter to THREE.js Viewer ---
async function generateAndShowTilesetInThreeJS(groupId, layerId, tilesetName) {
    // --- TEST PATH FOR 3DTILESRENDERERJS ---
    // Set useTest3DTilesPath to true to activate this test path.
    // It will attempt to load the test3DTilesetUrlForButton using loadTilesetWith3DTilesRenderer.
    const useTest3DTilesPath = true;
    const test3DTilesetUrlForButton = 'https://raw.githubusercontent.com/NASA-AMMOS/3DTilesRendererJS/master/example/tilesets/BatchedDeprecated/tileset.json';
    // const test3DTilesetUrlForButton = 'https://raw.githubusercontent.com/CesiumGS/3d-tiles-samples/main/1.1/SparseImplicitQuadtree/tileset.json'; // Another option
    
    // The test3DTilesetUrlForButton is now a tileset.json URL for conversion
    if (useTest3DTilesPath && test3DTilesetUrlForButton && typeof initiateTilesetConversionToGltf === 'function') {
        console.log(`[GENERATE_3D_VIEW] Test path: Calling initiateTilesetConversionToGltf with tileset.json URL: ${test3DTilesetUrlForButton}`);
        await initiateTilesetConversionToGltf(test3DTilesetUrlForButton);
        return; // Exit after attempting conversion
    }
    // --- END TEST PATH ---

    // Original function content starts here (the console.log below was the original first line):
    console.log(`Attempting to generate 3D view for tileset: ${tilesetName} (Group ID: ${groupId}, Layer ID: ${layerId})`);

    if (!window.userLayers || !window.userLayers[layerId] || !window.userLayers[layerId].layer) {
        console.error(`Layer with ID ${layerId} not found or invalid.`);
        alert(`Error: Layer ${layerId} not found.`);
        return;
    }

    const source = window.userLayers[layerId].layer.getSource();
    if (!source || typeof source.getFeatures !== 'function') {
        console.error(`Invalid source for layer ${layerId}.`);
        alert(`Error: Could not get features from layer ${layerId}.`);
        return;
    }

    const allFeaturesInLayer = source.getFeatures();
    const tilesetFeatures = allFeaturesInLayer.filter(feature => feature.get('tilesetGroupId') === groupId);

    if (tilesetFeatures.length === 0) {
        console.warn(`No features found for tilesetGroupId: ${groupId} in layer ${layerId}.`);
        alert(`No tiles found for tileset "${tilesetName}".`);
        return;
    }

    // Assuming selectionTileGrid is available and configured for ZL21 (e.g., state.selectionTileGrid or window.selectionTileGrid)
    // If not, a new one needs to be created here:
    const olTileGrid = window.selectionTileGrid || ol.tilegrid.createXYZ({maxZoom: 21, tileSize: 256});

    const tilesWithBoundsAndCoords = tilesetFeatures.map(feature => {
        const tileCoord = feature.get('tileCoord'); // Expecting [z, x, y]
        if (!tileCoord || tileCoord.length !== 3) {
            console.log('[3D VIEW DEBUG] Feature properties retrieved for 3D view (invalid tileCoord):', feature.getProperties());
            console.warn("Feature found without valid 'tileCoord':", feature.getId());
            return null;
        }
        try {
            const tileExtent3857 = olTileGrid.getTileCoordExtent(tileCoord);
            const tileExtent4326 = ol.proj.transformExtent(tileExtent3857, 'EPSG:3857', 'EPSG:4326');
            return {
                tileCoords: tileCoord,
                bounds: {
                    minLon: tileExtent4326[0],
                    minLat: tileExtent4326[1],
                    maxLon: tileExtent4326[2],
                    maxLat: tileExtent4326[3]
                }
            };
        } catch (e) {
            console.error("Error calculating bounds for tileCoord:", tileCoord, e);
            return null;
        }
    }).filter(item => item !== null);

    if (tilesWithBoundsAndCoords.length === 0) {
        console.error(`Valid tile coordinates or bounds could not be extracted for tileset "${tilesetName}".`);
        alert(`Error: Could not get valid tile coordinates/bounds for "${tilesetName}".`);
        return;
    }

    const textureUrlTemplate = 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}'; // Esri World Imagery
    const terrainUrlTemplate = 'https://terrain.openglobus.org/all/{z}/{x}/{y}.png';

    // Show a more user-friendly loading state
    const statusElement = document.getElementById('status-message'); // Assuming you have such an element
    if (statusElement) statusElement.textContent = `Generating 3D model for "${tilesetName}"...`;
    // Consider a proper loading spinner/overlay for better UX

    let cesiumScene = null;
    console.log(`[generateAndShowTilesetInThreeJS] Active globe library: ${state.activeGlobeLibrary}`);

    if (state.activeGlobeLibrary === 'olcesium') {
        // Prioritize the main OLCesium globe view instance if active
        // Assuming globePanelOlcesiumInstance is the global variable for the main OLCesium globe
        if (typeof globePanelOlcesiumInstance !== 'undefined' && globePanelOlcesiumInstance && typeof globePanelOlcesiumInstance.getEnabled === 'function' && globePanelOlcesiumInstance.getEnabled()) {
            cesiumScene = globePanelOlcesiumInstance.getCesiumScene();
            console.log("[generateAndShowTilesetInThreeJS] Using Cesium scene from active 'olcesium' globe view (globePanelOlcesiumInstance).");
        } else {
            console.log("[generateAndShowTilesetInThreeJS] 'olcesium' is active globe, but globePanelOlcesiumInstance is not available or not enabled.");
        }
    }

    // Fallback to state.olcsMapPanel (2D map's 3D mode) if the main OLCesium globe isn't the source or isn't ready
    if (!cesiumScene) {
        console.log("[generateAndShowTilesetInThreeJS] Attempting to use Cesium scene from state.olcsMapPanel (2D map's 3D mode).");
        console.log("[DEBUG OLCs Check] state.olcsMapPanel exists:", !!(state && state.olcsMapPanel));
        if (state && state.olcsMapPanel && typeof state.olcsMapPanel.getEnabled === 'function' && state.olcsMapPanel.getEnabled()) {
            try {
                cesiumScene = state.olcsMapPanel.getCesiumScene();
                console.log("[DEBUG OLCs Check] state.olcsMapPanel.getCesiumScene() obtained:", !!cesiumScene);
            } catch (e) {
                console.error("[DEBUG OLCs Check] Error getting scene from state.olcsMapPanel:", e);
            }
        } else {
            console.log("[DEBUG OLCs Check] state.olcsMapPanel is not defined, not enabled, or getEnabled is not a function.");
        }
    }

    if (cesiumScene) {
        console.log("[generateAndShowTilesetInThreeJS] Cesium scene is available and will be passed to the exporter for terrain sampling.");
    } else {
        console.log("[generateAndShowTilesetInThreeJS] Cesium scene is NOT available from any source. Exporter will use URL fallback for terrain.");
    }

    console.log("[MAIN.JS PRE-EXPORT] Final cesiumScene for exporter - Type:", typeof cesiumScene, "Value:", cesiumScene, "Is it truthy?", !!cesiumScene);
    try {
        const exportOptions = { // This is the 'options' object expected by tilesetExporter.js
            onComplete: (result) => {
                if (statusElement) statusElement.textContent = ''; // Clear loading message

                if (result.error) {
                    console.error(`Error generating 3D model for ${tilesetName}:`, result.error);
                    alert(`Error generating 3D model: ${result.error}`);
                } else if (result.sceneObject) {
                    console.log(`3D model (THREE.Group) generated for ${tilesetName}.`);
                    showThreeJsViewerModal(`3D View: ${tilesetName}`);
                    loadSceneObjectInViewer(result.sceneObject);
                } else if (result.gltfData) { // Should ideally always get sceneObject if target is 'viewer'
                    console.log(`GLTF data generated for ${tilesetName}. (This path might be deprecated if onComplete directly gives sceneObject)`);
                    // This path might not be hit if onComplete always provides sceneObject for 'viewer' target
                    if (typeof THREE.GLTFLoader === 'undefined') {
                        alert('THREE.GLTFLoader is not available. Cannot parse GLTF data.');
                        console.error('THREE.GLTFLoader is not available.');
                        return;
                    }
                    const loader = new THREE.GLTFLoader();
                    loader.parse(result.gltfData, '',
                        (gltf) => {
                            showThreeJsViewerModal(`3D View: ${tilesetName}`);
                            loadSceneObjectInViewer(gltf.scene);
                        },
                        (error) => {
                            console.error(`Error parsing GLTF data for ${tilesetName}:`, error);
                            alert(`Error parsing generated GLTF: ${error.message || 'Unknown parsing error'}`);
                        }
                    );
                } else {
                    console.error(`Unknown result from exportTilesetToGLTF for ${tilesetName}.`);
                    alert('Failed to generate 3D model: Unknown export result.');
                }
            }
            // onError callback could be added here if needed by exportTilesetToGLTF options
        };
        console.log("[MAIN.JS PRE-EXPORT] exportOptions - Type:", typeof exportOptions, "Value:", exportOptions);

        await exportTilesetToGLTF(
            tilesWithBoundsAndCoords,
            textureUrlTemplate,
            terrainUrlTemplate,
            tilesetName,
            exportOptions,
            cesiumScene // Ensure cesiumScene is passed as the last argument
        );
    } catch (exportError) {
        if (statusElement) statusElement.textContent = ''; // Clear loading message
        console.error(`Unhandled error during exportTilesetToGLTF call for ${tilesetName}:`, exportError);
        alert(`Critical error during 3D model generation: ${exportError.message || 'Unknown critical error'}`);
    }
}
// --- End generateAndShowTilesetInThreeJS ---
// Removing orphaned code remnants to restore syntax.
document.addEventListener('DOMContentLoaded', () => {
    const loadInitialData = true; // Declare and initialize loadInitialData
...
// Listener for THREE.js Viewer Modal Close Button
    const closeThreeJsViewerBtn = document.getElementById('close-threejs-viewer-modal-btn');
    if (closeThreeJsViewerBtn) {
        closeThreeJsViewerBtn.addEventListener('click', () => {
            hideThreeJsViewerModal();
        });
    }
    }
...
const userLayerListDiv = document.getElementById('user-layer-list');
    if (!userLayerListDiv) {
        console.error("[INIT_DOM_ERROR] CRITICAL: 'user-layer-list' DIV element not found. Layer list functionality will be broken.");
    }
...
// Update cursor and button state based on mode (from mundial517)
    function updateInteractionModeUI(mode) {
        if (!window.olMap) return; // This function primarily affects OpenLayers map UI
        const mapElement = document.getElementById('map');
        const interactionModeBtn = document.getElementById('interaction-mode-btn'); // Get button inside function

        if (mode === 'boxselect') { // Using 'boxselect' to match backup logic
            if (interactionModeBtn) {
                interactionModeBtn.textContent = 'Mode: Box Select';
                interactionModeBtn.title = 'Click to switch to Pan mode. Drag to select an area. (Shortcut: B or Ctrl)';
            ...
            if (interactionModeBtn) {
                ...
                interactionModeBtn.title = 'Click to switch to Box Select mode. Drag to pan. Click to select points. (Shortcut: B or Ctrl)';
...
console.log("MODE_BTN_INIT_DEBUG: interactionModeBtn element after getElementById:", interactionModeBtn);
    ...
    // --- Interaction Mode Button Setup (from mundial517/js/main.js) ---
    if (interactionModeBtn) {
        // Set initial mode and UI
        updateInteractionModeUI(currentInteractionMode);
        // Ensure initial OpenLayers interaction states match (dragPan active, dragBox inactive by default in initializeOpenLayersMap)
        // The initializeOpenLayersMap function should set dragPanInteraction.setActive(true) and dragBoxInteraction.setActive(false) initially.

        interactionModeBtn.addEventListener('click', function() {
            if (!window.olMap) return;
            
            if (currentInteractionMode === 'pan') {
                currentInteractionMode = 'boxselect'; // Match mode name used in updateInteractionModeUI
        ...
        console.warn("MODE_BTN_DEBUG: interactionModeBtn not found. Listener not attached.");
    }
    // --- Ctrl Key to Toggle Interaction Mode ---
    function setupCtrlKeyInteractionModeToggle() {
        if (!interactionModeBtn) {
            console.warn("CTRL_TOGGLE_DEBUG: interactionModeBtn not found, cannot attach Ctrl key listener.");
            return;
        }
        document.addEventListener('keydown', (event) => {
            ...
            if (event.key === 'Control') {
                // event.preventDefault(); // Optional: consider if needed
                console.log("CTRL_TOGGLE_DEBUG: Control key pressed, simulating click on interactionModeBtn.");
                interactionModeBtn.click();
            }
        });
        console.log("CTRL_TOGGLE_DEBUG: Ctrl key listener for interaction mode toggle attached.");
    }
    if (typeof setupCtrlKeyInteractionModeToggle === 'function') { // Ensure it's defined before calling
        setupCtrlKeyInteractionModeToggle();
    }
...
const viewTilesetInCesiumBtn = document.getElementById('view-tileset-in-cesium-btn');
    ...
    // Get DOM element for OLCesium container within Globe Panel (within DOMContentLoaded)
    // The 'let' declaration for this variable is at the top-level scope.
    // globePanelOlcesiumContainerElement is already defined and assigned as a const
    // around the globeViewContainers definition. This block is redundant and causes a TypeError.
    // if (document.getElementById('globe-panel-olcesium-container')) {
    //     // This would be an error: globePanelOlcesiumContainerElement = document.getElementById('globe-panel-olcesium-container');
    //     console.log("GLOBE_PANEL_OLCS_DOM: Element 'globe-panel-olcesium-container' should be fetched with other globe containers.");
    // } else {
    //     console.warn("GLOBE_PANEL_OLCS_DOM: 'globe-panel-olcesium-container' not found by redundant check. Check initial const declaration.");
    // }
    // Removed lookups for olcesiumDedicatedMapContainerElement and toggleOLCesiumDedicatedBtnElement
...
function updateInteractionModeUI(mode) { // Now only updates cursor
    ...
    function toggleTileSelection(tileCoord) {
        ...
        if (existingFeature) {
            ...
            newFeature.set('tileCoord', tileCoord); // Store the [z,x,y] coordinates
            console.log('[TOGGLETILE DEBUG] Feature from toggleTileSelection:', newFeature.getId(), newFeature.getProperties());
    ...
    function addTileToSelection(tileCoord) {
        ...
        if (!existingSelectionFeature) {
            ...
            if (!isTileSaved) {
                ...
                const newFeature = new ol.Feature({ geometry: ol.geom.Polygon.fromExtent(tileExtent) });
                newFeature.setId(tileId);
                newFeature.set('tileCoord', tileCoord); // <<< ADDED THIS LINE
                newFeature.set('isSelectionTile', true); // Changed from isIndividualSelection
                console.log('[ADDTILE DEBUG] Feature from addTileToSelection:', newFeature.getId(), newFeature.getProperties());
                selectionSource.addFeature(newFeature);
            }
        }
    }
function updateCesiumTerrainProvider(terrainType) {
    ...
    if (saveSelectionBtn) {
        saveSelectionBtn.addEventListener('click', () => {
            ...
            let name = tilesetNameInput.value.trim(); // Use let as it might be reassigned
            if (!name) {
                // alert("Please enter a name for the tileset."); // Old behavior - REMOVED
                // return; // Old behavior - REMOVED
                // New behavior: Use a default name
                const layerData = window.userLayers[window.selectedLayerId];
                const defaultTilesetNumber = (layerData?.tilesetCount || 0) + 1;
                name = `Tileset ${defaultTilesetNumber}`; // Assign to 'name'
                console.log(`SAVE_SELECTION: No name entered by user, using default: ${name}`);
            }
            // 'name' variable will now hold either user input or the default.
            ...
            .catch((error) => {
                ...
                console.warn(`Failed to save tileset to backend: ${error.message}. Tileset saved locally only.`);
                // Optionally, update a status bar or provide a less intrusive UI notification here
...
function initializeITowns() {
    ...
    if (itownsContainer && !itownsContainer.dataset.initialized) {
        ...
        try {
            ...
            window.olMap = state.olMap; // Ensure global window.olMap is also set for compatibility
            console.log("DEBUG: window.olMap also set to state.olMap.");
...
let isShiftKeyPressed = false; // For tracking shift key during box select
            // --- OpenLayers Map Interactions for Tile Selection (moved inside init) ---
            console.log("[INIT_OL_INTERACTIONS] About to set up interactions. state.olMap is:", state.olMap);
            if (!state.olMap) {
                console.error("[INIT_OL_INTERACTIONS] state.olMap is null/undefined before interaction setup! Interactions will fail.");
                return; // Cannot set up interactions without a map
            }
            ...
            if (!dragPanInteraction) {
                console.warn("Could not find default DragPan interaction. Creating new one.");
                dragPanInteraction = new ol.interaction.DragPan();
                state.olMap.addInteraction(dragPanInteraction);
            }

            // Ensure DragBox interaction is created and added
            if (!dragBoxInteraction) {
                dragBoxInteraction = new ol.interaction.DragBox({
                    // No specific condition, mode button will toggle it
                });
                state.olMap.addInteraction(dragBoxInteraction);
                console.log("DEBUG: Created and added DragBox interaction to map.");
            }
// Handle box start - clear selection unless shift key is pressed
            dragBoxInteraction.on('boxstart', function(event) {
                ...
                if (!state.olMap || !selectionSource) return; // Use state.olMap
                
                // Check if shift key is pressed during drag start
                // isShiftKeyPressed is defined at a higher scope (around line 2200)
            ...
            dragBoxInteraction.on('boxend', function() {
                ...
                if (!state.olMap || !selectionSource || !selectionTileGrid || !window.userLayers || !window.selectedLayerId || !dragBoxInteraction.getGeometry()) { // Use state.olMap
                ...
                selectionTileGrid.forEachTileCoord(boxExtent, TILE_SELECTION_ZOOM, function (tileCoord) {
                    ...
                    if (!existingTileIdsInLayer.has(tileId)) {
                        addTileToSelection(tileCoord); // Assumes addTileToSelection is defined
                        tilesAdded++;
                    }
                });
            ...
            console.log("DEBUG: Attached boxstart and boxend listeners to DragBox interaction.");

            // Set initial active state based on currentInteractionMode (globally defined, defaults to 'pan')
            if (currentInteractionMode === 'pan') {
                if (dragPanInteraction) dragPanInteraction.setActive(true);
                if (dragBoxInteraction) dragBoxInteraction.setActive(false);
            } else { // 'selectTiles' or 'boxselect'
                if (dragPanInteraction) dragPanInteraction.setActive(false);
                if (dragBoxInteraction) dragBoxInteraction.setActive(true);
            }
            updateInteractionModeUI(currentInteractionMode); // Update button text and cursor
            console.log("DEBUG: Set initial interaction states. Pan active:", dragPanInteraction ? dragPanInteraction.getActive() : 'N/A', "Box active:", dragBoxInteraction ? dragBoxInteraction.getActive() : 'N/A');

            // Attach the single click handler for tile selection when not in box select mode
            if (state.olMap) {
                state.olMap.on('singleclick', function(evt) {
                    // Only proceed if not in boxselect mode, as boxselect handles its own logic
                    if (currentInteractionMode !== 'boxselect') {
                        clickSelectHandler(evt);
                    }
                });
                console.log("DEBUG: Attached singleclick listener to map for tile selection.");
            } else {
                console.error("DEBUG: state.olMap not available to attach singleclick listener.");
            }
...
function setupXRPanelLogic() {
    ...
    // --- Polygonjs Editor Initialization ---
    function initPolygonjsEditor() {
        console.log("DEBUG_EDITOR: initPolygonjsEditor function called.");
        const container = document.getElementById('polygonjs-editor-container');
        if (!container) {
            console.error("DEBUG_EDITOR: Polygonjs editor container not found!");
            return;
        }

        function attemptInitialization() {
            // Explicitly check window.POLYGONJS
            if (window.POLYGONJS && typeof window.POLYGONJS.Editor !== 'undefined') {
                try {
                    console.log("DEBUG_EDITOR: window.POLYGONJS and window.POLYGONJS.Editor found. Initializing new Polygonjs.Editor...");
                    const editor = new window.POLYGONJS.Editor({ // Use window.POLYGONJS
                        domElement: container,
                        // Add other configuration options here
                        // e.g., sceneName, assetUrls, etc.
                    });
                    console.log("DEBUG_EDITOR: Polygonjs Editor initialized:", editor);
                    // Store the editor instance if needed for later interaction
                    state.polygonjsEditor = editor;

                    // Optional: Add a default scene or load a project
                    // editor.loadScene('default');
                } catch (error) {
                    console.error("DEBUG_EDITOR: Error initializing Polygonjs Editor:", error);
                }
            } else {
                let reason = "";
                if (!window.POLYGONJS) {
                    reason = "window.POLYGONJS is not defined.";
                } else if (typeof window.POLYGONJS.Editor === 'undefined') {
                    reason = "window.POLYGONJS is defined, but window.POLYGONJS.Editor is not.";
                } else {
                    reason = "Unknown reason."
                }
                console.log(`DEBUG_EDITOR: Polygonjs not ready (${reason}). Retrying...`);
                requestAnimationFrame(attemptInitialization);
            }
        }
        attemptInitialization();
    }
    // --- End Polygonjs Editor Initialization ---
...
console.log(`[ADD_LAYER_TO_LIST] Called with layerId: ${layerId}, layerName: ${layerName}, isVisible: ${isVisible}`);
    if (!userLayerListDiv) { 
        console.error("[ADD_LAYER_TO_LIST] CRITICAL: userLayerListDiv (the DOM element for layer list) is null or undefined. Cannot add layer item to UI.");
        return; 
    }
    // The original function body (now starting at line 2243 after this insertion) uses 'userLayerList'.
    // Let's ensure that variable is also checked or consistently used.
    // For now, the primary check is userLayerListDiv.
...
if (window.ogSavedTilesetsLayer) {
    ...
    if (createLayerBtn) {
        createLayerBtn.addEventListener('click', () => {
            console.log("[CREATE_LAYER_BTN_DEBUG] Clicked. Checking window.olMap immediately:", window.olMap);
...
console.log(`[CREATE_LAYER] Attempting to add layer to UI list. ID: ${newLayerId}, Name: ${trimmedName}, Layer object:`, window.userLayers[newLayerId]);
...
// console.log(`[POPULATE_DEBUG] Processing feature: ID=${feature.getId()}, GroupID=${groupId}, Name=${name}`);
            const groupId = feature.get('tilesetGroupId');
...
console.log(`[POPULATE_DEBUG] Grouped tilesets for layer ${layerId}:`, Object.keys(groupedTilesets));
        // For more detail: console.log('[POPULATE_DEBUG] groupedTilesets content:', JSON.stringify(groupedTilesets, null, 2));
...
console.log(`populateTilesetList: Creating list item for groupId: '${groupId}', name: '${groupData.name}'`);
            ...
            // "View 3D" button remains commented out as per user request to change interaction model
            // const view3DBtn = document.createElement('button');
            // view3DBtn.innerHTML = '👁️&nbsp;3D'; // Eye icon + 3D
            // view3DBtn.classList.add('settings-btn-small');
            // view3DBtn.title = `View "${groupData.name}" in 3D (THREE.js)`;
            // view3DBtn.addEventListener('click', (event) => {
            //     event.stopPropagation();
            //     const currentLayerId = window.selectedLayerId;
            //     console.log(`View in 3D clicked for tilesetGroupId: ${groupId} from layerId: ${currentLayerId}, Name: ${groupData.name}`);
            //     if (typeof generateAndShowTilesetInThreeJS === 'function') {
            //         const tilesetDisplayName = groupData.name || `Tileset ${groupId.substring(0, 8)}`;
            //         generateAndShowTilesetInThreeJS(groupId, currentLayerId, tilesetDisplayName);
            //     } else {
            //         console.error("generateAndShowTilesetInThreeJS function is not defined.");
            //         alert("Error: The 3D viewing function is not available.");
            //     }
            // });
            ...
            editBtn.addEventListener('click', (e) => { // Re-add event listener
                e.stopPropagation();
                console.log(`Edit clicked for tilesetGroupId: ${groupId}`);
                // Assuming groupData.features[0] is a valid representative feature
                // The openTilesetDetailsModal function needs to be robust for this.
                if (groupData.features && groupData.features.length > 0) {
                    openTilesetDetailsModal(groupData.features[0]);
                } else {
                    console.warn(`No features in group ${groupId} to open details modal.`);
                    alert("No feature data available for this tileset group to open details.");
                }
            });
            ...
            deleteBtn.title = `Delete tileset group "${groupData.name}"`;
            deleteBtn.addEventListener('click', (e) => { // Re-add event listener
                e.stopPropagation();
                if (confirm(`Are you sure you want to delete the tileset group "${groupData.name}"? This will remove all its tiles from the map layer.`)) {
                    deleteTilesetGroup(layerId, groupId);
                }
            });

            // buttonContainer.appendChild(view3DBtn); // View 3D button remains removed
            buttonContainer.appendChild(editBtn);
            buttonContainer.appendChild(deleteBtn);
...
// Helper function to load GLTF into the active OLCesium instance (Map Panel)
    ...
    if (saveSelectionBtn && !state.saveSelectionListenerAttached) {
        ...
        saveSelectionBtn.addEventListener('click', () => {
            ...
            // Always generate a default tileset name, ignoring tilesetNameInput
            const baseTilesetCount = window.userLayers[window.selectedLayerId]?.tilesetCount || 0;
            let tilesetName = `Tileset ${baseTilesetCount + 1}`;
            if (tilesetNameInput) { // Optionally clear the input field so it doesn't show old user input
                tilesetNameInput.value = '';
            }
            
            const selectedFeatures = selectionSource.getFeatures();
            if (selectedFeatures.length === 0) { alert("No tiles selected to save."); return; }
            // The check for !tilesetName is no longer needed as it's always generated.
...
console.log('[SAVE DEBUG] Original feature from selectionSource - Properties:', feature.getProperties());
                ...
                clonedFeature.unset('isIndividualSelection');
// console.log('[SAVE DEBUG] Feature properties before adding to targetSource:', clonedFeature.getProperties()); // Reduced noise
                featuresToAdd.push(clonedFeature);
            });

            if (featuresToAdd.length > 0) {
                // Before adding new features, remove any existing features in the targetSource
                // that have the same tilesetName, to prevent visual duplicates in the list.
                // This ensures that saving a tileset with a name that already exists effectively "overwrites" the old one
                // by removing its constituent features before adding the new ones.
                const existingFeaturesInLayer = targetSource.getFeatures();
                const featuresToRemove = existingFeaturesInLayer.filter(f => f.get('tilesetName') === tilesetName);
                
                if (featuresToRemove.length > 0) {
                    console.log(`[SAVE_CLEANUP] Found ${featuresToRemove.length} existing OL features for tileset name "${tilesetName}". Removing them before adding new set.`);
                    featuresToRemove.forEach(f => {
                        // console.log(`[SAVE_CLEANUP] Removing feature with ID: ${f.getId()}, tilesetGroupId: ${f.get('tilesetGroupId')}`);
                        targetSource.removeFeature(f);
                    });
                }

                targetSource.addFeatures(featuresToAdd);
                console.log(`%cSAVE HANDLER: Added ${featuresToAdd.length} features to targetSource for tileset "${tilesetName}" with new group ID "${tilesetGroupId}".`, "color: green;");
...
console.log("%cSAVE HANDLER: populateTilesetList has been called from save handler.", "color: green; font-weight: bold;");
                ...
                // populateTilesetList(window.selectedLayerId); // MOVED: Called by saveTilesetToKartService in its .finally() block
                tilesetNameInput.value = '';
...
// It was inserted inside the new og.Globe options object.
                ...
                if (globusContainerElement) {
                    globusContainerElement.style.display = 'none'; // Ensure it starts hidden
                    console.log("INIT_OG: globusContainer explicitly set to display:none after init.");
                }
...
console.log("%cDEBUG: POST-INSTANTIATION of ogSavedTilesetsLayer & addLayer call", "color: yellow; font-weight: bold;");
    ...
    let polygonjsEditorInitialized = false; // Flag to ensure initialization happens only once
        ...
        if (button && panel) {
            ...
            button.addEventListener('click', function() {
                ...
                if (isActiveAfterToggle) {
                    ...
                    // If this is the editor button, initialize Polygonjs if not already done
                    if (btnId === 'editor-btn' && !polygonjsEditorInitialized) {
                        console.log("DEBUG_EDITOR: Initializing Polygonjs editor from viewToggleButtons loop...");
                        initPolygonjsEditor(); // Call the initialization function
                        polygonjsEditorInitialized = true; // Set the flag
                    } else if (btnId === 'editor-btn' && polygonjsEditorInitialized) {
                        console.log("DEBUG_EDITOR: Polygonjs editor already initialized.");
                        // Optional: Add resize logic here if needed when panel is reopened
                    }
...
const downloadGltfBtn = document.getElementById('download-gltf-btn');
    if (downloadGltfBtn) {
        downloadGltfBtn.addEventListener('click', () => {
            if (window.currentGltfJsonForDownload && window.currentGltfFilename) {
                console.log(`[DOWNLOAD_GLTF_BTN] Triggering download for ${window.currentGltfFilename}`);
                // Assuming downloadJSON is globally available or defined elsewhere in main.js
                // If not, it needs to be defined or imported.
                // It was used in tilesetExporter.js, so it should be available.
                downloadJSON(window.currentGltfJsonForDownload, window.currentGltfFilename);
            } else {
                console.warn("[DOWNLOAD_GLTF_BTN] No GLTF data available for download.");
                alert("No GLTF model has been generated or loaded yet.");
            }
        });
    } else {
        console.warn("[INIT] Download GLTF button ('download-gltf-btn') not found during setup.");
    }

    // Placeholder for OBJ download button
    const downloadObjBtn = document.getElementById('download-obj-btn');
    if (downloadObjBtn) {
        downloadObjBtn.addEventListener('click', () => {
            alert('OBJ export is not yet implemented.');
            console.warn("[DOWNLOAD_OBJ_BTN] OBJ export not implemented.");
        });
    } else {
        console.warn("[INIT] Download OBJ button ('download-obj-btn') not found during setup.");
    }

    // Placeholder for FBX download button
const toggleTextureBtn = document.getElementById('toggle-texture-btn');
    if (toggleTextureBtn) {
        toggleTextureBtn.addEventListener('click', () => {
            if (state.threeJsViewer.currentModel && state.threeJsViewer.scene) {
                state.threeJsViewer.currentModel.traverse(child => {
                    if (child.isMesh && child.material) {
                        ...
                        materials.forEach(material => {
                            if (material && material.isMaterial) {
                                ...
                                // Initialize userData if first time
                                if (material.userData.textureEnabled === undefined) {
                                    if (material.map) {
                                        material.userData.originalMap = material.map;
                                        material.userData.textureEnabled = true;
                                    } else {
                                        material.userData.textureEnabled = false; // No initial map
                                    }
                                }

                                // Toggle texture state
                                if (material.userData.textureEnabled) {
                                    material.map = null; // Turn texture off
                                    material.userData.textureEnabled = false;
                                    console.log("Texture OFF for material on mesh:", child.name || child.uuid);
                                } else {
                                    if (material.userData.originalMap) {
                                        material.map = material.userData.originalMap; // Turn texture on
                                        material.userData.textureEnabled = true;
                                        console.log("Texture ON for material on mesh:", child.name || child.uuid);
                                    } else {
                                        console.log("No original texture to turn ON for material on mesh:", child.name || child.uuid);
                                    }
const sceneBtn = document.getElementById('scene-btn');
    const scenePanel = document.getElementById('scene-panel');
    console.log("[INIT_UI_DEBUG] Found sceneBtn:", !!sceneBtn, "scenePanel:", !!scenePanel);

    if (sceneBtn && scenePanel) {
        sceneBtn.addEventListener('click', () => {
            console.log("[SCENE_BTN_CLICK] Scene button clicked.");
            const isCurrentlyHidden = scenePanel.style.display === 'none' || !scenePanel.style.display;
            console.log(`[SCENE_BTN_CLICK] Panel current display: '${scenePanel.style.display}', isHidden: ${isCurrentlyHidden}`);
            
            if (isCurrentlyHidden) {
                scenePanel.style.display = 'block';
                sceneBtn.classList.add('active');
                console.log("[SCENE_BTN_CLICK_DEBUG] Scene panel display set to 'block'. 'active' class added to sceneBtn. ClassList:", sceneBtn.classList.toString());

                // Ensure Three.js viewer is initialized if not already
                if (typeof initThreeJsViewer === 'function' && (!state.threeJsViewer || !state.threeJsViewer.isInitialized)) {
                    console.log("[SCENE_BTN_CLICK_DEBUG] Three.js viewer is not initialized. Calling initThreeJsViewer().");
                    initThreeJsViewer();
                } else if (state.threeJsViewer && state.threeJsViewer.isInitialized) {
                    console.log("[SCENE_BTN_CLICK_DEBUG] Three.js viewer already initialized.");
                } else {
                     console.warn("[SCENE_BTN_CLICK_DEBUG] initThreeJsViewer function not found or state.threeJsViewer issue. Viewer cannot be initialized/confirmed.");
                }

                // Attempt to resize Three.js viewer
                if (state.threeJsViewer && state.threeJsViewer.isInitialized && state.threeJsViewer.container && state.threeJsViewer.renderer && state.threeJsViewer.camera) {
                    console.log("[SCENE_BTN_CLICK_DEBUG] Attempting to resize Three.js viewer.");
                    setTimeout(() => {
                        const container = state.threeJsViewer.container;
                        if (container.offsetParent !== null) {
                            const newWidth = container.clientWidth;
                            const newHeight = container.clientHeight;
                            console.log(`[SCENE_BTN_CLICK_DEBUG] Container dimensions for resize: ${newWidth}x${newHeight}`);
                            if (newWidth > 0 && newHeight > 0) {
                                state.threeJsViewer.camera.aspect = newWidth / newHeight;
                                state.threeJsViewer.camera.updateProjectionMatrix();
                                state.threeJsViewer.renderer.setSize(newWidth, newHeight);
                                console.log(`[SCENE_BTN_CLICK_DEBUG] Three.js viewer successfully resized to: ${newWidth}x${newHeight}`);
                            } else {
                                console.warn("[SCENE_BTN_CLICK_DEBUG] Resize failed: Three.js viewer container has zero dimensions.");
                            }
                        } else {
                             console.warn("[SCENE_BTN_CLICK_DEBUG] Resize check skipped: Scene panel container not truly visible (offsetParent is null).");
                        }
                    }, 150);
                } else {
                    console.warn("[SCENE_BTN_CLICK_DEBUG] Resize check skipped: Three.js viewer not ready. Initialized:", state.threeJsViewer ? state.threeJsViewer.isInitialized : 'N/A', "Container:", !!(state.threeJsViewer && state.threeJsViewer.container), "Renderer:", !!(state.threeJsViewer && state.threeJsViewer.renderer), "Camera:", !!(state.threeJsViewer && state.threeJsViewer.camera) );
                }
                // User request: "when a tileset is selected its tileset scene should automatically load."
                // This implies if currentEditingFeatureForModal exists, its 3D view should be triggered.
                // The sceneType3dtileBtn click handler already does this. We can simulate a click
} else { // This is the else for if (isCurrentlyHidden)
                scenePanel.style.display = 'none';
                sceneBtn.classList.remove('active');
                console.log("[SCENE_BTN_CLICK_DEBUG] Scene panel display set to 'none'. 'active' class removed from sceneBtn. ClassList:", sceneBtn.classList.toString());
            }
}); // Closes sceneBtn.addEventListener('click', () => {
} // Closes if (sceneBtn && scenePanel)
else {
        // This else corresponds to: if (sceneBtn && scenePanel)
        if (!sceneBtn) console.error("[INIT_UI_ERROR] CRITICAL: Toolbar Scene button ('scene-btn') NOT FOUND during DOMContentLoaded. Check ID in index.html.");
        if (!scenePanel) console.error("[INIT_UI_ERROR] CRITICAL: Scene panel ('scene-panel') for 3D viewer NOT FOUND during DOMContentLoaded. Check ID in index.html.");
    }
const copyWrldsLinkBtn = document.getElementById('copy-wrlds-link-btn');
    if (copyWrldsLinkBtn) {
        copyWrldsLinkBtn.addEventListener('click', () => {
            const wrldsLinkElement = document.getElementById('tileset-wrlds-link');
            // Check if href exists, is not '#', and is not the placeholder text
            if (wrldsLinkElement && wrldsLinkElement.href && wrldsLinkElement.href !== '#' && !wrldsLinkElement.href.endsWith('wrlds.id/...')) {
                navigator.clipboard.writeText(wrldsLinkElement.href).then(() => {
                    const originalButtonText = copyWrldsLinkBtn.textContent;
                    copyWrldsLinkBtn.textContent = 'Copied!';
                    copyWrldsLinkBtn.disabled = true; // Briefly disable to prevent multi-clicks
                    setTimeout(() => { 
                        copyWrldsLinkBtn.textContent = originalButtonText;
                        copyWrldsLinkBtn.disabled = false; 
                    }, 2000);
                }).catch(err => {
                    console.error('Failed to copy link to clipboard: ', err);
                    alert('Failed to copy link. Your browser might not support this feature or permissions might be denied. See console for details.');
                });
            } else {
                alert('No valid link available to copy.');
                console.warn('[COPY_LINK_BTN] Attempted to copy an invalid or placeholder link.');
            }
        });
// Initialize button state: disable if link is placeholder
        const initialLinkElement = document.getElementById('tileset-wrlds-link');
        if (initialLinkElement && (!initialLinkElement.href || initialLinkElement.href === '#' || initialLinkElement.href.endsWith('wrlds.id/...'))) {
            copyWrldsLinkBtn.disabled = true;
            console.log('[INIT_UI_DEBUG] copyWrldsLinkBtn disabled due to placeholder/invalid link.');
        } else if (!initialLinkElement) {
            copyWrldsLinkBtn.disabled = true; // Also disable if element itself not found
            console.log('[INIT_UI_DEBUG] copyWrldsLinkBtn disabled, tileset-wrlds-link element not found.');
        }
} // Closes if (copyWrldsLinkBtn)
else {
        console.warn("[INIT] Copy WRLDS Link button ('copy-wrlds-link-btn') not found during setup.");
    }

const showDownloadOptionsBtn = document.getElementById('show-download-options-btn');
    const downloadOptionsModal = document.getElementById('download-options-modal');
    
    if (showDownloadOptionsBtn && downloadOptionsModal) {
        const downloadFormatBtns = downloadOptionsModal.querySelectorAll('.download-format-btn'); // Query within modal

        showDownloadOptionsBtn.addEventListener('click', () => {
            // Check if there's a model loaded in the Three.js viewer or if GLTF JSON is available
            const modelAvailable = state.threeJsViewer && state.threeJsViewer.currentModel;
            const gltfJsonAvailable = window.currentGltfJsonForDownload && window.currentGltfFilename;

            if (modelAvailable || gltfJsonAvailable) {
                downloadOptionsModal.style.display = 'block';
            } else {
                alert("No model has been generated or loaded in the 3D viewer to download.");
                console.warn("[DOWNLOAD_OPTIONS] No model available for download.");
            }
        });

        // The close button for the modal is handled by an inline onclick in the HTML:
        // onclick="document.getElementById('download-options-modal').style.display='none';"

        downloadFormatBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                const format = btn.getAttribute('data-format');
                downloadOptionsModal.style.display = 'none'; // Hide modal after selection

                console.log(`[DOWNLOAD_FORMAT] User selected format: ${format}`);
                
                // Ensure there's something to process for download, even if modal was shown
                const modelAvailable = state.threeJsViewer && state.threeJsViewer.currentModel;
                const gltfJsonAvailable = window.currentGltfJsonForDownload && window.currentGltfFilename;

                if (!modelAvailable && !gltfJsonAvailable && format === 'gltf') {
                     alert("GLTF data is not available. Please generate a model first.");
                     console.warn("[DOWNLOAD_FORMAT] GLTF selected, but no data available.");
                     return;
                }
                // For OBJ/FBX, we'd need the sceneObject (state.threeJsViewer.currentModel)
                if (!modelAvailable && (format === 'obj' || format === 'fbx')) {
                    alert(`Cannot export to ${format.toUpperCase()} as no 3D model is loaded in the viewer.`);
                    console.warn(`[DOWNLOAD_FORMAT] ${format.toUpperCase()} selected, but no scene model available.`);
                    return;
                }


                if (format === 'gltf') {
                    if (gltfJsonAvailable) {
                        downloadJSON(window.currentGltfJsonForDownload, window.currentGltfFilename);
                    } else if (modelAvailable) {
                        // If only scene object is available, we need to re-export it to GLTF string first
                        // This requires GLTFExporter
                        if (typeof THREE !== 'undefined' && THREE.GLTFExporter) {
                            const exporter = new THREE.GLTFExporter();
                            exporter.parse(
                                state.threeJsViewer.currentModel,
                                function (gltf) {
                                    const output = JSON.stringify(gltf, null, 2);
                                    const filename = (state.threeJsViewer.currentModel.name || 'model') + '.gltf';
                                    downloadJSON(output, filename);
                                    // Optionally update window.currentGltfJsonForDownload here too
                                    window.currentGltfJsonForDownload = output;
                                    window.currentGltfFilename = filename;
                                },
                                function (error) {
                                    console.error('Error re-exporting current model to GLTF:', error);
                                    alert('Could not export current model to GLTF for download.');
                                },
                                { binary: false }
                            );
                        } else {
                            alert("GLTFExporter not available to export current model.");
                            console.error("[DOWNLOAD_FORMAT] THREE.GLTFExporter missing for on-the-fly GLTF export.");
                        }
                    } else { 
                        // This case should have been caught by the initial check, but as a fallback:
                        alert("GLTF data is not available for download.");
                    }
                } else if (format === 'obj') {
                    alert('OBJ export is not yet implemented. This would require OBJExporter.');
                    console.warn('[DOWNLOAD_FORMAT] OBJ export selected - NOT IMPLEMENTED.');
                    // Example: if (THREE.OBJExporter) { /* export logic */ }
                } else if (format === 'fbx') {
                    alert('FBX export is not yet implemented. This would require FBXExporter and is more complex.');
                    console.warn('[DOWNLOAD_FORMAT] FBX export selected - NOT IMPLEMENTED.');
                } else {
                    alert(`Download for format "${format}" is not yet implemented.`);
                }
            });
        });

    } else {
        if (!showDownloadOptionsBtn) console.warn("[INIT] 'Download...' button ('show-download-options-btn') not found.");
        if (!downloadOptionsModal) console.warn("[INIT] Download options modal ('download-options-modal') not found.");
    }
                                }
                ...
                alert("No model loaded to toggle textures.");
                console.warn("[TOGGLE_TEXTURE_BTN] No model loaded in state.threeJsViewer.currentModel");
            }
        });
    } else {
        console.warn("[INIT] Toggle Texture button ('toggle-texture-btn') not found during setup.");
    }
    const downloadFbxBtn = document.getElementById('download-fbx-btn');
    if (downloadFbxBtn) {
        downloadFbxBtn.addEventListener('click', () => {
            alert('FBX export is not yet implemented.');
            console.warn("[DOWNLOAD_FBX_BTN] FBX export not implemented.");
        });
    } else {
        console.warn("[INIT] Download FBX button ('download-fbx-btn') not found during setup.");
    }
...
// --- Scene Panel Logic ---
    ...
    if (sceneType3dtileBtn && sceneIframe && scenePanel) { // Ensure scenePanel is also defined
        ...
        sceneType3dtileBtn.addEventListener('click', () => {
            ...
            // NOTE: The attemptGLTFExportAndSend wrapper was removed in previous refactoring.
            // The logic is now directly in this event listener.
            ...
            if (groupFeatures.length > 0) {
            ...
            } else if (tilesetName === "Test Tileset SoL" && groupId.startsWith("test-tileset-")) { // Specific test case
                ...
                if (needsToLoadCesiumHtml) sceneIframe.setAttribute('src', 'scene_cesium.html');
                return;
            }
            if (!tilesArray || tilesArray.length === 0) {
            ...
            if (baseLayerSelectOL) {
                ...
                else if (val === 'terrarium') textureUrl = 'https://{a-c}.tile.openstreetmap.org/{z}/{x}/{y}.png'; // Still OSM for terrarium
                const customUrl = customLayerUrlInput ? customLayerUrlInput.value : '';
                if (val === 'add-custom' && customUrl) textureUrl = customUrl;
            }
            const terrainUrl = 'https://terrain.openglobus.org/all/{z}/{x}/{y}.png';
            ...
            let activeCesiumSceneForExport = null;
            if (state.activeGlobeLibrary === 'olcesium' && typeof globePanelOlcesiumInstance !== 'undefined' && globePanelOlcesiumInstance) {
                activeCesiumSceneForExport = globePanelOlcesiumInstance.getCesiumScene();
                console.log("[EXPORT_GLTF_PREP] Using OLCesium scene for terrain sampling.");
            } else if (state.activeGlobeLibrary === 'cesium' && typeof state.cesiumViewer !== 'undefined' && state.cesiumViewer && state.cesiumViewer.scene) {
                activeCesiumSceneForExport = state.cesiumViewer.scene;
                console.log("[EXPORT_GLTF_PREP] Using main Cesium scene for terrain sampling.");
            } else {
                console.log("[EXPORT_GLTF_PREP] No active Cesium scene for terrain sampling, will use terrainUrl fallback.");
            }

            const onExportCompleteCallback = ({ gltfData, sceneObject, error }) => {
                sceneType3dtileBtn.textContent = "3D Tile";
                sceneType3dtileBtn.disabled = false;

                if (error) {
                    alert("Error generating GLTF: " + error);
                    console.error("GLTF Export Error from onComplete:", error);
                    return;
                }

                const tilesetNameForDisplay = tilesetName || 'Generated Tileset';

                const ulidDisplayElement = document.getElementById('tileset-ulid-display');
                const wrldsLinkElement = document.getElementById('tileset-wrlds-link');
                const copyWrldsLinkBtn = document.getElementById('copy-wrlds-link-btn');
                if (ulidDisplayElement) {
                    ulidDisplayElement.textContent = `ULID: ${tilesetNameForDisplay}`;
                }
                if (wrldsLinkElement) {
                    const wrldsLink = `https://wrlds.id/${tilesetNameForDisplay}`;
                    wrldsLinkElement.href = wrldsLink;
                    wrldsLinkElement.textContent = wrldsLink;
                    if(copyWrldsLinkBtn) copyWrldsLinkBtn.disabled = false;
                } else {
                    if(copyWrldsLinkBtn) copyWrldsLinkBtn.disabled = true;
                }

                if (sceneObject) {
                    console.log(`[EXPORT_GLTF_COMPLETE] Received THREE.Object3D for ${tilesetNameForDisplay}. Loading into viewer.`);
                    if (typeof showThreeJsViewerModal === 'function' && typeof loadSceneObjectInViewer === 'function') {
                        showThreeJsViewerModal(`3D View: ${tilesetNameForDisplay}`);
                        loadSceneObjectInViewer(sceneObject);
                    } else {
                        console.warn("Three.js viewer functions not available.");
                        alert("3D viewer functions not ready.");
                    }
                } else if (gltfData) {
                    console.log(`[EXPORT_GLTF_COMPLETE] Received GLTF JSON for ${tilesetNameForDisplay}. Parsing and loading.`);
                    if (typeof THREE !== 'undefined' && THREE.GLTFLoader) {
                        try {
                            const loader = new THREE.GLTFLoader();
                            loader.parse(gltfData, '', (loadedGltf) => {
                                if (typeof showThreeJsViewerModal === 'function' && typeof loadSceneObjectInViewer === 'function') {
                                    showThreeJsViewerModal(`3D View: ${tilesetNameForDisplay}`);
                                    loadSceneObjectInViewer(loadedGltf.scene);
                                } else {
                                    console.warn("Three.js viewer functions not available after parsing.");
                                    alert("3D viewer functions not ready.");
                                }
                            }, (loadError) => {
                                console.error("Error parsing GLTF string:", loadError);
                                alert("Error parsing GLTF data.");
                            });
                        } catch (parseError) {
                            console.error("Error setting up GLTFLoader:", parseError);
                            alert("Failed to parse GLTF data.");
                        }
                    } else {
                         console.error("THREE.GLTFLoader missing.");
                         alert("Cannot parse GLTF: GLTFLoader missing.");
                    }
                } else {
                    console.warn("[EXPORT_GLTF_COMPLETE] No model content from export.");
                    alert("Failed to generate 3D model.");
                }
            };

            // Prepare options for the refactored exportTilesetToGLTF
// Determine if we should use direct GLB path or fallback to heightmap
            let useDirectGlbPath = false;
            let glbSelectionCriteria = [];

            // --- TEST: Hardcode a GLB URL to test the '3dtiles' dataSourceType path ---
            // To use this, ensure testGlbUrl is a valid, CORS-accessible GLB URL.
            // To revert to normal behavior (checking currentEditingFeatureForModal or falling back to heightmap), set testGlbUrl = null;
            // const testGlbUrl = 'https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/DamagedHelmet/glTF-Binary/DamagedHelmet.glb';
            const testGlbUrl = null; // Defaulting to NOT use hardcoded test GLB. Will try feature.get('glbUrl') or fallback to heightmap.

            if (testGlbUrl) {
                let transformMatrix = new THREE.Matrix4(); // Identity matrix - no transformation for this test model
                // Example of setting a transform if needed for a test model:
                // transformMatrix.setPosition(new THREE.Vector3(0, 0, 0));
                // transformMatrix.makeScale(10,10,10);

                glbSelectionCriteria = [{ contentUrl: testGlbUrl, transformMatrix: transformMatrix }];
                useDirectGlbPath = true;
                console.log(`[GLTF_EXPORT_PREP] USING HARDCODED TEST GLB PATH FOR: ${testGlbUrl}`);
            } else if (currentEditingFeatureForModal && typeof currentEditingFeatureForModal.get === 'function' && currentEditingFeatureForModal.get('glbUrl')) {
                // Original logic if testGlbUrl is not set (i.e., testGlbUrl = null)
                const glbUrl = currentEditingFeatureForModal.get('glbUrl');
                let transformMatrix = new THREE.Matrix4();
                const rawTransform = currentEditingFeatureForModal.get('glbTransform');
                if (rawTransform) {
                    if (Array.isArray(rawTransform) && rawTransform.length === 16) {
                        transformMatrix.fromArray(rawTransform);
                    } else if (typeof rawTransform.elements !== 'undefined' && typeof rawTransform.isMatrix4 !== 'undefined' && rawTransform.isMatrix4) {
                        transformMatrix.copy(rawTransform);
                    } else {
                        console.warn("[GLTF_EXPORT_PREP] glbTransform from feature is present but not in a recognized format. Using identity matrix.");
                    }
                }
                glbSelectionCriteria = [{ contentUrl: glbUrl, transformMatrix: transformMatrix }];
                useDirectGlbPath = true;
                console.log(`[GLTF_EXPORT_PREP] Using direct GLB path from feature data for: ${glbUrl}`);
            } else {
                console.log(`[GLTF_EXPORT_PREP] No hardcoded test GLB and no direct GLB URL on feature. Will use heightmap path.`);
            }
            // Conditional logic for export based on useDirectGlbPath (defined above this block)
            if (useDirectGlbPath) {
                const directGlbExportOptions = {
                    tilesetName: tilesetName,
                    onComplete: onExportCompleteCallback, // Defined above
                    dataSourceType: '3dtiles'
                    // textureUrlTemplate, terrainUrlTemplate, cesiumScene are not used by exporter in '3dtiles' mode
                };
                // Check for GLTFLoader for '3dtiles' mode
                if (typeof THREE === 'undefined' || !THREE.GLTFLoader) {
                    console.error("[MAIN] THREE.js or THREE.GLTFLoader is missing. Cannot process direct GLB.");
                    alert("Required Three.js components (GLTFLoader) missing for direct GLB display.");
                    sceneType3dtileBtn.textContent = originalButtonText; // Restore button
                    sceneType3dtileBtn.disabled = false;
                    return; // Exit if loader is missing
                }
                console.log("[GLTF_EXPORT_PREP] Calling exportTilesetToGLTF in '3dtiles' mode with direct GLB criteria.");
                window.exportTilesetToGLTF(glbSelectionCriteria, directGlbExportOptions);
            } else {
                // Fallback to heightmap generation
                const heightmapExportOptions = {
                    textureUrlTemplate: textureUrl, // Use pre-calculated textureUrl
                    terrainUrlTemplate: terrainUrl, // Use pre-calculated terrainUrl
                    tilesetName: tilesetName,
                    onComplete: onExportCompleteCallback, // Defined above
                    cesiumScene: activeCesiumSceneForExport, // Use pre-calculated activeCesiumSceneForExport
                    dataSourceType: 'heightmap'
                };

                // Check for GLTFExporter for 'heightmap' mode
                if (typeof THREE === 'undefined' || !THREE.GLTFExporter) {
                     console.error("[MAIN] THREE.js or THREE.GLTFExporter is missing. Cannot generate model from heightmap.");
                     alert("Required Three.js components (GLTFExporter) missing for heightmap model generation.");
                     sceneType3dtileBtn.textContent = originalButtonText; // Restore button
                     sceneType3dtileBtn.disabled = false;
                     return; // Exit if exporter is missing
                }
                if (!tilesArray || tilesArray.length === 0) {
                    alert("No tile coordinates available for heightmap generation. Please select tiles on the map.");
                    sceneType3dtileBtn.textContent = originalButtonText;
                    sceneType3dtileBtn.disabled = false;
                    return;
                }
                console.log("[GLTF_EXPORT_PREP] Calling exportTilesetToGLTF in 'heightmap' mode.");
                window.exportTilesetToGLTF(tilesArray, heightmapExportOptions);
            }
        }); // Clean closing for addEventListener callback
    ...
    function switchToMoonView() {
        ...
        // state.globus = new og.Globe({ // MOON GLOBE - Temporarily commented out to avoid conflict with Earth globe on "globusContainer"
        ...
        // if (state.globus.planet) { // This would refer to the Moon instance if uncommented
        ...
        // console.log("OpenGlobus switched to Moon."); // This log is now misleading if Moon is commented out
        // updateActiveGlobeButton('setting-globe-moon'); // This should only be called if Moon is actually active
    }
...
console.log("[DOMContentLoaded_EARLY_CHECK] Checking critical DOM elements...");
    const mapDivExists = !!document.getElementById('map');
    const globusContainerExists = !!document.getElementById('globusContainer');
    const userLayerListDivExists = !!document.getElementById('user-layer-list');
    console.log(`[DOMContentLoaded_EARLY_CHECK] 'map' div: ${mapDivExists}, 'globusContainer': ${globusContainerExists}, 'user-layer-list': ${userLayerListDivExists}`);
    if (!mapDivExists || !globusContainerExists || !userLayerListDivExists) {
        console.error("[DOMContentLoaded_EARLY_CHECK] CRITICAL DOM ELEMENT(S) MISSING. Site will not function correctly.");
        // Optionally, display an error to the user here if these are missing.
    }
    ...
    */
    ...
    setTimeout(() => {
        ...
        // Initialize OpenLayers and OpenGlobus maps - MOVED to be called directly in DOMContentLoaded
        // initializeOpenLayersMap();
        // initializeOpenGlobus();
        
        // Load the test tileset (SoL 2x2 grid) after maps are initialized - MOVED to after direct map inits (L7177 area)
        // if (typeof loadTestTilesetToLayer0 === 'function') {
        //     console.log("Attempting to load test tileset on startup (from within setTimeout)...");
        //     loadTestTilesetToLayer0();
        // } else {
        //     console.warn("loadTestTilesetToLayer0 function is not defined, cannot load test data (from within setTimeout).");
        // }
        
        // Load the test tileset after maps are initialized
        // if (typeof loadTestTilesetToLayer0 === "function") { // Call moved to loadInitialData block
        //     loadTestTilesetToLayer0();
        // }
...
// Load test tileset if loadInitialData is true, after direct map initializations
    if (loadInitialData && typeof loadTestTilesetToLayer0 === 'function') {
        console.log("[DOMContentLoaded] Attempting to load initial test tileset (single call)...");
        loadTestTilesetToLayer0();
    } else {
        if (!loadInitialData) console.log("[DOMContentLoaded] loadInitialData is false, skipping test tileset load.");
        if(typeof loadTestTilesetToLayer0 !== 'function') console.warn("[DOMContentLoaded] loadTestTilesetToLayer0 function not found for initial load.");
    }

    // Load test tileset after maps are initialized
    // if (typeof loadTestTilesetToLayer0 === 'function') { // Call moved to loadInitialData block
    //     console.log("DEBUG_STARTUP: Calling loadTestTilesetToLayer0().");
    //     loadTestTilesetToLayer0();
    // } else {
    //     console.warn("DEBUG_STARTUP: loadTestTilesetToLayer0 function not found at this point (call moved).");
    // }
...
// --- Initial Panel Layout Function ---
    ...
    try {
        ...
        state.olcsMapPanel = olcsMapPanel; // Assign to state object
        console.log("INIT_OLCS: olcsMapPanel instance assigned to state.olcsMapPanel.");
        
        const scene = state.olcsMapPanel.getCesiumScene(); // Use state.olcsMapPanel here too
        if (scene) {
...
async function initGlobePanelOLCesiumView() {
    console.log("INIT_OLCS_GLOBE: Entered initGlobePanelOLCesiumView.");
    if (isGlobePanelOLCesiumInitialized) {
        console.log("INIT_OLCS_GLOBE: Already initialized.");
        return;
    }
    if (typeof olcs === 'undefined' || typeof Cesium === 'undefined' || typeof ol === 'undefined') {
        console.error("INIT_OLCS_GLOBE_FAIL: OLCesium, Cesium, or OpenLayers library not loaded.");
        return;
    }
    console.log("INIT_OLCS_GLOBE: Libraries seem loaded.");

    const olCesiumGlobeContainer = document.getElementById('globe-panel-olcesium-container'); // CORRECTED ID
    if (!olCesiumGlobeContainer) {
        console.error("INIT_OLCS_GLOBE_FAIL: Container 'globe-panel-olcesium-container' not found."); // Updated log message
        return;
    }
    console.log("INIT_OLCS_GLOBE: ol-cesium-globe-container found.");

    let globeOlMap;
    try {
        // Create a new, dedicated OpenLayers map for this OLCesium globe instance
        globeOlMap = new ol.Map({
            layers: [
                new ol.layer.Tile({
                    source: new ol.source.OSM({
                        // attributions: [ol.source.OSM.ATTRIBUTION], // Handled by OLCesium?
                        // crossOrigin: 'anonymous' // May be needed for some tile sources if issues arise
                    })
                })
            ],
            view: new ol.View({
            ...
            // Do NOT set target here if OLCesium is going to manage the DOM element.
            // Or, if target is set, ensure OLCesium can work with it.
            // Let OLCesium create its own map viewport within the container.
        });
        console.log("INIT_OLCS_GLOBE: Dedicated ol.Map for OLCesium globe panel created.");
    } catch (olMapError) {
        console.error("INIT_OLCS_GLOBE_FAIL: Error creating dedicated ol.Map for OLCesium globe panel.", olMapError);
        alert("Failed to create OpenLayers map for OL-Cesium Globe View. Check console.");
        return;
    }

    try {
        console.log("INIT_OLCS_GLOBE: Attempting 'new olcs.OLCesium(...)' for globe panel with target:", olCesiumGlobeContainer.id);
        // Ensure the container is empty or OLCesium can handle existing content.
        // olCesiumGlobeContainer.innerHTML = ''; // Optionally clear container
        
        globePanelOlcesiumInstance = new olcs.OLCesium({
            map: globeOlMap,
            target: olCesiumGlobeContainer.id,
            // sceneModePicker: true, // Example: add scene mode picker
        });
        
        // It's crucial that globeOlMap is rendered into a div that OLCesium can use.
        // OLCesium usually handles this if target is set. If not, we might need to:
        // const olMapViewport = document.createElement('div');
        // olMapViewport.style.width = '100%'; olMapViewport.style.height = '100%';
        // olCesiumGlobeContainer.appendChild(olMapViewport);
        // globeOlMap.setTarget(olMapViewport);
        // globeOlMap.updateSize(); // Important after setting target and appending

        console.log("INIT_OLCS_GLOBE: 'new olcs.OLCesium(...)' SUCCEEDED. globePanelOlcesiumInstance:", globePanelOlcesiumInstance);
        
        const scene = globePanelOlcesiumInstance.getCesiumScene();
        if (scene) {
            console.log("INIT_OLCS_GLOBE: Cesium scene obtained from globePanelOlcesiumInstance.");
            try {
                scene.terrainProvider = await Cesium.createWorldTerrainAsync();
                console.log("INIT_OLCS_GLOBE: Cesium World Terrain set for globe panel instance.");
            } catch (terrainError) {
                console.error("INIT_OLCS_GLOBE: Error setting Cesium World Terrain. Falling back to Ellipsoid.", terrainError);
                scene.terrainProvider = new Cesium.EllipsoidTerrainProvider({});
            }

            if (scene.globe) {
                scene.globe.enableLighting = true;
                console.log("INIT_OLCS_GLOBE: scene.globe.enableLighting set to true.");
            } else {
                console.warn("INIT_OLCS_GLOBE: scene.globe is null/undefined after terrain setup.");
            }

            if (typeof updateCesiumZL21Grid === 'function') {
                updateCesiumZL21Grid(scene);
                scene.camera.changed.addEventListener(() => updateCesiumZL21Grid(scene));
                console.log("INIT_OLCS_GLOBE: ZL21 grid update function called and listener added.");
            } else {
                console.warn("INIT_OLCS_GLOBE: updateCesiumZL21Grid function not found. Grid will not be displayed.");
            }

            // Synchronize with the main OpenLayers map view if available
            if (state.olMap && state.olMap.getView()) {
                const mainOlView = state.olMap.getView();
                const center = mainOlView.getCenter();
                const zoom = mainOlView.getZoom();
                const olMapForGlobe = globePanelOlcesiumInstance.getOlMap();
                
                if (olMapForGlobe && center && zoom !== undefined) {
                    const globeOlView = olMapForGlobe.getView();
                    globeOlView.setCenter(center);
                    globeOlView.setZoom(zoom);
                    console.log(`INIT_OLCS_GLOBE: Synchronized OLCesium globe view with main OL map view (Center: ${center}, Zoom: ${zoom})`);
                }
                // OLCesium should automatically update its Cesium camera based on its OL map view.
                // If not, might need: globePanelOlcesiumInstance.setCameraFromView(globeOlView); or similar.
            }

        } else {
            console.error("INIT_OLCS_GLOBE_ERROR: globePanelOlcesiumInstance.getCesiumScene() returned null.");
            // This is a critical failure point for OLCesium.
        }
        
        isGlobePanelOLCesiumInitialized = true;
        console.log("INIT_OLCS_GLOBE: OLCesium for main globe panel initialization attempt finished. isGlobePanelOLCesiumInitialized:", isGlobePanelOLCesiumInitialized);

    } catch (error) {
        console.error("INIT_OLCS_GLOBE_ERROR: Error during OLCesium main globe instantiation or setup.", error);
        globePanelOlcesiumInstance = null;
        isGlobePanelOLCesiumInitialized = false;
        alert("Failed to initialize OL-Cesium Globe View components. Check console.");
    }
}
...
const testTilesetConversionBtn = document.getElementById('test-tileset-conversion-btn');
    if (testTilesetConversionBtn) {
        testTilesetConversionBtn.addEventListener('click', () => {
            console.log("[TEST_BUTTON] 'Test Convert 3D Tiles' button clicked.");
            const testTilesetJsonUrl = 'https://raw.githubusercontent.com/NASA-AMMOS/3DTilesRendererJS/master/example/tilesets/BatchedDeprecated/tileset.json';
            // User can replace with Fort Tryon Park tileset.json URL when available:
            // const testTilesetJsonUrl = 'URL_FOR_FORT_TRYON_PARK_TILESET.json'; 
            
            if (testTilesetJsonUrl && typeof initiateTilesetConversionToGltf === 'function') {
                initiateTilesetConversionToGltf(testTilesetJsonUrl).catch(err => {
                    console.error("[TEST_BUTTON] Error during manual test conversion:", err);
                    const statusBar = document.getElementById('scene-status-bar');
                    if (statusBar) {
                        statusBar.textContent = `Test Conversion Error: ${err.message}`;
                        statusBar.style.display = 'block';
                    }
                });
            } else {
                alert("Test conversion function or URL is not set up correctly.");
            }
        });
    } else {
        console.warn("[INIT] Test tileset conversion button ('test-tileset-conversion-btn') not found.");
    }
// --- Auto-load test tileset on startup - TEMPORARILY COMMENTED OUT ---
// const autoLoadTilesetJsonUrl = 'https://raw.githubusercontent.com/NASA-AMMOS/3DTilesRendererJS/master/example/tilesets/BatchedDeprecated/tileset.json';
// // User can replace with Fort Tryon Park tileset.json URL when available:
// // const autoLoadTilesetJsonUrl = 'URL_FOR_FORT_TRYON_PARK_TILESET.json';

// if (autoLoadTilesetJsonUrl && typeof initiateTilesetConversionToGltf === 'function') {
//     console.log(`[STARTUP_TEST] Automatically initiating conversion for: ${autoLoadTilesetJsonUrl}`);
//     // Ensure the 3DTilesRendererJS library (three-loader.module.js) is correctly placed by the user
//     // to avoid a 404 error on page load due to its import statement (even if not directly used by this path).
//     // The import is currently commented out for debugging site load issues.
//     // If that import is re-enabled, the file MUST be present.
    
//     // Delay slightly to ensure UI is somewhat settled, though initiateTilesetConversionToGltf handles panel display.
//     setTimeout(() => {
//         initiateTilesetConversionToGltf(autoLoadTilesetJsonUrl).catch(err => {
//             console.error("[STARTUP_TEST] Error during auto-load conversion:", err);
//             const statusBar = document.getElementById('scene-status-bar');
//             if (statusBar) {
//                 statusBar.textContent = `Auto-load Error: ${err.message}`;
//                 statusBar.style.display = 'block';
//             }
//         });
//     }, 1000); // 1-second delay
// } else {
//     if (!autoLoadTilesetJsonUrl) console.log("[STARTUP_TEST] No autoLoadTilesetJsonUrl defined.");
//     if (typeof initiateTilesetConversionToGltf !== 'function') console.log("[STARTUP_TEST] initiateTilesetConversionToGltf function not found.");
// }
// --- End Auto-load test ---
...
// --- Toggle for Maps Menu (#layer-switcher) via button in #map-panel header ---
    ...
    const closeThreeJsViewerModalBtn = document.getElementById('close-threejs-viewer-modal');
    if (closeThreeJsViewerModalBtn) {
        closeThreeJsViewerModalBtn.addEventListener('click', hideThreeJsViewerModal);
    } else {
        console.warn("Button 'close-threejs-viewer-modal' not found.");
    }

    const downloadModelBtn = document.getElementById('download-model-btn');
    if (downloadModelBtn) {
        downloadModelBtn.addEventListener('click', downloadCurrentModel);
    } else {
        console.warn("Button 'download-model-btn' not found.");
    }

// --- iTowns Initialization ---
    let itownsView = null; // Keep track of the iTowns view instance

    function initITownsView() {
        console.log("DEBUG_ITOWNS: initITownsView function called.");
        if (itownsView) {
            console.log("DEBUG_ITOWNS: iTowns view already initialized.");
            // May need to call a refresh or update function if available
            if (typeof itownsView.notifyChange === 'function') {
                itownsView.notifyChange(itownsView.camera.camera3D, true);
            }
            return;
        }
        ...
        const itownsContainer = document.getElementById('itowns-container');
        if (!itownsContainer) {
            console.error("DEBUG_ITOWNS: iTowns container ('itowns-container') not found!");
            return;
        }
        
        if (typeof itowns === 'undefined') {
            console.error("DEBUG_ITOWNS: iTowns library is not loaded. Please include it in your HTML.");
            itownsContainer.innerHTML = '<p style="padding:10px; color: #f00;">Error: iTowns library not loaded.</p>';
            return;
        }

        console.log("DEBUG_ITOWNS: Initializing iTowns view...");

        const placement = {
            coord: new itowns.Coordinates('EPSG:4326', -74.0445, 40.6892), // Example: Statue of Liberty
            range: 2500, // Initial zoom level (distance in meters)
            tilt: 25,    // Camera tilt in degrees
        };

        itownsView = new itowns.GlobeView(itownsContainer, placement, {
            renderer: null // Use default renderer
        });
        state.itownsView = itownsView; // Store in global state if needed elsewhere

        // Add OSM Color Layer
        itowns.Fetcher.json('../packages/itowns/examples/layers/JSONLayers/Ortho.json').then(function _(config) {
            config.source = new itowns.TMSSource(config.source);
            let layer = new itowns.ColorLayer('Ortho', config);
            itownsView.addLayer(layer).then(itowns.GlobeView.prototype.addElevationLayer);
            console.log("DEBUG_ITOWNS: OSM Color layer added.");
        }).catch(err => console.error("DEBUG_ITOWNS: Error loading Ortho.json for iTowns:", err));

        // Add World Elevation Layer
        itowns.Fetcher.json('../packages/itowns/examples/layers/JSONLayers/WORLD_DTM.json').then(function _(config) {
            config.source = new itowns.WMTSSource(config.source);
            let layer = new itowns.ElevationLayer(config.id, config);
            itownsView.addLayer(layer);
            console.log("DEBUG_ITOWNS: World DTM Elevation layer added.");
        }).catch(err => console.error("DEBUG_ITOWNS: Error loading WORLD_DTM.json for iTowns:", err));
        
        // Add Atmosphere
        itownsView.addLayer({
            type: 'atmosphere',
            sunPosition: {longitude: -25, latitude: 45}, // Example sun position
            id: 'atmosphere',
        });
        console.log("DEBUG_ITOWNS: Atmosphere layer added.");

        // Add event listener for view ready
        itownsView.addEventListener(itowns.VIEW_EVENTS.INITIALIZED, () => {
            console.log('DEBUG_ITOWNS: iTowns GlobeView initialized and ready.');
        });
         // Add an event listener for full layer loading
        itowns.View.prototype.addEventListener.call(itownsView, 'layers-initialized', () => {
            console.log('DEBUG_ITOWNS: All iTowns layers have been initialized.');
            // You can perform actions here that depend on all layers being ready
        });


        console.log("DEBUG_ITOWNS: iTowns view setup complete. Instance:", itownsView);
    }
    // --- End iTowns Initialization ---
// --- Globe View Switcher Logic ---
    ...
    const globePanelOlcesiumContainerElement = document.getElementById('globe-panel-olcesium-container'); // Corrected ID

    const globeViewContainers = {
        'openglobus': openglobusContainer,
        'itowns': itownsGlobeContainer,
        'radiogarden': radiogardenContainer,
        'olcesium': globePanelOlcesiumContainerElement // Now correctly references the defined element
    };

    async function switchGlobeView(selectedGlobeValue) { // Made async
        console.log(`GLOBE_SWITCH: Switching to ${selectedGlobeValue}`);
        
        // Pause/disable other globe instances
        if (selectedGlobeValue !== 'openglobus' && state.globus && typeof state.globus.planet?.pause === 'function') {
            state.globus.planet.pause();
            console.log("GLOBE_SWITCH: Paused OpenGlobus.");
        }
        if (selectedGlobeValue !== 'itowns' && state.itownsView) {
            // Assuming iTowns doesn't need explicit pause, hiding container is enough.
            // If it has a setEnabled or pause, call it here.
        }
        if (selectedGlobeValue !== 'olcesium' && globePanelOlcesiumInstance && typeof globePanelOlcesiumInstance.getEnabled === 'function' && globePanelOlcesiumInstance.getEnabled()) {
            globePanelOlcesiumInstance.setEnabled(false);
            console.log("GLOBE_SWITCH: Disabled globePanelOlcesiumInstance.");
        }

        // Hide all globe containers first
        ...
        if (activeContainer) {
            activeContainer.style.display = 'block'; // Show the selected one
            console.log(`GLOBE_SWITCH: Displaying ${activeContainer.id}`);

            // Specific logic for each type
            if (selectedGlobeValue === 'openglobus') {
                if (!state.globus && typeof initializeOpenGlobus === 'function') { // Ensure OpenGlobus is initialized
                    initializeOpenGlobus();
                }
                if (state.globus && typeof state.globus.planet?.play === 'function') {
                    state.globus.planet.play(); // Resume if paused
                    console.log("GLOBE_SWITCH: Resumed OpenGlobus.");
                }
                 // Ensure OpenGlobus container is visible (it should be activeContainer already)
                if (openglobusContainer) openglobusContainer.style.display = 'block';
            } else if (selectedGlobeValue === 'radiogarden') {
                if (radiogardenIframe && !radiogardenIframe.src) {
                    radiogardenIframe.src = 'https://radio.garden/visit/paris/JqN8oJ3N';
                    console.log("GLOBE_SWITCH: Radio Garden iframe src set.");
                }
            } else if (selectedGlobeValue === 'itowns') {
                if (typeof initITownsView === 'function') {
                    initITownsView(); // This should handle its own initialization and visibility if needed
                    console.log("GLOBE_SWITCH: iTowns view selected/initialized.");
                } else {
                    console.warn("GLOBE_SWITCH: initITownsView function not found.");
                    if(itownsGlobeContainer) itownsGlobeContainer.innerHTML = '<p style="padding:10px; color: #f00;">Error: iTowns init script not found.</p>';
                }
            } else if (selectedGlobeValue === 'olcesium') {
                console.log("GLOBE_SWITCH: 'olcesium' case entered.");
                const olcContainer = globeViewContainers['olcesium'];
                if (olcContainer) {
                    console.log("GLOBE_SWITCH: OLCesium container found, ensuring display:block", olcContainer);
                    olcContainer.style.display = 'block'; // Explicitly show this container
                } else {
                    console.error("GLOBE_SWITCH: OLCesium container ('globe-panel-olcesium-container') NOT FOUND in globeViewContainers!");
                }

                if (!isGlobePanelOLCesiumInitialized) {
                    console.log("GLOBE_SWITCH: OLCesium not initialized, calling initGlobePanelOLCesiumView...");
                    await initGlobePanelOLCesiumView();
                } else {
                    console.log("GLOBE_SWITCH: OLCesium already initialized.");
                }

                if (globePanelOlcesiumInstance) {
                    console.log("GLOBE_SWITCH: Enabling globePanelOlcesiumInstance.");
                    globePanelOlcesiumInstance.setEnabled(true);
                    console.log("GLOBE_SWITCH: OLCesium view selected and instance enabled. IsEnabled:", globePanelOlcesiumInstance.getEnabled());
                    
                    // Ensure other main globe containers are explicitly hidden again, just in case.
                    if (openglobusContainer && openglobusContainer !== olcContainer) openglobusContainer.style.display = 'none';
                    if (itownsGlobeContainer && itownsGlobeContainer !== olcContainer) itownsGlobeContainer.style.display = 'none';
                    if (radiogardenContainer && radiogardenContainer !== olcContainer) radiogardenContainer.style.display = 'none';

                    setTimeout(() => {
                        console.log("GLOBE_SWITCH: Attempting to resize OLCesium components.");
                        const olMapInOlCesium = globePanelOlcesiumInstance.getOlMap();
                        if (olMapInOlCesium) {
                            console.log("GLOBE_SWITCH: Resizing OL map within OLCesium.");
                            olMapInOlCesium.updateSize();
                        }
                        // The OLCesium instance itself doesn't have getCesiumViewer().
                        // getCesiumScene() is correct for accessing the scene.
                        // Requesting a render on the scene should be sufficient if sizes are managed by OL.
                        const cesiumScene = globePanelOlcesiumInstance.getCesiumScene();
                        if (cesiumScene) {
                            console.log("GLOBE_SWITCH: Requesting render of Cesium scene after OL map update.");
                            cesiumScene.requestRender();
                        } else {
                            console.warn("GLOBE_SWITCH: Cesium scene not available for requesting render after OL map update.");
                        }
                        console.log("GLOBE_SWITCH: Finished resize attempt for OLCesium map/viewer.");
                    }, 150); // Slightly longer timeout
                } else {
                    console.error("GLOBE_SWITCH: globePanelOlcesiumInstance is null after init attempt. OLCesium view cannot be enabled.");
                }
            }
            state.activeGlobeLibrary = selectedGlobeValue;
        } else {
            console.warn(`GLOBE_SWITCH: No container found for globe value: ${selectedGlobeValue}. Defaulting to OpenGlobus.`);
            // Fallback to OpenGlobus if selected container not found
            if (openglobusContainer) openglobusContainer.style.display = 'block';
            if (state.globus && typeof state.globus.planet?.play === 'function') state.globus.planet.play();
            state.activeGlobeLibrary = 'openglobus';
        }
    }

    if (globeLibrarySelect && openglobusContainer && itownsGlobeContainer && radiogardenContainer && globePanelOlcesiumContainerElement) { // Added globePanelOlcesiumContainerElement
        ...
        // Defer this slightly to ensure DOM is fully settled
        setTimeout(() => {
            if (globeLibrarySelect) { // Re-check in case it became null
                 switchGlobeView(globeLibrarySelect.value);
                 console.log("GLOBE_SWITCH: Globe view switcher initialized (deferred). Default value:", globeLibrarySelect.value);
            } else {
                console.error("GLOBE_SWITCH: globeLibrarySelect is null in deferred init. Cannot set default globe.");
            }
        }, 0); // End of setTimeout
    } else {
        console.warn("GLOBE_SWITCH: Could not initialize globe view switcher. Missing one or more elements:", {
            ...
            // Note: globePanelOlcesiumContainerElement check is implicitly part of the main if condition
        });
    }
...
// --- GLOBE_VIEW_SWITCHER_LOGIC_MOVED_PLACEHOLDER_FINAL_CUT ---
