...
console.log("%cDEBUG: POST-INSTANTIATION of ogSavedTilesetsLayer & addLayer call", "color: yellow; font-weight: bold;");
    ...
    function handleGlobeClick(mouse, eventName) {
        ...
        if (currentInteractionMode === 'selectTiles') {
            console.log("HANDLEGLOBECLICK: In 'selectTiles' mode. Toggling individual tile.");
            
            const ogTileCoords = og.mercator.getTileCoordinate(lonLat.lon, lonLat.lat, TILE_SELECTION_ZOOM);
            const ogTileToSelect = [ogTileCoords.zoom, ogTileCoords.x, ogTileCoords.y];

            if (typeof toggleTileSelection === "function") {
                toggleTileSelection(ogTileToSelect); // Pass [z,x,y] from OpenGlobus calculation
            } else {
                console.error("HANDLEGLOBECLICK: toggleTileSelection function is not defined!");
            }
            ...
            if (window.ogSavedTilesetsLayer) {
                window.ogSavedTilesetsLayer.clear();
                console.log("DEBUG: ogSavedTilesetsLayer cleared after individual tile selection.");
            }
            if (window.gridLayerOG) {
                window.gridLayerOG.clear();
                console.log("DEBUG: gridLayerOG cleared after individual tile selection.");
            }
            return; // End here for individual tile selection mode
        }

        // If not in 'selectTiles' mode, proceed with group selection logic
        try {
            // Use OpenGlobus mercator functions to get the tile ID for lookup
            const ogTileInfo = og.mercator.getTileCoordinate(lonLat.lon, lonLat.lat, TILE_SELECTION_ZOOM);

            if (ogTileInfo) {
                 console.log(`HANDLEGLOBECLICK: Calculated OG TileInfo for ZL${TILE_SELECTION_ZOOM}: Z=${ogTileInfo.zoom}, X=${ogTileInfo.x}, Y=${ogTileInfo.y}`);
                 
                 const tileIdToLookup = getTileId([ogTileInfo.zoom, ogTileInfo.x, ogTileInfo.y]);
                 const targetLayer = window.userLayers[window.selectedLayerId]?.layer;
                 if (targetLayer) {
                     const source = targetLayer.getSource();
                     // Find features using the OpenGlobus-derived tileId
...
window.highlightedGlobeGroupId = groupId; // Set for globe highlighting
                         ...
                         console.log(`HANDLEGLOBECLICK: Clicked tile ${tileIdToLookup} (Z${TILE_SELECTION_ZOOM}) is not part of a known tileset group.`);
                 ...
                 console.warn("HANDLEGLOBECLICK: Could not calculate OG TileInfo from clicked location for ZL" + TILE_SELECTION_ZOOM);
            }
        } catch (e) {
