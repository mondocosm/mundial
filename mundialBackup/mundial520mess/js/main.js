...
async function generateAndShowTilesetInThreeJS(groupId, layerId, tilesetName) {
    ...
    if (tilesetFeatures.length === 0) {
        ...
        return;
// --- RECONSTRUCTED Interaction Mode Toggle Function ---
function toggleInteractionMode() { // No 'newMode' param, always toggles
    const interactionModeBtn = document.getElementById('interaction-mode-btn');
    if (!interactionModeBtn) {
        console.warn("[TOGGLE_MODE_DEBUG] Interaction mode button not found during toggle call.");
        return;
    }
    console.log(`[TOGGLE_MODE_DEBUG] Called. currentInteractionMode was: ${currentInteractionMode}`);
    console.log(`[TOGGLE_MODE_DEBUG] dragPanInteraction:`, dragPanInteraction, `dragBoxInteraction:`, dragBoxInteraction);
    ...
    console.log(`[TOGGLE_MODE_DEBUG] currentInteractionMode is now: ${currentInteractionMode}`);

    if (dragPanInteraction) {
        dragPanInteraction.setActive(currentInteractionMode === 'pan');
        console.log(`[TOGGLE_MODE_DEBUG] dragPanInteraction active: ${currentInteractionMode === 'pan'}`);
    } else {
        console.warn("[TOGGLE_MODE_DEBUG] dragPanInteraction is null during toggle call.");
    }

    if (dragBoxInteraction) {
        dragBoxInteraction.setActive(currentInteractionMode === 'selectTiles');
        console.log(`[TOGGLE_MODE_DEBUG] dragBoxInteraction active: ${currentInteractionMode === 'selectTiles'}`);
    } else {
        console.warn("[TOGGLE_MODE_DEBUG] dragBoxInteraction is null during toggle call.");
    }

    const mapElement = document.getElementById('map'); // Define mapElement here

    if (currentInteractionMode === 'pan') {
        interactionModeBtn.textContent = 'Pan/Select';
        interactionModeBtn.title = 'Current Mode: Pan (Click to switch to Select Tiles, Ctrl to toggle)';
        if (mapElement) mapElement.style.cursor = 'grab';
    } else { // 'selectTiles'
        interactionModeBtn.textContent = 'Select/Pan';
        interactionModeBtn.title = 'Current Mode: Select Tiles (Click to switch to Pan, Ctrl to toggle)';
        if (mapElement) mapElement.style.cursor = 'crosshair';
    }
    
    // Ensure updateSelectionActionsVisibility is defined or called safely
    if (typeof updateSelectionActionsVisibility === 'function') {
        updateSelectionActionsVisibility(); 
    } else {
        console.warn("[TOGGLE_MODE_DEBUG] updateSelectionActionsVisibility function not found.");
    }
}
// --- END RECONSTRUCTED Interaction Mode Toggle Function ---
    }
