...
document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            window.olMap = new ol.Map({
                ...
                view: new ol.View({
                    ...
                    maxZoom: 20, // Reduced to prevent map jumping and match desired limit
                    minZoom: 0
                })
            });
    ...
    function addLayerToList(layerId, layerName, isVisible) {
        ...
        // Only add edit, privacy, and delete buttons if it's not Layer 0
        if (layerId !== 'layer-0') { // layer0Id is 'layer-0'
            ...
            // For Layer 0, ensure the button container might still be needed for alignment or future non-destructive icons
            // but keep it empty of these specific buttons. Or, adjust styling if the container itself is not added.
            // For now, we just don't add the buttons. The container will be empty for Layer 0.
        }
    ...
    function zoomToTilesetGroup(groupId) {
        ...
        if (groupFeatures.length > 0) {
            ...
            if (!ol.extent.isEmpty(groupExtent)) {
                ...
                window.olMap.getView().fit(groupExtent, { padding: [50, 50, 50, 50], duration: 500, maxZoom: 20 }); // Match map view's maxZoom
    ...
    function initializeOpenGlobus() {
        // Content from the user-provided "reference" file (mundial5-12/js/main.js, lines 1570-1903)
        console.log("%cDEBUG: initializeOpenGlobus function ENTERED (using reference code).", "color: orange; font-weight: bold;");
        ...
        try {
            ...
            window.globus = new og.Globe({
                ...
                // terrain: globusRgbTerrainInstance, // Commented out to match reference, set in setTimeout
                lon: -74.0445,
                lat: 40.6892,
                alt: 3000,
            ...
            if (window.globus.planet) {
                ...
                console.log("DEBUG: ogSavedTilesetsLayer (CanvasTiles from reference) added to planet.");
                ...
                setTimeout(() => {
                    if (window.globus && window.globus.planet) {
                        const globusRgbTerrainInstance = new og.terrain.GlobusRgbTerrain({ heightFactor: 1.0, maxNativeZoom: 17 }); // Explicitly set maxNativeZoom
                        console.log("%cDEBUG: Freshly created globusRgbTerrainInstance in setTimeout.", "color: orange; font-weight: bold;", globusRgbTerrainInstance);
                        if (globusRgbTerrainInstance) {
                            console.log("%cDEBUG: maxNativeZoom of new instance in setTimeout BEFORE setTerrain:", "color: red; font-weight: bold;", globusRgbTerrainInstance.maxNativeZoom);
                        }
                        if (globusRgbTerrainInstance && typeof window.globus.planet.setTerrain === 'function') {
                            if (!window.globus.planet.terrain || window.globus.planet.terrain.name !== 'GlobusEarthRgb') {
                                ...
                                if (window.globus.planet.terrain && window.globus.planet.terrain.name === 'GlobusEarthRgb') {
                                    console.log("%cSUCCESS: GlobusRgbTerrain explicitly set and active!", "color: green; font-weight: bold;");
                                } else {
                                    console.error("ERROR: GlobusRgbTerrain NOT active after explicit setTerrain. Current terrain:", window.globus.planet.terrain);
                                }
                            } else {
                                 console.log("DEBUG: GlobusRgbTerrain already set (likely by constructor), skipping delayed setTerrain.");
                            }
                        } else {
                ...
                if (window.globus && window.globus.planet && window.globus.planet.events) {
                    ...
                    if (typeof handleGlobeClick === 'function') {
                        ...
                        console.error("initializeOpenGlobus: handleGlobeClick function not found globally!");
                    }
                } else {
            ...
            console.log("OpenGlobus initialization sequence completed in JS (using reference code).");
        } catch (e) {
            console.error("%cFATAL ERROR during OpenGlobus initialization (reference code logic):", "color: red; font-size: 1.2em; font-weight: bold;", e);
            window.globus = null;
        }
                ...
                if (typeof og !== 'undefined' && og.mercator && window.globus && window.globus.planet) {
                    ...
                    try {
                        ...
                        if (tileCoordsArr && tileCoordsArr.length === 3) {
                            ...
                            // }
                            console.log("PROGRAMMATIC TEST: selectedGlobeTiles logic commented out.");
    ...
    // Initializations are called within DOMContentLoaded

    function createTestTileset() {
        console.log("Attempting to create test tileset...");
        if (!window.olMap || !selectionSource || !selectionTileGrid || !window.userLayers || !window.userLayers['layer-0'] || !saveSelectionBtn) { // Use string literal
        ...
        if (window.selectedLayerId !== 'layer-0') { // Use string literal
            selectLayerInList('layer-0'); // Use string literal
        }
        
        const targetSource = window.userLayers['layer-0'].layer.getSource(); // Use string literal
        const existingFeaturesInLayer = targetSource.getFeatures();
...
// --- Base Layer Switching for OpenLayers ---
    const baseLayersOL = {
        'satellite': new ol.layer.Tile({
            source: new ol.source.XYZ({
                url: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
                attributions: 'Tiles © ArcGIS',
                maxZoom: 19 // Satellite imagery often doesn't go as high res
            }),
            isBaseLayer: true // Custom property to identify base layers
        }),
    ...
    if (baseLayerSelectOL && window.olMap) {
        // The initial layer (ArcGIS Satellite) is added in initializeOpenLayersMap.
        // We need to ensure it's marked as a base layer if we rely on that property.
        // Or, more simply, the first layer is always the base.
        const initialLayers = window.olMap.getLayers();
        if (initialLayers.getLength() > 0) {
            initialLayers.item(0).set('isBaseLayer', true); // Mark the initially added layer
        }
        baseLayerSelectOL.value = 'satellite'; // Default select value

        baseLayerSelectOL.addEventListener('change', function() {
            if (!window.olMap) return;
            const selectedValue = this.value;
            const layers = window.olMap.getLayers();
            // Remove current base layer (assuming it's the first one or marked with isBaseLayer)
            if (layers.getLength() > 0) {
                const currentBase = layers.item(0); // Assume first layer is base
                if (currentBase.get('isBaseLayer')) {
                     window.olMap.removeLayer(currentBase);
                } else { // Fallback if not marked, remove first layer anyway
                    window.olMap.removeLayer(currentBase);
                    console.warn("Base layer was not marked with isBaseLayer, removed first layer.");
                }
            }
            
            const newBaseLayer = baseLayersOL[selectedValue];
            if (newBaseLayer) {
                // newBaseLayer already has isBaseLayer: true from its definition
                layers.insertAt(0, newBaseLayer);
                console.log(`Switched OL base layer to: ${selectedValue}`);
            } else {
                console.error(`Could not find new base layer for value: ${selectedValue}`);
            }
        });
        console.log("DEBUG: OpenLayers base layer switcher initialized.");
    } else {
        if (!baseLayerSelectOL) console.warn("DEBUG: baseLayerSelectOL element not found.");
        if (!window.olMap) console.warn("DEBUG: window.olMap not ready for base layer switcher.");
    }
    }
...
// Initializations are called within DOMContentLoaded
    ...
    if (window.userLayers && window.userLayers[window.selectedLayerId] && userLayerList) {
        console.log("DEBUG: Setting up Layer 0 in UI list.");
        addLayerToList(window.selectedLayerId, window.userLayers[window.selectedLayerId].name, true);
        selectLayerInList(window.selectedLayerId);
    } else {
        console.warn("DEBUG: Could not set up Layer 0 in UI list - userLayers, selectedLayerId, or userLayerList not ready.", 
            { layers: !!window.userLayers, selId: window.selectedLayerId, listEl: !!userLayerList });
    }
    updateSelectionActionsVisibility(); // Initial state
    updateSelectedTileCountDisplay();   // Initial state
    console.log("DEBUG: Initial UI setup calls completed.");
