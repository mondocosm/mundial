...
function loadTestTilesetToLayer0() {
    ...
    if (saveSelectionBtn) {
        saveSelectionBtn.addEventListener('click', () => {
            ...
            selectedFeatures.forEach(feature => {
                ...
                const chosenColor = detailsColorPicker?.value; // Get value safely
                ...
                clonedFeature.set('color', chosenColor && chosenColor !== "" && !isBlack ? chosenColor : '#90EE90'); // Fallback to Light Green if empty or various forms of black
                let opacityValue = detailsFillOpacityInput ? parseFloat(detailsFillOpacityInput.value) : 0.6;
                if (isNaN(opacityValue)) {
                    opacityValue = 0.6; // Ensure fallback if parseFloat results in NaN
                }
            ...
            if (featuresToAdd.length > 0) {
                ...
                // updateSelectedTileCountDisplay(); // Now called by clearMapSelectionAndDetails
    ...
    function initializeOpenGlobus() {
            ...
            try {
                ...
                window.globus = new og.Globe({
                    ...
                    terrain: new og.terrain.EmptyTerrain(), // Initialize with EmptyTerrain for stability
                    lon: -74.0445,
                    lat: 40.6892,
                    alt: 3000, // Initial constructor altitude
                    resourcesSrc: "/packages/openglobus/res",
                    fontsSrc: "/packages/openglobus/res/fonts",
                    controls: [new og.control.LayerSwitcher()] // Add LayerSwitcher in constructor
                });
            ...
            if (window.globus.planet) {
                ...
                setTimeout(() => {
                    if (window.globus && window.globus.planet && typeof window.globus.planet.setTerrain === 'function') {
                        console.log("%cAttempting to switch to GlobusRgbTerrain in setTimeout...", "color: orange; font-weight: bold;");
                        ...
                        console.warn("DEBUG: Globus, planet, or setTerrain not ready for delayed terrain switch.");
                    }
                }, 1500); // Slightly longer delay for this attempt
                    console.log("DEBUG: tileCubeLayer instantiated, but NOT added to planet for debugging.");
                } else {
                    console.warn("DEBUG: Could not add tileCubeLayer, globus or planet not ready.");
                }
    ...
    Object.keys(viewToggleButtons).forEach(btnId => {
        ...
        if (button && panel) {
            button.addEventListener('click', function() {
                ...
                // The special handling for mainViewPanels that made them replace each other is removed.
                // All panels will now use the general toggle logic.

                // General logic for any panel: always show and activate
                this.classList.add('active');
                panel.style.display = 'block';

                // Log toggling for overlayPanels and other control panels if needed
                if (overlayPanels.includes(panel) || controlPanels.includes(panel) && !mainViewPanels.includes(panel) ) {
    ...
    if (window.userLayers && window.userLayers[window.selectedLayerId] && userLayerList) {
        console.log("DEBUG: Adding Layer 0 to list. selectedLayerId:", window.selectedLayerId, "Name:", window.userLayers[window.selectedLayerId].name);
        addLayerToList(window.selectedLayerId, window.userLayers[window.selectedLayerId].name, true);
        selectLayerInList(window.selectedLayerId);
    } else {
        console.error("DEBUG: Could not add Layer 0 to list. Prerequisites missing.", {
            userLayers: !!window.userLayers,
            selectedLayerId: window.selectedLayerId,
            userLayerList: !!userLayerList
        });
    }
