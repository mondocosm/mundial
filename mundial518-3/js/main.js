                ...
                if (event.target.classList.contains('xr-engine-btn')) {
                    ...
                    switch (engine) {
                        ...
                        case 'thirdroom':
                            ...
                            targetUrl = '../packages/thirdroom/index.html'; // Placeholder: assumes Third Room serves an index.html from its root
...
console.log("%cDEBUG: POST-INSTANTIATION of ogSavedTilesetsLayer & addLayer call", "color: yellow; font-weight: bold;");
    ...
    function handleGlobeClick(mouse, eventName) {
        ...
        if (currentInteractionMode !== 'selectTiles') {
            console.log(`HANDLEGLOBECLICK: Mode is '${currentInteractionMode}', not 'selectTiles'. Ignoring click for selection.`);
            return;
        }
    ...
    const controlPanels = [
        // document.getElementById('layer-switcher'), // No longer a standard draggable panel
...
// --- Scene Panel Logic ---
    ...
    function initMapLibreMap() {
        ...
        if (globeView && typeof globeView.getLonLat === 'function') {
            ...
            const olCenter = olView.getCenter(); // This is in EPSG:3857
            if (olCenter) {
                try {
                    initialCenter = ol.proj.transform(olCenter, 'EPSG:3857', 'EPSG:4326');
                } catch (e) { console.error("Error transforming OL center for ML:", e); }
        ...
        try {
            state.mapLibreMap = new maplibregl.Map({
                container: 'maplibre-map-container', // ID of the MapLibre div
                style: {
                    version: 8,
            ...
            state.mapLibreMap.on('load', () => {
                ...
                if (baseLayerSelectElement) { // Use the correct DOM element variable
                    updateMapLibreBasemap(baseLayerSelectElement.value);
                }
                
                updateZ21GridMapLibre(); // Initial grid draw for MapLibre
                updateSavedTilesetsMapLibre(); // Initial draw for saved tilesets
                state.mapLibreMap.on('moveend', updateZ21GridMapLibre);
                state.mapLibreMap.on('zoomend', updateZ21GridMapLibre);
                // Note: Saved tilesets are static, so they don't need move/zoom listeners for redraw,
                // but will need updates when window.userLayers changes.

                // Ensure resize observer is attached after map is loaded
                if (mapLibreMapContainerElement && !mapLibreMapContainerElement._resizeObserverAttached) {
                    new ResizeObserver(() => {
                        if (state.mapLibreMap && mapLibreMapContainerElement.offsetParent !== null) {
                            state.mapLibreMap.resize();
                        }
                    }).observe(mapLibreMapContainerElement);
                    mapLibreMapContainerElement._resizeObserverAttached = true;
                }
            });
    ...
    function switchGlobeView(selectedGlobeValue) {
        ...
        if (activeContainer) {
            ...
            if (selectedGlobeValue === 'radiogarden') {
                ...
                // Placeholder for iTowns initialization
                if (typeof initializeITowns === 'function') {
                    // initializeITowns(); // Call this only if not already initialized
